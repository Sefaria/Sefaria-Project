...
- **Consumes the component library** ([spec 1601](../2026-06-08-1601-react-component-library/plan.md)): `Input`, `FormField`, `ProviderButton`, `Captcha`, `Divider`, `AuthCard`, `LegalText`. No bespoke input/button CSS in this spec — if something's missing, it's added to the library, not here.
...
- **Provider buttons are the custom `ProviderButton`, not SDK-rendered chrome.** The Figma renders "Continue with Google/Apple" as Sefaria's secondary button (white, bordered, provider icon, black text) — so the React buttons are custom and trigger the SDK programmatically on click, following Google/Apple branding guidelines. (See library spec 1601.)
