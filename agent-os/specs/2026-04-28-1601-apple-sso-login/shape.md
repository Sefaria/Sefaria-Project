...
- **No silent account merging.** Email collision always returns a clear error. Users link accounts explicitly via the Settings UI.
- **Last-login-method guard is enforced at the service layer**, not just the UI. `unlink_provider()` raises `LastLoginMethodError` before removing a `SocialIdentity` if it would leave the user with no usable password and no other social identity.
