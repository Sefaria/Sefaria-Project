...
- **Auto-link on verified email, then erase the password (target behavior).** When an SSO login's email matches an existing account, the service **auto-links** a new `SocialIdentity`, **disables the legacy password** (`set_unusable_password()`), and logs the user in — rather than returning a collision error. The account becomes **SSO-only** thereafter (the user can re-add a password in Settings). Rationale: Google and Apple sign a verified email into the ID token, so the SSO user demonstrably controls the mailbox. This supersedes the original POC decision ("no silent merging / always return an error"). **Hardening dependency:** the provider verifier must check the `email_verified` claim before auto-linking — auto-linking on an unverified email is an account-takeover surface. Conversely, an SSO account is **blocked + informed (with a provider sign-in link) on both the email/password register and login paths**. Password-erase, the `email_verified` gate, and the login-path block are tracked in [`2026-06-08-1603-sso-phase2-linking-onetap`](../2026-06-08-1603-sso-phase2-linking-onetap/plan.md); the registration block already exists in `forms.py`.
