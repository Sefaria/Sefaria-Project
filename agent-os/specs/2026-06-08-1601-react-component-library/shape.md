...
- **Provider buttons follow the custom Figma component.** `ProviderButton` renders the secondary visual treatment from `Buttons [for now]`. Apple exposes a programmatic `signIn()` API. Google GIS does not, so its official interaction target is transparently layered over the custom visual while `AuthPage` retains popup/redirect configuration.
