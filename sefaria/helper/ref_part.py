...
def make_html(
    bulk_resolved_list: List[List[List[Union[ResolvedRef, AmbiguousResolvedRef]]]],
    texts: List[List[str]],
    output_filename,
    lang="he",
):
    ...
    def get_resolved_metadata(resolved: ResolvedRef, i: int) -> dict:
        metadata = {
            ...
            "resolved_part_strs": [
                p.term.slug if isinstance(p, TermContext) else p.text
                for p in resolved.resolved_parts
            ],
            "resolved_part_types": [p.type.name for p in resolved.resolved_parts],
            "resolved_part_classes": [
                p.__class__.__name__ for p in resolved.resolved_parts
            ],
            "context_ref": resolved.context_ref.normal()
            if resolved.context_ref
            else "N/A",
            "context_type": resolved.context_type.name
            if resolved.context_type
            else "N/A",
        }
        if RefPartType.RANGE.name in metadata["final_part_types"]:
            range_part = next(
                (
                    p
                    for p in resolved.raw_ref.parts_to_match
                    if p.type == RefPartType.RANGE
                ),
                None,
            )
            metadata["input_range_sections"] = [p.text for p in range_part.sections]
            metadata["input_range_to_sections"] = [
                p.text for p in range_part.toSections
            ]
    ...
    for bulk_resolved, inner_texts in zip(bulk_resolved_list, texts):
        for temp_resolved_list, input_text in zip(bulk_resolved, inner_texts):
            ...
            for resolved in temp_resolved_list:
                rlist = (
                    resolved.resolved_raw_refs if resolved.is_ambiguous else [resolved]
                )
            ...
            wrapped_text = wrap_chars_with_overlaps(
                input_text, chars_to_wrap, get_wrapped_text
            )
            html += f"""
...
def make_find_refs_response(
    resolved: List[List[Union[AmbiguousResolvedRef, ResolvedRef]]], with_text=False
):
    ref_results = []
    ref_data = {}
    resolved_ref_list = [
        resolved_ref for inner_resolved in resolved for resolved_ref in inner_resolved
    ]
    for resolved_ref in resolved_ref_list:
        resolved_refs = (
            resolved_ref.resolved_raw_refs
        ...
        ref_results += [
            {
                ...
                "refs": None
                if link_failed
                else [rr.ref.normal() for rr in resolved_refs],
            }
        ]
    ...
    response = {"results": ref_results, "refData": ref_data}
    return response
...
def make_ref_response_for_linker(oref: text.Ref, with_text=False) -> dict:
    ...
    if with_text:
        text_fam = text.TextFamily(
            oref, commentary=0, context=0, pad=False, stripItags=True
        )
        he = text_fam.he
        en = text_fam.text
        res.update(
...
def load_spacy_model(path: str) -> spacy.Language:
    ...
    from sefaria.spacy_function_registry import (
        inner_punct_tokenizer_factory,
    )  # this looks unused, but spacy.load() expects this function to be in scope
