...
class SheetCreatorManager(TrendManager):
    def __init__(self, period="alltime", public=False, valueThresholdMin=3):
        ...
        TrendManager.__init__(
            self, name, "SheetsCreated", period, valueThresholdMin=valueThresholdMin
        )
        self.public = public
    ...
    def getPersonInfo(self, trends):
        if self.public == False:
            ...
            person_info_public["value"] = TrendManager.getPersonInfo(self, trends)[
                "value"
            ]
            return person_info_public
...
class CustomTraitManager(TrendManager):
    ...
    def getPersonInfo(self, trends):
        person_info = {"key": self.key, "name": self.name, "period": self.period}
        try:
