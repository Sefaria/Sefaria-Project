...
from sefaria.helper.trend_manager import (
    CategoryTrendManager,
    CustomTraitManager,
...
def get_nationbuilder_connection():
    access_token_url = (
        "http://%s.nationbuilder.com/oauth/token" % sls.NATIONBUILDER_SLUG
    )
...
def get_tags_for_user(
    profile, trendManagers, custom_field_trend_managers=[]
):  # TODO - split up?
    # trends
    trends = {}
    ...
    custom_tags_info = [
        trend_manager.getPersonInfo(trends)
        for trend_manager in custom_field_trend_managers
    ]
...
def nationbuilder_update_all_tags():
    ...
    category_trend_managers = [
        CategoryTrendManager(category, period=period)
        for category in TOP_CATEGORIES
        for period in ["alltime", "currently"]
    ]
    trend_managers = []
    for period in ["currently", "alltime"]:
        trend_managers += [
            SheetReaderManager(period=period),
            SheetCreatorManager(period=period),
    ...
    custom_field_trend_managers = [
        CustomTraitManager("hebrew_ability", "HebrewAbility")
    ]
    for profile in db.profiles.find({"nationbuilder_id": {"$exists": True}}):
        tags_to_add, tags_to_remove, custom_tags_info = get_tags_for_user(
            profile, trend_managers, custom_field_trend_managers
        )
        print(tags_to_add)
        print(tags_to_remove)
        nationbuilder_update_person_tags(
            session,
            profile["nationbuilder_id"],
            json.dumps({"tagging": {"tag": tags_to_add}}),
            json.dumps({"tagging": {"tag": tags_to_remove}}),
        )
        nationbuilder_update_person_custom_fields(
            session, profile["nationbuilder_id"], custom_tags_info
        )
...
def nationbuilder_update_person_custom_fields(session, id, person_info_list):
    headers = {"Content-type": "application/json", "Accept": "application/json"}
    person_data = {
        person_info["name"]: person_info["value"] for person_info in person_info_list
    }
    req_add = session.put(
        update_person(id), data=json.dumps({"person": person_data}), headers=headers
    )
    print(req_add.json())
    ...
    while next_endpoint:
        ...
        for attempt in range(0, 3):
            try:
                ...
                if (
                    "nation-ratelimit-remaining" in res.headers
                    and res.headers["nation-ratelimit-remaining"] == "0"
                ):
                    time.sleep(10)
                    print("sleeping")
                break
                ...
                session = get_nationbuilder_connection()
                print(
                    "Trying again to access and process {}. Attempts: {}. Exception: {}".format(
                        next_endpoint, attempt + 1, e
                    )
                )
                print(next_endpoint)
        else:
            session.close()
            raise Exception(
                "Error when attempting to connect to and process " + next_endpoint
            )
