...
def get_community_page_data(language="english", refresh=False):
    ...
    in_memory_cache.set(
        "community-page-data-{}".format(language), data, timeout=60 * 60
    )
...
def get_community_page_items(
    date=None, language="english", diaspora=True, refresh=False
):
    """
...
def get_featured_item(data, date):
    ...
    return {"heading": todays_data["Block Title"], "sheet": sheet}
...
def get_featured_sheet_from_topic(slug):
    ...
    sheets = [
        s
        for s in sheets
        if not is_hebrew(s["title"]) and s["summary"] and len(s["summary"]) > 140
    ]
...
def sheets_with_content_by_category(cat, print_results=True):
    ...
    sheets = db.sheets.find(
        {"status": "public"},
        {"title": 1, "id": 1, "owner": 1, "includedRefs": 1, "status": 1},
    )
    for sheet in sheets:
