...
def update_authors_data():
    ...
    eras = {"Gaonim": "GN", "Rishonim": "RI", "Achronim": "AH", "Contemporary": "CO"}
    era_slug_map = {
        "GN": "geon-person",
        "RI": "rishon-person",
        "AH": "achron-person",
    ...
    isa_object_aggregate_map = {"tosafot": "group-of-rishon-people"}
    ...
    for l in rows:
        ...
        if (
            re.search(
                rf"^{re.escape(SluggedAbstractMongoRecord.normalize_slug(primary_title))}\d*$",
                slug,
            )
            is None
        ):
            error_texts.append(
                f"ERROR: slug '{slug}' does not match primary title '{primary_title}'. Expected slug '{SluggedAbstractMongoRecord.normalize_slug(primary_title)}'"
            )
            has_slug_issues = True
        if len(l[9]) == 0:
            error_texts.append(
                f"ERROR: slug '{slug}' must have column 'Halachic Era' filled in."
            )
            has_slug_issues = True
        if len(slug.strip()) == 0:
            continue
    ...
    for slug, count in internal_slug_count.items():
        ...
        if SluggedAbstractMongoRecord.normalize_slug(slug) != slug:
            error_texts.append(
                f"ERROR: slug '{slug}' does not match slugified version which is '{SluggedAbstractMongoRecord.normalize_slug(slug)}'. Please slugify in the sheet."
            )
            has_slug_issues = True
    if has_slug_issues:
        return (
            ["Errors:"]
            + error_texts
            + ["Please Correct these errors and re-run the update:"]
        )
    ...
    response_texts.append(
        f"Links Deleted '{db.topic_links.count_documents(link_query)}'",
    )
    ...
    for irow, l in enumerate(rows):
        ...
        if p.get_property("era"):
            to_topic = isa_object_aggregate_map.get(
                p.slug, era_slug_map[p.get_property("era")]
            )
            itl = IntraTopicLink(
                {
                    "toTopic": to_topic,
        ...
        for i, link_type_slug in rowmap.items():
            if l[i]:
                for pkey in l[i].split(","):
                    ...
                    to_slug, from_slug = (
                        (from_slug, to_slug)
                        if link_type_slug in flip_link_dir
                        else (to_slug, from_slug)
                    )
    ...
    link_query = {"generatedBy": "update_authors_data"}
    response_texts.append(
        f"links created '{db.topic_links.count_documents(link_query)}' "
    )
