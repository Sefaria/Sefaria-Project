...
def get_topic(
    ...
    if with_links and with_refs:
        ...
        ref_links = [
            l.contents()
            for l in all_links
            if isinstance(l, RefTopicLink)
            and (len(ref_link_type_filters) == 0 or l.linkType in ref_link_type_filters)
        ]
    else:
        if with_links:
            intra_links = [
                l.contents() for l in topic_obj.link_set(_class="intraTopic")
            ]
        if with_refs:
            query_kwargs = (
                {"linkType": {"$in": list(ref_link_type_filters)}}
                if len(ref_link_type_filters) > 0
                else None
            )
            ref_links = [
                l.contents()
                for l in topic_obj.link_set(
                    _class="refTopic", query_kwargs=query_kwargs
                )
            ]
    if with_links:
        response["links"] = group_links_by_type(
            "intraTopic", intra_links, annotate_links, group_related
        )
    if with_refs:
    ...
    if with_indexes and isinstance(topic_obj, AuthorTopic):
        response["indexes"] = [
            {"text": text_dict, "url": url}
            for (url, text_dict) in topic_obj.get_aggregated_urls_for_authors_indexes()
        ]

    if getattr(topic_obj, "isAmbiguous", False):
        possibility_links = topic_obj.link_set(
            _class="intraTopic",
            query_kwargs={"linkType": TopicLinkType.possibility_type},
        )
        possibilities = []
        for link in possibility_links:
            ...
            if possible_topic is None:
                continue
            possibilities += [
                possible_topic.contents(annotate_time_period=annotate_time_period)
            ]
...
def group_links_by_type(link_class, links, annotate_links, group_related):
    link_dups_by_type = defaultdict(
        set
    )  # duplicates can crop up when group_related is true
    grouped_links = {}
    agg_field = "links" if link_class == "intraTopic" else "refs"

    if link_class == "intraTopic" and len(links) > 0 and annotate_links:
        link_topic_dict = {
            other_topic.slug: other_topic
            for other_topic in TopicSet(
                {"$or": [{"slug": link["topic"]} for link in links]}
            )
        }
    else:
        link_topic_dict = {}
    for link in links:
        ...
        if link_type_slug in grouped_links:
            ...
            if link_type.get("pluralDisplayName", is_inverse, False):
                grouped_links[link_type_slug]["pluralTitle"] = link_type.get(
                    "pluralDisplayName", is_inverse
                )
...
def get_all_topics(limit=1000, displayableOnly=True):
    query = (
        {"shouldDisplay": {"$ne": False}, "numSources": {"$gt": 0}}
        if displayableOnly
        else {}
    )
...
def sort_refs_by_relevance(a, b):
    ...
    return (bord.get("numDatasource", 0) * bord.get("tfidf", 0)) - (
        aord.get("numDatasource", 0) * aord.get("tfidf", 0)
    )


def get_random_topic(good_to_promote=True) -> Optional[Topic]:
    query = {"good_to_promote": True} if good_to_promote else {}
    random_topic_dict = list(
        db.topics.aggregate([{"$match": query}, {"$sample": {"size": 1}}])
    )
...
def get_random_topic_source(topic: Topic) -> Optional[Ref]:
    random_source_dict = list(
        db.topic_links.aggregate(
            [
                {
                    "$match": {
                        "toTopic": topic.slug,
...
def get_topics_for_ref(tref, annotate=False):
    ...
    if annotate:
        if len(serialized) > 0:
            link_topic_dict = {
                ...
                for topic in TopicSet(
                    {"$or": [{"slug": link["topic"]} for link in serialized]}
                )
            }
        else:
            link_topic_dict = {}
        serialized = list(
            filter(
                None,
                (annotate_topic_link(link, link_topic_dict) for link in serialized),
            )
        )
...
def generate_all_topic_links_from_sheets(topic=None):
    ...
    for slug, related_topics_to_slug in tqdm(
        all_related_topics.items(), desc="creating sheet related topic links"
    ):
        ...
        related_topics = [
            related_topic
            for related_topic in related_topics_to_slug.items()
        ...
        for related_topic, user_votes in related_topics:
            if related_topic == slug:
                continue
            key = (
                (related_topic, slug) if related_topic > slug else (slug, related_topic)
            )
            if already_created_related_links.get(key, False):
                continue
            already_created_related_links[key] = True
            related_links += [
                {"a": related_topic, "b": slug, "user_votes": len(user_votes)}
            ]
    topic_idf_dict = {
    ...
    for tref, related_topics_to_tref in tqdm(
        ...
        topic_scores = [
            ...
            for slug, numerator, owners in zip(
                related_topics_to_tref.keys(), numerator_list, owner_counts
            )
        ]
        ...
        for slug, _, owners in filter(
            lambda x: x[1] >= TFIDF_CUTOFF and x[2] >= OWNER_THRESH, topic_scores
        ):
    ...
    for slug, sources in tqdm(raw_topic_ref_links.items()):
        ...
        for cluster in clustered:
            ...
            for icount, (_, temp_count) in enumerate(counts):
                ...
                if temp_count > (
                    STD_DEV_CUTOFF * count_std + count_xbar
                ) or temp_count < (count_xbar - STD_DEV_CUTOFF * count_std):
                    temp_range = counts[curr_range_start][0].to(counts[icount - 1][0])
                    temp_sources += [
                        (
                            temp_range.normal(),
                            [r.normal() for r in temp_range.range_list()],
                            count_xbar,
                        )
                    ]
...
def generate_sheet_topic_links():
    ...
    sheet_list = db.sheets.find(
        {"status": "public", "assignment_id": {"$exists": 0}}, projection
    )
    sheet_links = []
...
def calculate_tfidf_related_sheet_links(related_links):
    ...
    idf_dict = {
        slug: math.log2(len(docs) / count) for slug, count in doc_topic_counts.items()
    }
    ...
    if len(text) != 0:
        ...
        words = list(
            filter(
                ...
                [re.sub("^\u05d5", "", w.replace('"', "")) for w in text.split()],
            )
        )
    return words
...
def calculate_mean_tfidf(ref_topic_links):
    ...
    for l in tqdm(ref_topic_links, total=len(ref_topic_links), desc="process text"):
        ...
        if l.ref not in ref_words_map:
            ...
            ref_words_map[l.ref] = tokenize_words_for_tfidf(
                oref.text("he").as_string(), stopwords
            )
    ...
    for topic, ref_list in ref_topic_map.items():
        ...
        for tref in ref_list:
            ...
            topic_tref_score_map[(topic, tref)] = sum(
                (tfidf_dict[w] - tref_tf[tref].get(w, 0) * idf_dict[w]) for w in words
            ) / len(words)
...
def calculate_pagerank_scores(ref_topic_map):
    ...
    for topic, ref_list in tqdm(ref_topic_map.items(), desc="calculate pr"):
        ...
        for tref in ref_list:
            ...
            oref_list += [oref]
        seg_ref_map = {
            r.normal(): [rr.normal() for rr in r.all_segment_refs()] for r in oref_list
        }
        oref_pr_list = pagerank_rank_ref_list(
            oref_list, normalize=True, seg_ref_map=seg_ref_map
        )
...
def calculate_other_ref_scores(ref_topic_map):
    ...
    comp_date_map = {}
    order_id_map = {}
    for topic, ref_list in tqdm(
        ref_topic_map.items(), desc="calculate other ref scores"
    ):
        ...
        for tref in ref_list:
            ...
            langs_available[(topic, tref)] = [
                lang for lang in LANGS_CHECKED if oref.is_text_fully_available(lang)
            ]
            for seg_ref in tref_range_lists[tref]:
                seg_ref_counter[seg_ref] += 1
        for tref in ref_list:
            range_list = tref_range_lists.get(tref, None)
            num_datasource_map[(topic, tref)] = (
                0
                if (range_list is None or len(range_list) == 0)
                else max(seg_ref_counter[seg_ref] for seg_ref in range_list)
            )
...
def update_ref_topic_link_orders(sheet_source_links, sheet_topic_links):
    other_ref_topic_links = list(
        RefTopicLinkSet(
            {
                "is_sheet": False,
                "generatedBy": {"$ne": TopicLinkHelper.generated_by_sheets},
            }
        )
    )
    ...
    (
        ...
        comp_date_map,
        order_id_map,
    ) = calculate_other_ref_scores(ref_topic_map)
    ...
    def get_sheet_order(topic_slug, sheet_id):
            ...
            for tref in sheet["includedRefs"]:
                try:
                    ...
                    if oref is None:
                        continue
                    includedRefs += [
                        [sub_oref.normal() for sub_oref in oref.all_segment_refs()]
                    ]
                except InputError:
                    continue
        ...
        for ref_range in sheet["includedRefs"]:
            if len(ref_range) == 0:
                continue
            total_pr += sum(
                [pr_seg_map.get((topic_slug, ref), 1e-5) for ref in ref_range]
            ) / len(
                ref_range
            )  # make default pr epsilon so that relevance can tell difference between sheets that have sources on topic and those that dont
        avg_pr = (
            0
            if len(sheet["includedRefs"]) == 0
            else total_pr / len(sheet["includedRefs"])
        )

        # relevance based on other topics on this sheet
        other_topic_slug_set = {
            t["slug"] for t in sheet.get("topics", []) if t["slug"] != topic_slug
        }
        total_tfidf = 0
        for other_topic_slug in other_topic_slug_set:
            intra_topic_link = IntraTopicLink().load(
                {
                    "$or": [
        ...
        avg_tfidf = (
            1
            if len(other_topic_slug_set) == 0
            else (total_tfidf / len(other_topic_slug_set)) + 1
        )

        relevance = (
            0 if avg_pr == 0 else avg_pr + avg_tfidf
        )  # TODO: test this equation again
        ...
        if not isinstance(sheet_title, str):
            ...
            title_lang = (
                "english"
                if re.search(r"[a-zA-Z]", re.sub(r"<[^>]+>", "", sheet_title))
                is not None
                else "hebrew"
            )
        return {
    ...
    for l in tqdm(all_ref_topic_links, desc="update link orders"):
        if l.is_sheet:
            setattr(
                l, "order", get_sheet_order(l.toTopic, int(l.ref.replace("Sheet ", "")))
            )
        else:
            key = (l.toTopic, l.ref)
            try:
...
def update_intra_topic_link_orders(sheet_related_links):
    ...
    other_related_links = IntraTopicLinkSet(
        {"generatedBy": {"$ne": TopicLinkHelper.generated_by_sheets}}
    )
    for link in tqdm(
        chain(other_related_links, sheet_related_links), desc="update intra orders"
    ):
    ...
    for topic_slug, topic_links in topic_link_dict.items():
        idf_dict[topic_slug] = (
            0 if len(topic_links) == 0 else math.log2(N / len(topic_links))
        )
...
def get_top_topic(sheet):
    ...
    def topic_score(t):
        rtl = RefTopicLink().load(
            {"toTopic": t["slug"], "ref": "Sheet {}".format(sheet.get("id"))}
        )
        if rtl is None:
            return t["slug"], 0
...
def add_num_sources_to_topics():
    updates = [
        {
            "numSources": RefTopicLinkSet(
                {"toTopic": t.slug, "linkType": {"$ne": "mention"}}
            ).count(),
            "_id": t._id,
        }
        for t in TopicSet()
    ...
    db.topics.bulk_write(
        [
            UpdateOne({"_id": t["_id"]}, {"$set": {"numSources": t["numSources"]}})
            for t in updates
        ]
    )
...
def calculate_popular_writings_for_authors(top_n, min_pr):
    ...
    for rd in tqdm(rds, total=rds.count()):
        try:
            tref = rd.ref.replace(
                "&amp;", "&"
            )  # TODO this is a stopgap to prevent certain refs from failing
            oref = Ref(tref)
    ...
    for author, rd_list in by_author.items():
        ...
        if len(rd_list) == 0:
            continue
        top_rd_indexes = sorted(
            range(len(rd_list)), key=lambda i: rd_list[i]["pagesheetrank"]
        )[-top_n:]
...
def recalculate_secondary_topic_data():
    ...
    print(
        f"Num to Update {len(list(filter(lambda x: getattr(x, '_id', False), all_ref_links + related_links)))}"
    )
    print(
        f"Num to Insert {len(list(filter(lambda x: not getattr(x, '_id', False), all_ref_links + related_links)))}"
    )

    db.topic_links.bulk_write(
        [
            UpdateOne({"_id": l._id}, {"$set": {"order": l.order}})
            if getattr(l, "_id", False)
            else InsertOne(l.contents(for_db=True))
