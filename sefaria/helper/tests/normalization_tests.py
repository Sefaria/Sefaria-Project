...
def test_normalizer_composer():
    ...
    nsc = NormalizerComposer(
        ["html", "parens-plus-contents", "brackets", "double-space"]
    )
    assert nsc.normalize(text) == normalized
    text_to_remove = nsc.find_text_to_remove(text)
    assert len(text_to_remove) == 5
...
def test_html_normalizer_for_empty_prefix():
    ...
    ne_inds = normalizer.convert_normalized_indices_to_unnormalized_indices(
        [ne_norm_prefix_inds], mapping
    )[0]
    # actual test
...
def test_nested_itag():
    ...
    assert (
        text[s:e]
        == """<i class="footnote">bull<sup>nested</sup><i class="footnote">The</i>.</i>"""
    )
...
def test_word_to_char():
    ...
    start, end = result
    assert (
        test_string[start:end] == "go here\n\nhello "
    )  # TODO used to not have trailing space. not sure how critical this is.
