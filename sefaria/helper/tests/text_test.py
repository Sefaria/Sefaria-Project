...
def test_modify_text_by_function():
    ...
    original = TextChunk(
        Ref("Job"), vtitle="The Holy Scriptures: A New Translation (JPS 1917)"
    )
    total_spaces = (
        JaggedTextArray(original.text).flatten_to_string(joiner="|").count(" ")
    )
    ...
    modify_text_by_function(
        "Job", "TextChangeTest", "en", lambda x, sections: x.replace(" ", "$"), 23432
    )
    modified = TextChunk(Ref("Job"), vtitle="TextChangeTest")
    total_dollars = (
        JaggedTextArray(modified.text).flatten_to_string(joiner="|").count("$")
    )
    v.delete()
