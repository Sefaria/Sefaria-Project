...
def create_simple_text():
    ...
    Link(
        {"refs": ["MigrateBook 1:1", "Guide for the Perplexed, Part 1"], "type": "None"}
    ).save()
    ...
    Link(
        {
            "refs": [
                "MigrateBook 5:4",
                "Guide for the Perplexed, Introduction, Introduction 3",
            ],
            "type": "None",
        }
    ).save()

    Link({"refs": ["MigrateBook 1:2-5", "Genesis 3"], "type": "None"}).save()

    Link({"refs": ["MigrateBook 2", "Genesis 2"], "type": "None"}).save()
...
def setup_module():
    ...
    # add some links
    Link({"refs": ["Delete Me, Part1 1:1", "Shabbat 2a:5"], "type": "None"}).save()
    Link(
        {"refs": ["Delete Me, Part1 2:1", "Delete Me, Part2 2:1"], "type": "None"}
    ).save()
    Link({"refs": ["Delete Me, Part1 3", "Shabbat 2a:5"], "type": "None"}).save()
    Link({"refs": ["Delete Me, Part2 1:1", "Shabbat 2a:5"], "type": "None"}).save()
    Link({"refs": ["Delete Me, Part2 3", "Shabbat 2a:5"], "type": "None"}).save()
    ...
    sheet = save_sheet(
        create_test_sheet(
            ["Delete Me, Part1 1:1", "MigrateBook 1:1", "MigrateBook 4:1"]
        ),
        1,
    )
...
def test_migrate_to_complex_structure():
    ...
    assert TextChunk(children[0].ref(), "en", "Schema Test").text == [
        [p1, p7, p7, p7, p7],
        ["", p2],
    ]
    ...
    assert isinstance(
        Link().load(
            {
                "refs": sorted(
                    ["MigrateBook, Part 1 1:1", "Guide for the Perplexed, Part 1"]
                ),
            }
        ),
        Link,
    ...
    assert isinstance(
        Link().load(
            {
                "refs": sorted(
                    ["MigrateBook, Part 1 2:2", "Guide for the Perplexed, Part 1 2"]
                ),
            }
        ),
        Link,
    ...
    assert isinstance(
        Link().load(
            {
                "refs": sorted(
                    ["MigrateBook, Part 2 3", "Guide for the Perplexed, Part 2 4-8"]
                ),
            }
        ),
        Link,
    ...
    assert isinstance(
        Link().load(
            {
                "refs": sorted(
                    ["MigrateBook, Part 3 1", "Guide for the Perplexed, Part 3 1"]
                ),
            }
        ),
        Link,
    ...
    assert isinstance(
        Link().load(
            {
                "refs": sorted(["MigrateBook, Part 1 1:2-5", "Genesis 3"]),
            }
        ),
        Link,
    )
    assert isinstance(
        Link().load(
            {
                "refs": sorted(["MigrateBook, Part 1 2", "Genesis 2"]),
            }
        ),
        Link,
    )
...
def test_change_node_title():
    ...
    assert isinstance(
        Link().load({"refs": ["Delete Me, 1st Part 1:1", "Shabbat 2a:5"]}), Link
    )
    assert isinstance(
        Link().load({"refs": ["Delete Me, 1st Part 2:1", "Delete Me, Part2 2:1"]}), Link
    )
    assert isinstance(Note().load({"ref": "Delete Me, 1st Part 1:1"}), Note)
    assert (
        Link().load({"refs": ["Delete Me, Part1 2:1", "Delete Me, Part2 2:1"]}) is None
    )
...
def test_change_node_structure_complex():
    ...
    assert isinstance(
        Link().load(
            {
                "refs": ["Delete Me, Part1 1:1:1", "Shabbat 2a:5"],
            }
        ),
        Link,
    )
    assert isinstance(
        Link().load(
            {
                "refs": ["Delete Me, Part1 2:1:1", "Delete Me, Part2 2:1"],
            }
        ),
        Link,
    )
    assert isinstance(
        Link().load(
            {
                "refs": ["Delete Me, Part1 3:1", "Shabbat 2a:5"],
            }
        ),
        Link,
    )
    ...
    assert (
        library.get_index("Delete Me").get_alt_structure("alt").wholeRef
        == "Delete Me, Part1 1:2:1-3:1:1"
    )
