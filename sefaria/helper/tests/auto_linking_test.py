...
class Test_AutoLinker(object):
    link_set_lambda = lambda x: LinkSet(
        {
            "refs": {"$regex": Ref(x).regex()},
            "auto": True,
    ...
    def setup_class(cls):
        ...
        chunk = TextChunk(
            Ref("Many to One on Genesis, Introduction"), "en", "Schema Test"
        )
        chunk.text = p1
        chunk.save()
        ...
        Link(
            {
                "refs": [
                    "Many to One on Genesis, Introduction 2:1",
                    "Many to One on Genesis 2:1:2",
                ],
                "type": "commentary",
                "auto": True,
                "generated_by": "intro_parser",
        ...
        p1 = [["intro intro", "intro"], ["intro"], ["intro", "", "intro"]]
        chunk = TextChunk(
            Ref("One to One on Genesis, Introduction"), "en", "Schema Test"
        )
        chunk.text = p1
        chunk.save()
        ...
        Link(
            {
                "refs": [
                    "One to One on Genesis, Introduction 2:1",
                    "One to One on Genesis 2:1",
                ],
                "type": "commentary",
                "auto": True,
                "generated_by": "intro_parser",
        ...
        cls.desired_link_counts["Many to One on Genesis"] = link_set_lambda(
            "Many to One on Genesis"
        ).count()
        cls.desired_link_counts["One to One on Genesis"] = link_set_lambda(
            "One to One on Genesis"
        ).count()
    ...
    def test_refresh_commentary_links_one_to_one_default_node(self):
        ...
        Link(
            {
                ...
                "generated_by": linker._generated_by_string,
                "type": "commentary",
                "auto": True,
            }
        ).save()
        ...
        linker.refresh_links()
        link_count = LinkSet(
            {"generated_by": linker._generated_by_string, "refs": {"$regex": regex}}
        ).count()
        assert desired_link_count == link_count
    ...
    def test_refresh_links_with_text_save_many_to_one_default_node(self):
        ...
        stext = [
            "Intro first segment text",
            "Intro second segment text",
            "Intro third segment text",
        ]
        oref = Ref("{}, Introduction 1".format(title))
        tracker.modify_text(1, oref, "test", "en", stext)
        base_link_count = LinkSet(
            {
                "refs": {"$regex": regex},
                "auto": True,
                "generated_by": "add_commentary_links",
    ...
    def test_bulk_refresh_links_with_text_save_complex(self):
        ...
        version = Version().load(
            {"title": title, "versionTitle": vtitle, "language": lang}
        )
        text_map = {
        ...
        assert (
            version.chapter["Kadesh"][0][3] == ""
        )  # 4th element should have been padded

        # now delete
        for k in text_map.keys():
            text_map[
                k
            ] = ""  # set all values to empty string. which should trigger trim_ending_whitespace to delete them
        tracker.modify_bulk_text(1, version, text_map)
        link_count = LinkSet(
            {
                "refs": {"$regex": regex},
