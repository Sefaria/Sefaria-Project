...
def setup_module(module):
    ...
    texts[
        "bible_ref"
    ] = "אם נביא הוא נבואתו מסתלקת ממנו מדבורה דכתיב (שופטים ה, ז) חדלו פרזון בישראל"
    texts[
        "he_bible_begin"
    ] = "(שמות כא, ד) אם אדוניו יתן לו אשה"  # These work, even though the presentation of the parens may be confusing.
    ...
    texts[
        "2talmud"
    ] = "ודין גזל קורה ובנאה בבירה מה יהא עליה (גיטין נה א). ודין גזל בישוב ורצה להחזיר במדבר (ב''ק קיח א). ודין גזל והקדיש"
    texts[
        "bk-abbrev"
    ] = 'ודין גזל קורה ובנאה בבירה מה יהא עליה (גיטין נה א). ודין גזל בישוב ורצה להחזיר במדבר (ב"ק קיח א). ודין גזל והקדיש'
    ...
    texts[
        "ignored_middle"
    ] = "(תהלים לז, א) אל תתחר במרעים ולא עוד אלא שדרכיו מצליחין שנא תהלים י, ה יחילו דרכיו בכל עת ולא עוד אלא שזוכה בדין שנאמר מרום משפטיך מנגדו ולא עוד אלא שרואה בשונאיו שנאמר כל צורריו יפיח בהם איני והאמר ר יוחנן משום רש בן יוחי מותר להתגרות ברשעים בעולם הזה שנא (משלי כח, ד)"
    texts[
        "weird_ref"
    ] = "In this string The Book of Susanna 1.2 should match the long regex only, but Leviticus 12.4 should match both of them"
    texts[
        "weird_ref_he"
    ] = "המקור (ספר שושנה א ב) אמור לעבוד רק בחיפוש המלא, אבל המקור (ויקרא יב ד) צריך לעבוד בשניהם"
...
class Test_get_refs_in_text(object):
    ...
    def test_bare_digits(self, citing_only):
        assert set() == set(
            library.get_refs_in_string(texts["barenum"], citing_only=citing_only)
        )  # Fixed in 5a4b813819ef652def8360da2ac1b7539896c732
    ...
    def test_multiple(self, citing_only):
        ...
        assert 2 == len(ref)
        assert {Ref("Brachot 7b"), Ref("Isaiah 12:13")} == set(
            library.get_refs_in_string(texts["2ref"], citing_only=True)
        )

    @pytest.mark.parametrize(("citing_only"), (True, False))
    def test_inner_parenthesis(self, citing_only):

        ref = library.get_refs_in_string(
            "Bereishit Rabbah (55:7)", "en", citing_only=citing_only
        )
        assert 1 == len(ref)
    ...
    def test_commentary(self, citing_only):
        ...
        assert library.get_refs_in_string(s, "en", citing_only=citing_only) == [
            Ref("Rashi on Genesis 2:5:3")
        ]
        assert library.get_refs_in_string(s2, "en", citing_only=citing_only) == [
            Ref("Rashi on Genesis 3:4"),
            Ref("Exodus 5:2"),
        ]
        assert library.get_refs_in_string(s3, "en", citing_only=citing_only) == [
            Ref("Genesis 2:3")
        ]
        assert library.get_refs_in_string(s4, "en", citing_only=False) == [
            Ref("Rashi on Shabbat 25a:5")
        ]  # Rashi on Shabbat has `is_citing=False`

    @pytest.mark.parametrize(("citing_only"), (True, False))
    def test_citing_only(self, citing_only):
        matched_refs = library.get_refs_in_string(
            texts["weird_ref"], lang="en", citing_only=citing_only
        )
        if citing_only:
            assert matched_refs == [Ref("Leviticus 12.4")]
        else:
            assert matched_refs == [
                Ref("The Book of Susanna 1.2"),
                Ref("Leviticus 12.4"),
            ]
    ...
    def test_supposed_ref_graceful_fail(self, citing_only):
        matched_refs = library.get_refs_in_string(
            "What's important is that you get the Job done.",
            lang="en",
            citing_only=citing_only,
        )
        assert matched_refs == []
    ...
    def test_ranged_ref(self, citing_only):
        trefs = [
            "Deuteronomy 23:8-9",
            "Job.2:3-3:1",
            "Leviticus 15:3 - 17:12",
            "Shabbat 15a-16b",
            "Shabbat 15a:15-15b:13",
            "Shabbat 15a:10-13",
            "Rashi on Exodus 3:1-3:10",
            "Rashi on Exodus 3:1:1-3:1:3",
            "Rashi on Exodus 3:1:1-1:3",
            "Rashi on Exodus 3:1:1-3",
            "Berakhot 3a-b",
        ]
        test_strings = [
            "I am going to quote a range. hopefully you can parse it. ({}) plus some other stuff.".format(
                temp_tref
            )
        ...
        for i, test_string in enumerate(test_strings):
            matched_refs = library.get_refs_in_string(
                test_string, lang="en", citing_only=citing_only
            )
    ...
    def test_ranged_ref_not_cited(self):
        trefs = [
            "Berakhot 2a-b",
            "Rashi on Shabbat 15a:10-13",
            "Shulchan Arukh, Orach Chayim 444:4–6",
        ]  # NOTE the m-dash in the Shulchan Arukh ref
        test_strings = [
            "I am going to quote a range. hopefully you can parse it. ({}) plus some other stuff.".format(
                temp_tref
            )
        ...
        for i, test_string in enumerate(test_strings):
            matched_refs = library.get_refs_in_string(
                test_string, lang="en", citing_only=False
            )
            assert matched_refs == [Ref(trefs[i])]

    def test_bad_ranged_refs(self):
        trefs = [
            "Rashi on Shabbat 15a:4-16a",
            "Rashi on Shabbat 2a:2-2b",
            "Rashi on Shabbat 2b:1:1-2a:2:1",
            "Rashi on Shabbat 2b-2a",
            "Genesis 3:1-2:5",
            "Genesis 3-4:2",
        ]
        test_strings = [
            "I am going to quote a range. hopefully you can NOT parse it. ({}) plus some other stuff.".format(
                temp_tref
            )
            for temp_tref in trefs
        ]
    ...
    def test_wrap_refs(self, citing_only):
        ...
        res = reduce(
            lambda a, b: a
            + '<a class ="refLink" href="/{}" data-ref="{}">{}</a> blah blah '.format(
                b[0].url(), b[0].normal(), b[1]
            ),
            list(zip(orefs, trefs)),
            "",
        )
        wrapped = library.get_wrapped_refs_string(
            st, lang="en", citing_only=citing_only
        )
        assert wrapped == res
...
class Test_he_get_refs_in_text(object):
    ...
    def test_divrei_hayamim(self, citing_only):
        ref = library.get_refs_in_string(
            "(דברי הימים ב לב, יט)", citing_only=citing_only
        )
        assert 1 == len(ref)
    ...
    def test_double_ref_alt(self, citing_only):
        ref = library.get_refs_in_string(
            "עמי הארץ (דברי הימים ב לב,יט), וכתיב (הושע ט ג): לא ישבו בארץ",
            citing_only=citing_only,
        )
        assert 2 == len(ref)
    ...
    def test_out_of_brackets(self, citing_only):
        ref = library.get_refs_in_string(
            texts["ignored_middle"], citing_only=citing_only
        )
        assert 2 == len(ref)
    ...
    def test_two_single_quotes(self, citing_only):
        ref = library.get_refs_in_string(
            "עין ממש דכתיב (במדבר ל''ה) ולא תקחו", citing_only=citing_only
        )
        assert 1 == len(ref)
        assert ref[0] == Ref("במדבר ל''ה")

        ref = library.get_refs_in_string(
            "דאמר קרא (שופטים כ י''א) ויאסף כל איש", citing_only=True
        )
        assert 1 == len(ref)
    ...
    def test_spelled_mishnah(self, citing_only):
        ref = library.get_refs_in_string(
            'דתנן (טהרות פ"ג משנה ב) רמ אומר', citing_only=citing_only
        )
        assert 1 == len(ref)
    ...
    def test_beyond_length(self, citing_only):
        ref = library.get_refs_in_string(
            "דתנן (דברים שם) דכל (דברים ל, א) צריך", citing_only=citing_only
        )
        assert 1 == len(ref)
    ...
    def test_citing_only_he(self, citing_only):
        matched_refs = library.get_refs_in_string(
            texts["weird_ref_he"], lang="he", citing_only=citing_only
        )
        if citing_only:
            assert matched_refs == [Ref("Leviticus 12.4")]
        else:
            assert set(matched_refs) == {
                Ref("The Book of Susanna 1.2"),
                Ref("Leviticus 12.4"),
            }
    ...
    def test_supposed_ref_graceful_fail(self, citing_only):
        matched_refs = library.get_refs_in_string(
            "אלו דברים בני ישראל", lang="he", citing_only=citing_only
        )
        assert matched_refs == []
    ...
    def test_ranged_ref_plain(self, citing_only):
        ...
        test_strings = [
            "בלה בלה שנאמר ({}) וכולי".format(temp_tref) for temp_tref in trefs
        ]
        for i, test_string in enumerate(test_strings):
            matched_refs = library.get_refs_in_string(
                test_string, lang="he", citing_only=citing_only
            )
            assert matched_refs == [Ref(trefs[i])]
    ...
    def test_wrap_refs(self, citing_only):
        ...
        res = reduce(
            lambda a, b: a
            + '(<a class ="refLink" href="/{}" data-ref="{}">{}</a>) בלה בלה '.format(
                b[0].url(), b[0].normal(), b[1]
            ),
            list(zip(orefs, trefs)),
            "",
        ...
        wrapped = library.get_wrapped_refs_string(
            st, lang="he", citing_only=citing_only
        )
        assert wrapped == res
...
class Test_get_titles_in_text(object):
    ...
    def test_positions(self, citing_only):
        ...
        for a in [bible_mid, bible_begin, bible_end]:
            assert {"Genesis"} <= set(
                library.get_titles_in_string(a, citing_only=citing_only)
            )
    ...
    def test_he_positions(self, citing_only):
        ...
        for a in [bible_mid, bible_begin, bible_end]:
            assert {"שמות"} <= set(
                library.get_titles_in_string(a, "he", citing_only=citing_only)
            )

    @pytest.mark.parametrize(("citing_only"), (True, False))
    def test_citing_only_en(self, citing_only):
        titles = library.get_titles_in_string(
            texts["weird_ref"], lang="en", citing_only=citing_only
        )
        if citing_only:
...
class Test_Library(object):
    ...
    def test_cache_populated_on_instanciation(self):
        ...
        for lang in library.langs:
            ...
            with pytest.raises(Exception):
                assert library._index_title_commentary_maps[
                    lang
    ...
    def test_all_index_caches_removed_and_added_commentary(self):
        ...
        assert (
            "Rashi on Bereishit" in library._index_title_maps["en"]["Rashi on Genesis"]
        )
