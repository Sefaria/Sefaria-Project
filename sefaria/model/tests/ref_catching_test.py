...
class In(object):
    ...
    def finds(self, result):
        ...
        if m.Ref(match.group(1)).normal() == result:
            ...
            print(
                "Mismatched.  Found: {}, which normalizes to: {}, not {}".format(
                    match.group(1), m.Ref(match.group(1)).normal(), result
                )
            )
            return False
    ...
    def finds_multiple(self, result):
        ...
        for title_match in m.library.all_titles_regex(lang, citing_only=False).finditer(
            self._haystack
        ):
            ...
            if m.Ref(match.group(1)).normal() in result:
                ...
                print(
                    "Mismatched.  Found: {}, which normalizes to: {}, which is not in {}".format(
                        match.group(1), m.Ref(match.group(1)).normal(), result
                    )
                )
                return False
    ...
    def _do_search(self, needle, haystack):
        ...
        reg_str = m.library.get_regex_string(
            ...
            anchored=False,
            capture_title=False,
            parentheses=self._with_parenthesis,
        )
...
class Test_find_citation_in_text(object):
    ...
    def test_regex_string_en_array(self):
        assert (
            In("Genesis 2:1-Genesis 2:3")
            .looking_for("Genesis")
            .finds_multiple(["Genesis 2:1", "Genesis 2:3"])
        )
        assert (
            In("Genesis 2:1/Bereshit 2:3")
            .looking_for("Genesis")
            .finds_multiple(["Genesis 2:1", "Genesis 2:3"])
        )
    ...
    def test_regex_string_he_in_parentheses(self):
        assert (
            In("(בדברים לב ובספרות ג ב)")
            .looking_for("דברים")
            .with_parenthesis()
            .finds("Deuteronomy 32")
        )
        assert (
            In("(בדברים לב ובספרות ג ב)")
            .looking_for("רות")
    ...
    def test_regex_string_he_in_parentheses_3(self):
        assert (
            In("<p>[שיר השירים א ירושלמי כתובות (פרק א הלכה ב) בשורות א]")
            .looking_for("ירושלמי כתובות")
            .with_parenthesis()
            .finds("Jerusalem Talmud Ketubot 1:2")
        )

        assert (
            In("<p>[שיר השירים א ירושלמי כתובות (דף כח:) בשורות א]")
            .looking_for("שיר השירים")
            .with_parenthesis()
            .finds("Song of Songs 1")
        )

    @pytest.mark.xfail(
        reason="Linker doesn't know that it should look for either Mishnah or Talmud. This is as opposed to Ref instantiation where this ref would parse correctly because it would find the check_first field on the index."
    )
    def test_check_first(self):
        assert (
            In('בבא מציעא פ"ד מ"ו, ועיין לעיל')
            .looking_for("בבא מציעא")
            .finds("Mishnah Bava Metzia 4:6")
        )
