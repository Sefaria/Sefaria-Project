...
@pytest.mark.deep
def test_inline_reference_dependence():

    # make sure test indices have been properly cleaned up
    titles = ["Dummy Text", "Dummy Commentary", "Different Title"]
    for title in titles:
        i = model.Index().load({'title': title})
        if i is not None:
            i.delete()

    # create dummy index and commentary
    def make_schema(en_title, he_title):
        j = model.JaggedArrayNode()
        j.add_primary_titles(en_title, he_title)
        j.add_structure(['Chapter', 'Verse'])
        return j.serialize()

    base_index = model.Index({
        'title': 'Dummy Text',
        'categories': ['Tanakh'],
        'schema': make_schema('Dummy Text', u'טקסט לדוגמא')
    })
    commentary_index = model.Index({
        'title': 'Dummy Commentary',
        'categories': ['Tanakh', 'Commentary'],
        'schema': make_schema('Dummy Commentary', u'פרשן לדוגמא')
    })
    base_index.save()
    commentary_index.save()

    base_version = model.Version({
        'language': 'en',
        'title': 'Dummy Text',
        'versionSource': 'http://foobar.com',
        'versionTitle': 'test_version',
        'chapter': [['Lorem <i data-commentator="Dummy Commentary" data-order="1"></i>ipsum'
                     'foo <i data-commentator="Other Commentary" data-order="1"></i>bar', 'more text'], ['other section']]
    })
    commentary_version = model.Version({
        'language': 'en',
        'title': 'Dummy Commentary',
        'versionSource': 'http://foobar.com',
        'versionTitle': 'commentary_test_version',
        'chapter': [['some words']]
    })
    base_version.save()
    commentary_version.save()

    model.Link({
        'refs': ['Dummy Text 1:1', 'Dummy Commentary 1:1'],
        'type': 'None',
        'inline_reference': {
            'data-commentator': 'Dummy Commentary',
            'data-order': 1
        }
    }).save()

    commentary_index.title = 'Different Title'
    commentary_index.save()
    l = model.Link().load({'refs': ['Dummy Text 1:1', 'Different Title 1:1']})
    assert l.inline_reference['data-commentator'] == 'Different Title'
    b_text = model.TextChunk(model.Ref("Dummy Text 1:1"), 'en', 'test_version').text
    assert b_text == 'Lorem <i data-commentator="Different Title" data-order="1"></i>ipsum' \
                     'foo <i data-commentator="Other Commentary" data-order="1"></i>bar'

    # cleanup
    model.library.get_index("Dummy Text").delete()
    model.library.get_index("Different Title").delete()
