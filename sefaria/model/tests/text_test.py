    ...
    d = {
        ...
        "schema": {
            "titles": [
                ...
                {"lang": "he", "text": "פרשן", "primary": True},
            ],
...
def test_invalid_index_save_no_category():
    ...
    with pytest.raises(InputError) as e_info:
        idx.save()
    assert (
        "You must create category Mishnah/Commentary/Bartenura/Gargamel before adding texts to it."
        in str(e_info.value)
    )
    assert model.IndexSet({"title": title}).count() == 0
...
def test_invalid_index_save_no_hebrew_collective_title():
    ...
    with pytest.raises(InputError) as e_info:
        idx.save()
    assert (
        "You must add a hebrew translation Term for any new Collective Title: Gargamel."
        in str(e_info.value)
    )
    assert model.IndexSet({"title": title}).count() == 0
...
def test_merge():
    assert model.merge_texts([["a", ""], ["", "b", "c"]], ["first", "second"]) == [
        ["a", "b", "c"],
        ["first", "second", "second"],
    ]
...
def test_text_helpers():
    ...
    res = model.library.get_dependant_indices(
        book_title="Exodus", dependence_type="Commentary"
    )
    assert "Ibn Ezra on Exodus" in res
    assert "Ramban on Exodus" in res
    assert "Meshekh Chokhmah" in res
    ...
    res = model.library.get_dependant_indices(
        book_title="Exodus", dependence_type="Commentary", structure_match=True
    )
    assert "Ibn Ezra on Exodus" in res
    assert "Ramban on Exodus" in res
...
def dep_counts(name, indx):
    ...
    patterns = [
        pattern.replace(re.escape(indx.title), re.escape(name)) for pattern in patterns
    ]

    ret = {
        "version title exact match": model.VersionSet(
            {"title": name}, sort=[("title", 1)]
        ).count(),
        "history title exact match": model.HistorySet(
            {"title": name}, sort=[("title", 1)]
        ).count(),
        "note match ": model.NoteSet(
            construct_query("ref", patterns), sort=[("ref", 1)]
        ).count(),
        "link match ": model.LinkSet(construct_query("refs", patterns)).count(),
        "history refs match ": model.HistorySet(
            construct_query("ref", patterns), sort=[("ref", 1)]
        ).count(),
        "history new refs match ": model.HistorySet(
            construct_query("new.refs", patterns), sort=[("new.refs", 1)]
        ).count(),
    }
...
def test_version_word_count():
    ...
    assert (
        model.Version()
        .load({"title": "Rashi on Shabbat", "language": "he"})
        .word_count()
        > 0
    )
    # complex
    assert (
        model.Version()
        .load({"title": "Pesach Haggadah", "language": "he"})
        .word_count()
        > 0
    )
    assert model.Version().load({"title": "Orot", "language": "he"}).word_count() > 0
    assert (
        model.Version().load({"title": "Ephod Bad on Pesach Haggadah"}).word_count() > 0
    )
...
class TestModifyVersion:
    ...
    def setup_class(cls):
                    ...
                    "nodes": [
                        {
                            ...
                            "titles": [
                                {"text": "Node 1", "lang": "en", "primary": True},
                                {"text": "Node 1 he", "lang": "he", "primary": True},
                            ],
                            "key": "Node 1",
                        },
                        {
                            ...
                            "titles": [
                                {"text": "Node 3", "lang": "en", "primary": True},
                                {"text": "Node 3 he", "lang": "he", "primary": True},
                            ],
                            "key": "Node 3",
                        },
                    ],
                    "titles": [
                        {"text": cls.complexIndexTitle, "lang": "en", "primary": True},
                        {
        ...
        cls.complexVersion.chapter = {
            "Node 1": {"Node 2": [["yo"], ["", "blah"], ["original text", "2nd"]]},
            "Node 3": ["1", "2", "3", "4"],
        }
    ...
    def test_sub_content_with_ref(self):
        self.simpleVersion.sub_content_with_ref(
            model.Ref(f"{self.simpleIndexTitle} 3:2"), "new text"
        )
        assert self.simpleVersion.chapter[2][1] == "new text"

        self.complexVersion.sub_content_with_ref(
            model.Ref(f"{self.complexIndexTitle}, Node 1, Node 2 3:2"), "new text"
        )
        assert self.complexVersion.chapter["Node 1"]["Node 2"][2][1] == "new text"

        self.complexVersion.sub_content_with_ref(
            model.Ref(f"{self.complexIndexTitle}, Node 1, Node 2 3"), ["blah", "blarg"]
        )
        assert self.complexVersion.chapter["Node 1"]["Node 2"][2] == ["blah", "blarg"]
        self.complexVersion.sub_content_with_ref(
            model.Ref(f"{self.complexIndexTitle}, Node 1, Node 2 3"),
            ["original text", "2nd"],
        )  # set back to original content for other tests

        self.complexVersion.sub_content_with_ref(
            model.Ref(f"{self.complexIndexTitle}, Node 1, Node 2"),
            [["blah", "blarg"], ["more content"]],
        )
        assert self.complexVersion.chapter["Node 1"]["Node 2"] == [
            ["blah", "blarg"],
            ["more content"],
        ]
        self.complexVersion.sub_content_with_ref(
            model.Ref(f"{self.complexIndexTitle}, Node 1, Node 2"),
            [["yo"], ["", "blah"], ["original text", "2nd"]],
        )  # set back to original content for other tests

    def test_sub_content_with_ref_padding(self):
        self.simpleVersion.sub_content_with_ref(
            model.Ref(f"{self.simpleIndexTitle} 3:5"), "new text"
        )
        ...
        self.simpleVersion.sub_content_with_ref(
            model.Ref(f"{self.simpleIndexTitle} 5:1"), "new text2"
        )
        ...
        # reset
        self.simpleVersion.sub_content_with_ref(
            model.Ref(f"{self.simpleIndexTitle}"),
            [["1"], ["2"], ["original text", "2nd"]],
        )

    def test_sub_content_simple_setter(self):
        self.simpleVersion.sub_content(
            self.simpleIndex.nodes.version_address(), value=[[], [], []]
        )
        for i in range(3):
            assert self.simpleVersion.chapter[i] == []

        self.simpleVersion.sub_content(
            self.simpleIndex.nodes.version_address(), [1], value=["yo1", "yo2", "yo3"]
        )
        assert self.simpleVersion.chapter[1] == ["yo1", "yo2", "yo3"]

        self.simpleVersion.sub_content(
            self.simpleIndex.nodes.version_address(), [0, 1], value="yo"
        )
    ...
    def test_sub_content_complex_setter(self):
        self.complexVersion.sub_content(
            [], value={"Node 1": {"Node 2": [["wadup"]], "Node 3": []}}
        )
        ...
        self.complexVersion.sub_content_with_ref(
            model.Ref(f"{self.complexIndexTitle}"),
            {
                "Node 1": {"Node 2": [["yo"], ["", "blah"], ["original text", "2nd"]]},
                "Node 3": ["1", "2", "3", "4"],
            },
...
class TestVersionActualLanguage:
    ...
    def teardown_class(cls):
        for c in [
            cls.myIndex,
            cls.versionWithTranslation,
            cls.versionWithoutTranslation,
            cls.versionThatWillBreak,
        ]:
            try:
                c.delete()
            except Exception:
    ...
    def test_fails_validation_when_language_mismatch(self):
        with pytest.raises(
            InputError, match="Version actualLanguage does not match bracketed language"
        ):
