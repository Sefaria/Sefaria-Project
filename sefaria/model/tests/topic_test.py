...
def make_it_link(a, b, type):
    l = IntraTopicLink(
        {"fromTopic": a, "toTopic": b, "linkType": type, "dataSource": "sefaria"}
    )
    l.save()
    return l
...
def make_rt_link(a, tref):
    l = RefTopicLink(
        {"toTopic": a, "ref": tref, "linkType": "about", "dataSource": "sefaria"}
    )
    l.save()
    return l
...
def topic_graph():
    ...
    for a, b in isa_links:
        ...
        clean_links(str(b))
    graph = {
        "topics": {str(i): make_topic(str(i)) for i in range(1, 10)},
        "links": [make_it_link(str(a), str(b), "is-a") for a, b in isa_links]
        + [make_rt_link("1", r) for r in trefs],
    }
    yield graph
...
def topic_graph_to_merge():
    ...
    graph = {
        "topics": {str(i): make_topic(str(i)) for i in range(10, 100, 10)},
        "links": [make_it_link(str(a), str(b), "is-a") for a, b in isa_links]
        + [make_rt_link("10", r) for r in trefs]
        + [make_rt_link("20", r) for r in trefs1]
        + [make_rt_link("40", r) for r in trefs2],
    }
...
class TestTopics(object):
    def test_graph_funcs(self, topic_graph):
        ...
        assert {
            t.slug
            for t in ts["5"].topics_by_link_type_recursively(
                linkType="is-a", only_leaves=True
            )
        } == {"1", "6"}
        assert ts["1"].topics_by_link_type_recursively(
            linkType="is-a", only_leaves=True
        ) == [ts["1"]]
    ...
    def test_link_set(self, topic_graph):
        ...
        assert {getattr(l, "ref", getattr(l, "topic", None)) for l in ls} == (
            trefs | {"2"}
        )
...
class TestTopicLinkHelper(object):
    def test_init_by_class(self, topic_graph):
        l1 = db.topic_links.find_one(
            {"fromTopic": "1", "toTopic": "2", "linkType": "is-a"}
        )
