...
@pytest.mark.parametrize(('resolver_data', 'expected_trefs'), [
    ...
    [crrd(None, 'he', 'בגמרא בסנהדרין צז:-צח.', [slice(0, 2), slice(2, 4), 4, slice(5, 7)], [RPT.NAMED, RPT.NUMBERED, RPT.RANGE_SYMBOL, RPT.NUMBERED]), ("Sanhedrin 97b-98a",)],  # one big ref part that actually matches two separate terms + each part has prefix
])
