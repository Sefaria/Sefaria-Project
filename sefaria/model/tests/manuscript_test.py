...
def teardown_module():
    ...
    for title in possible_titles:
        ...
        if m:
            m.delete()
        ManuscriptPageSet(
            {"manuscript_slug": Manuscript.normalize_slug(title)}
        ).delete()
...
class TestPageRefs:
    ...
    def test_load_by_segment(self):
        ...
        test_specific = [
            m
            for m in mps
            if m.manuscript_slug == Manuscript.normalize_slug("Delete Me")
        ]
    ...
    def test_load_for_client(self):
        ...
        data = [d for d in data if d["manuscript_slug"] == slug][
            0
        ]  # this is here to limit us to just the testing data
...
class TestDependencies:
    def test_rename_manuscript(self):
        ...
        mps = ManuscriptPageSet(
            {"manuscript_slug": Manuscript.normalize_slug(original_title)}
        )
        assert mps.count() == 0
        mps = ManuscriptPageSet(
            {"manuscript_slug": Manuscript.normalize_slug(new_title)}
        )
        assert mps.count() == 5
