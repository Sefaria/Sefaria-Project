...
def create_good_web_page():
    data_good_url = {
        "url": "http://blogs.timesofisrael.com/dvar-torah2",
...
def create_web_page_wout_desc():
    data_good_url = {
        "url": "http://blogs.timesofisrael.com/dvar-torah4",
...
def test_add_bad_domain_from_linker():
    ...
    data = {
        "url": "http://localhost:8000/static/test/linker_test.html",
...
def test_add_no_refs_from_linker():
    ...
    data = {
        "url": "http://blogs.timesofisrael.com/random",
        "title": "Random Blog",
        "description": "A Page We Do Not Want",
        "refs": [],
    }
...
def test_add_bad_title_from_linker():
    ...
    data = {
        "url": "http://rabbisacks.org/archives",
        "title": "Page 1 of 2",
        "description": "A Page We Do Not Want",
        "refs": ["Genesis 1:1"],
    }
...
def test_add_webpage_without_website(create_web_page_wout_site):
    ...
    result, webpage, data = (
        create_web_page_wout_site["result"],
        create_web_page_wout_site["webpage"],
        create_web_page_wout_site["data"],
    )
    assert result == "saved"
...
def test_add_and_update_good_url_from_linker(create_good_web_page):
    ...
    result, webpage, data = (
        create_good_web_page["result"],
        create_good_web_page["webpage"],
        create_good_web_page["data"],
    )
    assert result == "saved"
    ...
    assert set(WebPage().load({"url": data["url"]}).refs) == set(
        ["Genesis 3:3", "Exodus 3:10"]
    )
...
def test_add_search_URL():
    urls = [
        "https://opensiddur.org/search/",
        "https://opensiddur.org/search?q=prayer",
        "https://opensiddur.org/search",
    ]
    for url in urls:
        linker_data = {
...
def test_page_wout_description(create_web_page_wout_desc):
    result, webpage, data = (
        create_web_page_wout_desc["result"],
        create_web_page_wout_desc["webpage"],
        create_web_page_wout_desc["data"],
    )
    assert result == "saved"
