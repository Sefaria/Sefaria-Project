...
class Test_Ref(object):
    ...
    def test_each_title(object):
        for lang in ["en", "he"]:
            for t in library.full_title_list(lang, False):
                assert library.all_titles_regex(lang).match(
                    t
                ), "'{}' doesn't resolve".format(t)

    def test_comma(self):
        assert Ref("Me'or Einayim, Chayei Sara 24") == Ref(
            "Me'or Einayim, Chayei Sara, 24"
        )
    ...
    def test_padded_ref(self):
        ...
        assert (
            Ref("Rashi on Genesis 2:3:1").padded_ref().normal()
            == "Rashi on Genesis 2:3:1"
        )
        ...
        assert (
            Ref("Rashi on Shabbat 2a:1:1").padded_ref().normal()
            == "Rashi on Shabbat 2a:1:1"
        )
    ...
    def test_starting_and_ending(self):
        ...
        assert Ref("Rashi on Leviticus 15:3-17:12").starting_ref() == Ref(
            "Rashi on Leviticus 15:3"
        )
        assert Ref("Rashi on Leviticus 15:3-17:12").ending_ref() == Ref(
            "Rashi on Leviticus 17:12"
        )

        assert Ref("Rashi on Leviticus 15-17").starting_ref() == Ref(
            "Rashi on Leviticus 15"
        )
        assert Ref("Rashi on Leviticus 15-17").ending_ref() == Ref(
            "Rashi on Leviticus 17"
        )

        assert Ref("Rashi on Leviticus 15:17-21").starting_ref() == Ref(
            "Rashi on Leviticus 15:17"
        )
        assert Ref("Rashi on Leviticus 15:17-21").ending_ref() == Ref(
        ...
        assert Ref("Rashi on Shabbat 15a-16b").starting_ref() == Ref(
            "Rashi on Shabbat 15a"
        )
        assert Ref("Rashi on Shabbat 15a-16b").ending_ref() == Ref(
            "Rashi on Shabbat 16b"
        )
        ...
        assert Ref("Rashi on Shabbat 15a:15-15b:13").starting_ref() == Ref(
            "Rashi on Shabbat 15a:15"
        )
        assert Ref("Rashi on Shabbat 15a:15-15b:13").ending_ref() == Ref(
            "Rashi on Shabbat 15b:13"
        )

        assert Ref("Rashi on Exodus 3:1-4:1").starting_ref() == Ref(
            "Rashi on Exodus 3:1"
        )
        assert Ref("Rashi on Exodus 3:1-4:1").ending_ref() == Ref("Rashi on Exodus 4:1")

        assert Ref("Rashi on Exodus 3:1-4:10").starting_ref() == Ref(
            "Rashi on Exodus 3:1"
        )
        assert Ref("Rashi on Exodus 3:1-4:10").ending_ref() == Ref(
            "Rashi on Exodus 4:10"
        )

        assert Ref("Rashi on Exodus 3:1-3:10").starting_ref() == Ref(
            "Rashi on Exodus 3:1"
        )
        assert Ref("Rashi on Exodus 3:1-3:10").ending_ref() == Ref(
            "Rashi on Exodus 3:10"
        )

        assert Ref("Rashi on Exodus 3:1:1-3:1:3").starting_ref() == Ref(
            "Rashi on Exodus 3:1:1"
        )
        assert Ref("Rashi on Exodus 3:1:1-3:1:3").ending_ref() == Ref(
            "Rashi on Exodus 3:1:3"
        )
    ...
    def test_all_context_refs(self):
        assert Ref("Rashi on Genesis 2:3:4").all_context_refs() == [
            Ref("Rashi on Genesis 2:3:4"),
            Ref("Rashi on Genesis 2:3"),
            Ref("Rashi on Genesis 2"),
        ]
        assert Ref("Rashi on Genesis 2:3:4").all_context_refs(
            include_self=False, include_book=True
        ) == [
            Ref("Rashi on Genesis 2:3"),
            Ref("Rashi on Genesis 2"),
            Ref("Rashi on Genesis"),
        ]
        assert Ref("Rashi on Genesis 2:3:4").all_context_refs(
            include_self=False, include_book=False
        ) == [Ref("Rashi on Genesis 2:3"), Ref("Rashi on Genesis 2")]
        assert Ref("Rashi on Genesis 2:3:4").all_context_refs(
            include_self=True, include_book=True
        ) == [
            Ref("Rashi on Genesis 2:3:4"),
            Ref("Rashi on Genesis 2:3"),
            Ref("Rashi on Genesis 2"),
        ...
        assert Ref(
            "Pesach Haggadah, Magid, First Fruits Declaration 2"
        ).all_context_refs() == [
            Ref("Pesach Haggadah, Magid, First Fruits Declaration 2"),
            Ref("Pesach Haggadah, Magid, First Fruits Declaration"),
            Ref("Pesach Haggadah, Magid"),
        ]
        assert Ref(
            "Pesach Haggadah, Magid, First Fruits Declaration 2"
        ).all_context_refs(include_self=True, include_book=True) == [
            Ref("Pesach Haggadah, Magid, First Fruits Declaration 2"),
            Ref("Pesach Haggadah, Magid, First Fruits Declaration"),
            Ref("Pesach Haggadah, Magid"),
            Ref("Pesach Haggadah"),
        ...
        assert Ref(
            "Pesach Haggadah, Magid, First Fruits Declaration 2"
        ).all_context_refs(include_self=False, include_book=True) == [
            Ref("Pesach Haggadah, Magid, First Fruits Declaration"),
            Ref("Pesach Haggadah, Magid"),
            Ref("Pesach Haggadah"),
        ]
        assert Ref(
            "Pesach Haggadah, Magid, First Fruits Declaration 2"
        ).all_context_refs(include_self=False, include_book=False) == [
            Ref("Pesach Haggadah, Magid, First Fruits Declaration"),
            Ref("Pesach Haggadah, Magid"),
        ]

        # Don't choke on Schema nodes.
        assert Ref("Pesach Haggadah, Magid").all_context_refs() == [
            Ref("Pesach Haggadah, Magid")
        ]

        # Don't choke on Virtual nodes
        assert Ref("Jastrow, ג").all_context_refs() == [
            Ref("Jastrow, ג"),
            Ref("Jastrow<d>"),
        ]
    ...
    def test_context_ref(self):
        ...
        assert (
            Ref("Rashi on Genesis 2:3:1").context_ref().normal()
            == "Rashi on Genesis 2:3"
        )
        assert (
            Ref("Rashi on Genesis 2:3:1").context_ref(2).normal()
            == "Rashi on Genesis 2"
        )

    def test_section_ref(self):
        assert (
            Ref("Rashi on Genesis 2:3:1").section_ref().normal()
            == "Rashi on Genesis 2:3"
        )
    ...
    def test_top_section_ref(self):
        ...
        assert (
            Ref("Rashi on Genesis 1:2:3").top_section_ref().normal()
            == "Rashi on Genesis 1"
        )
    ...
    def test_next_ref(self):
        ...
        assert (
            Ref("Rashi on Genesis 5:32:2").next_section_ref().normal()
            == "Rashi on Genesis 6:2"
        )
        assert Ref("Mekhilta 35.3").next_section_ref() is None
        # This will start to fail when we fill in this text
        assert (
            Ref("Mekhilta d'Rabbi Yishmael 23:19").next_section_ref().normal()
            == "Mekhilta d'Rabbi Yishmael 31:12"
        )

    def test_complex_next_ref(
        self,
    ):  # at time of test we only had complex commentaries stable to test with
        assert (
            Ref("Pesach Haggadah, Kadesh").next_section_ref().normal()
            == "Pesach Haggadah, Urchatz"
        )
        assert (
            Ref("Orot, Lights from Darkness, Lights of Rebirth 72")
            .next_section_ref()
            .normal()
            == "Orot, Lights from Darkness, Great Calling"
        )
        assert (
            Ref("Orot, Lights from Darkness, Great Calling").next_section_ref().normal()
            == "Orot, The Process of Ideals in Israel, The Godly and the National Ideal in the Individual"
        )
        assert (
            Ref("Ephod Bad on Pesach Haggadah, Magid, The Four Sons 1")
            .next_section_ref()
            .normal()
            == "Ephod Bad on Pesach Haggadah, Magid, The Four Sons 2"
        )
        assert (
            Ref(
                "Ephod Bad on Pesach Haggadah, Magid, In the Beginning Our Fathers Were Idol Worshipers 5"
            )
            .next_section_ref()
            .normal()
            == "Ephod Bad on Pesach Haggadah, Magid, First Fruits Declaration 2"
        )
        assert (
            Ref("Naftali Seva Ratzon on Pesach Haggadah, Kadesh 2")
            .next_section_ref()
            .normal()
            == "Naftali Seva Ratzon on Pesach Haggadah, Karpas 1"
        )
        assert (
            Ref("Naftali Seva Ratzon on Pesach Haggadah, Magid, Ha Lachma Anya 2")
            .next_section_ref()
            .normal()
            == "Naftali Seva Ratzon on Pesach Haggadah, Magid, We Were Slaves in Egypt 2"
        )
        assert (
            Ref("Ephod Bad on Pesach Haggadah, Magid, First Half of Hallel 4")
            .next_section_ref()
            .normal()
            == "Ephod Bad on Pesach Haggadah, Barech, Pour Out Thy Wrath 2"
        )
        assert Ref(
            "Kos Shel Eliyahu on Pesach Haggadah, Magid, Second Cup of Wine 2"
        ).next_section_ref() is Ref(
            "Kos Eliyahu on Pesach Haggadah, Barech, Pour Out Thy Wrath 2"
        )
    ...
    def test_prev_ref(self):
        ...
        assert (
            Ref("Rashi on Genesis 6:2:1").prev_section_ref().normal()
            == "Rashi on Genesis 5:32"
        )
        assert Ref("Mekhilta 12:1").prev_section_ref() is None
        # This will start to fail when we fill in this text
        assert (
            Ref("Mekhilta d'Rabbi Yishmael 31:12").prev_section_ref().normal()
            == "Mekhilta d'Rabbi Yishmael 23:19"
        )

    def test_complex_prev_ref(self):
        assert (
            Ref("Pesach Haggadah, Urchatz").prev_section_ref().normal()
            == "Pesach Haggadah, Kadesh"
        )
        assert (
            Ref("Orot, Lights from Darkness, Great Calling").prev_section_ref().normal()
            == "Orot, Lights from Darkness, Lights of Rebirth 72"
        )
        assert (
            Ref(
                "Orot, The Process of Ideals in Israel, The Godly and the National Ideal in the Individual"
        ...
        assert (
            Ref("Ephod Bad on Pesach Haggadah, Magid, The Four Sons 2")
            .prev_section_ref()
            .normal()
            == "Ephod Bad on Pesach Haggadah, Magid, The Four Sons 1"
        )
        assert (
            Ref("Ephod Bad on Pesach Haggadah, Magid, First Fruits Declaration 2")
            .prev_section_ref()
            .normal()
            == "Ephod Bad on Pesach Haggadah, Magid, In the Beginning Our Fathers Were Idol Worshipers 5"
        )
        assert (
            Ref("Naftali Seva Ratzon on Pesach Haggadah, Karpas 1")
            .prev_section_ref()
            .normal()
            == "Naftali Seva Ratzon on Pesach Haggadah, Kadesh 2"
        )
        assert (
            Ref(
                "Naftali Seva Ratzon on Pesach Haggadah, Magid, We Were Slaves in Egypt 2"
            ...
            .prev_section_ref()
            .normal()
            == "Naftali Seva Ratzon on Pesach Haggadah, Magid, Ha Lachma Anya 2"
        )
        assert (
            Ref("Ephod Bad on Pesach Haggadah, Hallel, Second Half of Hallel 2")
            .prev_section_ref()
            .normal()
            == "Ephod Bad on Pesach Haggadah, Barech, Pour Out Thy Wrath 2"
        )
        assert (
            Ref(
                "Kos Shel Eliyahu on Pesach Haggadah, Magid, Ha Lachma Anya 3"
            ).prev_section_ref()
            is None
        )
    ...
    def test_next_segment_ref(self):
        ...
        assert Ref("Rashi on Exodus 3:1:1").next_segment_ref() == Ref(
            "Rashi on Exodus 3:1:2"
        )
        assert Ref("Rashi on Exodus 2:25:1").next_segment_ref() == Ref(
            "Rashi on Exodus 3:1:1"
        )
        assert Ref("Rashi on Exodus 3:19:2").next_segment_ref() == Ref(
            "Rashi on Exodus 3:22:1"
        )
        assert Ref("Shabbat 5b:9").next_segment_ref() == Ref("Shabbat 5b:10")
        assert Ref("Shabbat 5b:11").next_segment_ref() == Ref("Shabbat 6a:1")
        assert Ref("Rashi on Shabbat 5b:5:4").next_segment_ref() == Ref(
            "Rashi on Shabbat 5b:5:5"
        )
        assert Ref("Rashi on Shabbat 6a:1:1").next_segment_ref() == Ref(
            "Rashi on Shabbat 6a:3:1"
        )
        assert Ref("Rashi on Shabbat 5b:10:1").next_segment_ref() == Ref(
            "Rashi on Shabbat 6a:1:1"
        )
    ...
    def test_prev_segment_ref(self):
        ...
        assert Ref("Rashi on Exodus 3:1:2").prev_segment_ref() == Ref(
            "Rashi on Exodus 3:1:1"
        )
        assert Ref("Rashi on Exodus 3:1:1").prev_segment_ref() == Ref(
            "Rashi on Exodus 2:25:1"
        )
        assert Ref("Rashi on Exodus 3:22:1").prev_segment_ref() == Ref(
            "Rashi on Exodus 3:19:2"
        )
        assert Ref("Shabbat 5b:10").prev_segment_ref() == Ref("Shabbat 5b:9")
        assert Ref("Shabbat 6a:1").prev_segment_ref() == Ref("Shabbat 5b:11")
        assert Ref("Rashi on Shabbat 5b:5:5").prev_segment_ref() == Ref(
            "Rashi on Shabbat 5b:5:4"
        )
        assert Ref("Rashi on Shabbat 6a:3:1").prev_segment_ref() == Ref(
            "Rashi on Shabbat 6a:1:1"
        )
        assert Ref("Rashi on Shabbat 6a:1:1").prev_segment_ref() == Ref(
            "Rashi on Shabbat 5b:10:1"
        )

    def test_last_segment_ref(self):
        assert Ref("Exodus").last_segment_ref() == Ref("Exodus 40:38")
        assert Ref("Rashi on Exodus").last_segment_ref() == Ref(
            "Rashi on Exodus 40:38:1"
        )
        assert Ref("Shabbat").last_segment_ref() == Ref("Shabbat 157b:3")
        assert Ref("Rashi on Shabbat").last_segment_ref() == Ref(
            "Rashi on Shabbat 157b:2:2"
        )
    ...
    def test_split_spanning_ref(self):
        assert Ref("Leviticus 15:3 - 17:12").split_spanning_ref() == [
            Ref("Leviticus 15:3-33"),
            Ref("Leviticus 16"),
            Ref("Leviticus 17:1-12"),
        ]
        assert Ref("Leviticus 15-17").split_spanning_ref() == [
            Ref("Leviticus 15"),
            Ref("Leviticus 16"),
            Ref("Leviticus 17"),
        ]
        assert Ref("Leviticus 15:17-21").split_spanning_ref() == [
            Ref("Leviticus 15:17-21")
        ]
        assert Ref("Leviticus 15:17").split_spanning_ref() == [Ref("Leviticus 15:17")]
        assert Ref("Shabbat 15a-16b").split_spanning_ref() == [
            Ref("Shabbat 15a"),
            Ref("Shabbat 15b"),
            Ref("Shabbat 16a"),
            Ref("Shabbat 16b"),
        ...
        assert Ref("Shabbat 15a:8-15b:8").split_spanning_ref() == [
            Ref("Shabbat 15a:8-10"),
            Ref("Shabbat 15b:1-8"),
        ]
        assert Ref("Rashi on Exodus 5:3-6:7").split_spanning_ref() == [
            Ref("Rashi on Exodus 5:3"),
            Ref("Rashi on Exodus 5:4"),
            Ref("Rashi on Exodus 5:5"),
            Ref("Rashi on Exodus 5:6"),
            Ref("Rashi on Exodus 5:7"),
            Ref("Rashi on Exodus 5:8"),
            Ref("Rashi on Exodus 5:9"),
            Ref("Rashi on Exodus 5:10"),
            Ref("Rashi on Exodus 5:11"),
            Ref("Rashi on Exodus 5:12"),
            Ref("Rashi on Exodus 5:13"),
            Ref("Rashi on Exodus 5:14"),
            Ref("Rashi on Exodus 5:15"),
            Ref("Rashi on Exodus 5:16"),
            Ref("Rashi on Exodus 5:17"),
            Ref("Rashi on Exodus 5:18"),
            Ref("Rashi on Exodus 5:19"),
            Ref("Rashi on Exodus 5:20"),
            Ref("Rashi on Exodus 5:21"),
            Ref("Rashi on Exodus 5:22"),
            Ref("Rashi on Exodus 5:23"),
            Ref("Rashi on Exodus 6:1"),
            Ref("Rashi on Exodus 6:2"),
            Ref("Rashi on Exodus 6:3"),
            Ref("Rashi on Exodus 6:4"),
            Ref("Rashi on Exodus 6:5"),
            Ref("Rashi on Exodus 6:6"),
            Ref("Rashi on Exodus 6:7"),
        ]
        assert Ref("Targum Neofiti 5-7").split_spanning_ref() == [
            Ref("Targum Neofiti 5"),
            Ref("Targum Neofiti 6"),
            Ref("Targum Neofiti 7"),
        ]
    ...
    def test_range_list(self):
        assert Ref("Leviticus 15:12-17").range_list() == [
            Ref("Leviticus 15:12"),
            Ref("Leviticus 15:13"),
            Ref("Leviticus 15:14"),
            Ref("Leviticus 15:15"),
        ...
        assert Ref("Shabbat 15b:5-8").range_list() == [
            Ref("Shabbat 15b:5"),
            Ref("Shabbat 15b:6"),
            Ref("Shabbat 15b:7"),
            Ref("Shabbat 15b:8"),
        ...
        assert Ref("Shabbat 15a:9-15b:2").range_list() == [
            Ref("Shabbat 15a:9"),
            Ref("Shabbat 15a:10"),
            Ref("Shabbat 15b:1"),
            Ref("Shabbat 15b:2"),
    ...
    def test_range_list_first_and_last_segment(self):
        assert Ref("Shabbat 15a:9-15b:1").range_list() == [
            Ref("Shabbat 15a:9"),
            Ref("Shabbat 15a:10"),
            Ref("Shabbat 15b:1"),
        ]
        assert Ref("Shabbat 15a:10-15b:1").range_list() == [
            Ref("Shabbat 15a:10"),
            Ref("Shabbat 15b:1"),
        ]
        assert Ref("Shabbat 15a:10-15b:2").range_list() == [
            Ref("Shabbat 15a:10"),
            Ref("Shabbat 15b:1"),
            Ref("Shabbat 15b:2"),
        ]
        assert Ref("Exodus 15:25-16:1").range_list() == [
            Ref("Exodus 15:25"),
            Ref("Exodus 15:26"),
            Ref("Exodus 15:27"),
            Ref("Exodus 16:1"),
    ...
    def test_stating_refs_of_span(self):
        assert Ref("Zohar 1:3b:12-3:12b:1").starting_refs_of_span() == [
            Ref("Zohar 1:3b:12"),
            Ref("Zohar 2"),
            Ref("Zohar 3"),
        ]
        assert Ref("Genesis 12:1-14:3").starting_refs_of_span() == [
            Ref("Genesis 12:1"),
            Ref("Genesis 13"),
            Ref("Genesis 14"),
        ]
        assert Ref("Zohar 1:3b:12-1:4b:12").starting_refs_of_span() == [
            Ref("Zohar 1:3b:12")
        ]
        assert Ref("Zohar 1:3b:12-1:4b:12").starting_refs_of_span(True) == [
            Ref("Zohar 1:3b:12"),
            Ref("Zohar 1:4a"),
            Ref("Zohar 1:4b"),
        ]
    ...
    def test_as_ranged_segment_ref(self):
        ...
        assert Ref("Rashi on Shabbat.3b").as_ranged_segment_ref() == Ref(
            "Rashi on Shabbat.3b.1.1-3b.13.1"
        )
        assert Ref("Tur, Orach Chaim.57-59").as_ranged_segment_ref() == Ref(
            "Tur, Orach Chaim.57.1-59.1"
        )
        # empty at the end
        assert Ref("Tosafot on Bava Metzia.2a").as_ranged_segment_ref() == Ref(
            "Tosafot on Bava Metzia.2a.1.1-2a.12.1"
        )
        # empty at the beginning
        assert Ref("Tosafot on Bava Metzia.3a").as_ranged_segment_ref() == Ref(
            "Tosafot on Bava Metzia.3a.1.1-3a.18.1"
        )
    ...
    def test_ref_regex(self):
        ...
        assert (
            Ref("Exodus 15:15-17").regex()
            == "^Exodus( 15:15$| 15:15:| 15:15 \\d| 15:16$| 15:16:| 15:16 \\d| 15:17$| 15:17:| 15:17 \\d)"
        )
        assert Ref("Yoma 14a").regex() == "^Yoma( 14a$| 14a:| 14a \\d)"
        assert (
            Ref("Yoma 14a:12-15").regex()
            == "^Yoma( 14a:12$| 14a:12:| 14a:12 \\d| 14a:13$| 14a:13:| 14a:13 \\d| 14a:14$| 14a:14:| 14a:14 \\d| 14a:15$| 14a:15:| 14a:15 \\d)"
        )
        assert Ref("Yoma").regex() == "^Yoma($|:| \\d)"  # This is as legacy had it

    def test_spanning_ref_regex(self):
        assert (
            Ref("Exodus 4:30-6:2").regex()
            == "^Exodus( 4:30$| 4:30:| 4:30 \\d| 4:31$| 4:31:| 4:31 \\d| 5$| 5:| 5 \\d| 6:1$| 6:1:| 6:1 \\d| 6:2$| 6:2:| 6:2 \\d)"
        )
    ...
    def test_version_list(self):
        ...
        assert len(Ref("Shabbat").version_list()) > len(
            Ref("Shabbat 5b").version_list()
        )
    ...
    def test_surrounding_ref(self):
        ...
        assert Ref("Genesis 1:3-2:23").surrounding_ref(3) == Ref(
            "Genesis 1:1-2:25"
        )  # Chapter ends on both sides
    ...
    def test_is_segment_level(self):
        ...
        assert not Ref(
            "Orot,_Lights_from_Darkness,_Land_of_Israel"
        ).is_segment_level()  # JA root in complex text
        assert not Ref(
            "Orot,_Lights_from_Darkness,_Land_of_Israel.4"
        ).is_segment_level()
    ...
    def test_is_section_level(self):
        ...
        assert not Ref(
            "Orot,_Lights_from_Darkness,_Land_of_Israel"
        ).is_section_level()  # JA root in complex text
        assert Ref("Orot,_Lights_from_Darkness,_Land_of_Israel.4").is_section_level()
        assert not Ref(
            "Orot,_Lights_from_Darkness,_Land_of_Israel.4.1"
        ).is_section_level()
...
class Test_Cache(object):
    ...
    def test_obj_created_cache_identity(self):
        ...
        assert Ref("Rashi on Genesis 2:3:1").context_ref() is Ref(
            "Rashi on Genesis 2:3"
        )
...
class Test_normal_forms(object):
    ...
    def test_talmud_range_b_to_b(self):
        ...
        assert (
            oref.he_normal() == "בבא מציעא כ׳ ב-כ״א ב" == oref_capitalized.he_normal()
        )
    ...
    def test_first_available_section_ref(self):
        ...
        assert Ref("Siddur Ashkenaz").first_available_section_ref() == Ref(
            "Siddur Ashkenaz, Weekday, Shacharit, Preparatory Prayers, Modeh Ani"
        )
        assert Ref(
            "Penei Moshe on Jerusalem Talmud Shabbat 2"
        ).first_available_section_ref() == Ref(
            "Penei Moshe on Jerusalem Talmud Shabbat 2:1:1"
        )
        assert Ref(
            "Animadversions by Elias Levita on Sefer HaShorashim"
        ).first_available_section_ref() == Ref(
            "Animadversions by Elias Levita on Sefer HaShorashim, אבב"
        )
        assert Ref("Jastrow, שְׁמַע I 1").first_available_section_ref() == Ref(
            "Jastrow, שְׁמַע I 1"
        )
...
class Test_term_refs(object):
    def test_ref_resolution(self):
        ...
        assert Ref("Shemot") == Ref(
            "Exodus"
        )  # This behavior may change, if we spec it more carefully
...
class Test_comparisons(object):
    def test_overlaps(self):
        ...
        assert not Ref("Rashi on Genesis 5:10-20").overlaps(
            Ref("Rashi on Genesis 5:21-25")
        )

        assert Ref("Rashi on Genesis 5:10-6:20").overlaps(
            Ref("Rashi on Genesis 6:18-25")
        )
        assert not Ref("Rashi on Genesis 5:10-6:20").overlaps(
            Ref("Rashi on Genesis 6:21-25")
        )
    ...
    def test_contains(self):
        ...
        assert not Ref("Rashi on Genesis 5:10-20").contains(
            Ref("Rashi on Genesis 5:21-25")
        )
        assert not Ref("Rashi on Genesis 5:10-20").contains(
            Ref("Rashi on Genesis 5:15-25")
        )

        assert Ref("Rashi on Genesis 5:10-6:20").contains(
            Ref("Rashi on Genesis 6:18-19")
        )
        assert not Ref("Rashi on Genesis 5:10-6:20").contains(
            Ref("Rashi on Genesis 6:21-25")
        )
        assert not Ref("Rashi on Genesis 5:10-6:20").contains(
            Ref("Rashi on Genesis 6:5-25")
        )
        ...
        assert not Ref(
            "Steinsaltz_on_Jerusalem_Talmud_Shekalim.4.4.42-5.1.10"
        ).contains(Ref("Steinsaltz on Jerusalem Talmud Shekalim 4:4:1"))
    ...
    def test_precedes(self):
        ...
        assert not Ref("Rashi on Genesis 5:10-20").precedes(
            Ref("Rashi on Genesis 5:18-20")
        )
        ...
        assert not Ref("Rashi on Genesis 5:10-6:20").precedes(
            Ref("Rashi on Genesis 6:18-19")
        )
        assert Ref("Rashi on Genesis 5:10-6:20").precedes(
            Ref("Rashi on Genesis 6:21-25")
        )
        assert not Ref("Rashi on Genesis 5:10-6:20").precedes(
            Ref("Rashi on Genesis 6:5-25")
    ...
    def test_follows(self):
        ...
        assert not Ref("Rashi on Genesis 5:18-20").follows(
            Ref("Rashi on Genesis 5:10-20")
        )
        assert Ref("Rashi on Genesis 5:21-25").follows(Ref("Rashi on Genesis 5:10-20"))
        assert not Ref("Rashi on Genesis 5:15-25").follows(
            Ref("Rashi on Genesis 5:10-20")
        )

        assert not Ref("Rashi on Genesis 6:18-19").follows(
            Ref("Rashi on Genesis 5:10-6:20")
        )
        assert Ref("Rashi on Genesis 6:21-25").follows(
            Ref("Rashi on Genesis 5:10-6:20")
        )
        assert not Ref("Rashi on Genesis 6:5-25").follows(
            Ref("Rashi on Genesis 5:10-6:20")
        )
...
class Test_condition_and_projection(object):
    ...
    def test_projection_simple_section(self):
        ...
        assert all(
            [
                k in p
                for k in Version.required_attrs + Version.optional_attrs
                if k != Version.content_attr
            ]
        )
...
class Test_Order_Id(object):
    ...
    def test_ordering_of_complex_texts(self):
        assert (
            Ref("Meshekh Chokhmah, Vaera 2").order_id()
            > Ref("Meshekh Chokhmah, Shemot 6").order_id()
        )
