...
from . import following, passage, ref_data, text, topic, user_profile
...
class Story(abst.AbstractMongoRecord):
    ...
    def build_sheet_metadata_dict(metadata, sheet_id, return_id=False):
        d = {
            ...
            "sheet_summary": strip_tags(metadata["summary"])
    ...
    def sheet_metadata_bulk(sid_list, return_id=False, public=True):
        ...
        return [
            Story.build_sheet_metadata_dict(metadata, metadata["id"], return_id)
    ...
    def contents(self, **kwargs):
        ...
        if "ref" in d:
            ...
            d["text"] = {  # todo: should we allow this to be stored, alternatively?
                "en": text.TextChunk(
                    oref, "en", d.get("versions", {}).get("en")
                ).as_sized_string(),
                "he": text.TextChunk(
                    oref, "he", d.get("versions", {}).get("he")
                ).as_sized_string(),
            }
        if "refs" in d:
        ...
        if "sheet_ids" in d:
            d["sheets"] = [
                self.sheet_metadata(i, return_id=True) for i in d["sheet_ids"]
            ]
            if "publisher_id" not in d:
                for sheet_dict in d["sheets"]:
                    sheet_dict.update(
                        self.publisher_metadata(sheet_dict["publisher_id"])
                    )

        if "author_key" in d:
            p = topic.Topic.init(d["author_key"])
            d["author_names"] = {
                "en": p.get_primary_title("en"),
                "he": p.get_primary_title("he"),
            }
...
class SharedStory(Story):
    ...
    def from_global_notification(cls, gn):
        mapping = {"general": "freeText", "index": "newIndex", "version": "newVersion"}
...
class SharedStorySet(abst.AbstractMongoSet):
    ...
    def __init__(self, query=None, page=0, limit=0, sort=None):
        ...
        super(SharedStorySet, self).__init__(
    ...
    def for_traits(cls, traits, query=None, page=0, limit=0, sort=None):
        q = {
            "$or": [
                {"mustHave": {"$exists": False}},
                {"$expr": {"$setIsSubset": ["$mustHave", traits]}},
            ],
            "cantHave": {"$nin": traits},
        }
...
class UserStory(Story):
    ...
    required_attrs = ["uid", "is_shared", "timestamp"]
        ...
        c = cls(
            {
                ...
                "uid": user_id,
                "data": {"publisher_id": publisher_id, "sheet_id": sheet_id},
            }
        )
    ...
    def latest_shared_for_user(uid):
        n = db.user_story.find_one(
            {"uid": uid, "is_shared": True}, {"_id": 1}, sort=[["_id", -1]]
        )
...
class UserStorySet(abst.AbstractMongoSet):
    ...
    def __init__(self, query=None, page=0, limit=0, sort=None, uid=None):
        ...
        super(UserStorySet, self).__init__(
            query=query, page=page, limit=limit, sort=sort
        )
    ...
    def _add_shared_stories(uid, traits):
        ...
        if latest_shared_id and (latest_shared_id != latest_id_for_user):
            query = (
                ...
                if latest_id_for_user is not None
                else None
            )
            SharedStorySet.for_traits(traits, query=query, limit=10).register_for_user(
                uid
            )
...
class NewVersionStoryFactory(object):
    ...
    def _data_object(cls, **kwargs):
        ...
        assert v
        d = {"index": i.title, "version": v.versionTitle, "language": v.language}
...
class TextPassageStoryFactory(AbstractStoryFactory):
    ...
    def _data_object(cls, **kwargs):
        ...
        if (
            kwargs.get("lead")
            and kwargs.get("lead").get("en")
            and kwargs.get("lead").get("he")
        ):
    ...
    def create_aliyah(cls, **kwargs):
        def _create_aliyah_story(parasha_obj, mustHave=None, **kwargs):
            ...
            if aliyah >= 6:  # Friday
                ...
                title = {
                    "en": cal["displayValue"]["en"] + ", Sixth and Seventh Aliyah",
                    "he": cal["displayValue"]["he"] + " - " + "שישי ושביעי",
                }
            else:
                ref = aliyah_ref(parasha_obj, aliyah).normal()
                title = {
                    "en": cal["displayValue"]["en"]
                    + ", "
                    + schema.AddressAliyah.en_map[aliyah - 1]
                    + " Aliyah",
                    "he": cal["displayValue"]["he"]
                    + " - "
                    + schema.AddressAliyah.he_map[aliyah - 1],
                }
            cls._generate_shared_story(
                ref=ref, lead=lead, title=title, mustHave=mustHave or [], **kwargs
            ).save()
    ...
    def generate_from_user_history(cls, hist, **kwargs):
        ...
        return cls._generate_user_story(
            uid=hist.uid,
            ref=hist.ref,
...
class MultiTextStoryFactory(AbstractStoryFactory):
    ...
    def create_daf_connection_story(cls, **kwargs):
        ...
        if category == "Talmud":
            ...
            title = {
                "en": category + " on the Daf",
                "he": hebrew_term(category) + " " + "על הדף",
            }

        try:
            cls.generate_story(
                refs=[
                    connection_link.ref_opposite(connection_ref).normal(),
    ...
    def create_parasha_verse_connection_stories(cls, iteration=1, **kwargs):
        def _create_parasha_verse_connection_story(
            ...
            cls.generate_story(
                ...
                title={
                    "en": category + " on " + cal["displayValue"]["en"],
                    "he": hebrew_term(category) + " על " + cal["displayValue"]["he"],
                },
        ...
        create_israel_and_diaspora_stories(
            _create_parasha_verse_connection_story, **kwargs
        )

    @classmethod
    def create_parasha_verse_commentator_stories(cls, iteration=1, **kwargs):
        def _create_parasha_verse_commentator_story(
            parasha_obj, mustHave=None, **kwargs
        ):
            from sefaria.utils.calendars import (
                make_parashah_response_from_calendar_entry,
            ...
            cls.generate_story(
                ...
                title={
                    "en": commentator + " on " + cal["displayValue"]["en"],
                    "he": hebrew_term(commentator) + " על " + cal["displayValue"]["he"],
                },
                lead={"en": "Weekly Torah Portion", "he": "פרשת השבוע"},
                mustHave=mustHave,
                **kwargs
        ...
        create_israel_and_diaspora_stories(
            _create_parasha_verse_commentator_story, **kwargs
        )
...
class AuthorStoryFactory(AbstractStoryFactory):
    ...
    def _data_object(cls, **kwargs):
        ...
        return {
            "author_key": prs.slug,
            "example_work": random.choice(prs.get_authored_indexes()).title,
        }
...
class UserSheetsFactory(AbstractStoryFactory):
    ...
    def _data_object(cls, **kwargs):
        ...
        sheets = (
            db.sheets.find({"owner": int(author_uid)}, {"id": 1})
            .sort([["views", -1]])
            .limit(4)
        )
...
class CollectionSheetListFactory(AbstractStoryFactory):
    ...
    def create_shared_story(cls, collection_slug, sheet_ids, **kwargs):
        story = cls._generate_shared_story(
            collection_slug=collection_slug, sheet_ids=sheet_ids, **kwargs
        )
        story.save()

    @classmethod
    def create_user_story(cls, uid, collection_slug, sheet_ids, **kwargs):
        story = cls._generate_user_story(
            uid=uid, collection_slug=collection_slug, sheet_ids=sheet_ids, **kwargs
        )
        story.save()
    ...
    def create_nechama_sheet_stories(cls, **kwargs):
        def _create_nechama_sheet_story(parasha_obj, mustHave=None, **kwargs):
            ...
            sheets = db.sheets.find(
                {
                    "status": "public",
                    "displayedCollection": "גיליונות-נחמה",
            ...
            cls.generate_story(
                ...
                title={
                    "en": "Nechama on " + cal["displayValue"]["en"],
                    "he": "נחמה על " + cal["displayValue"]["he"],
                },
                lead={"en": "Weekly Torah Portion", "he": "פרשת השבוע"},
                mustHave=mustHave,
                **kwargs
...
class SheetListFactory(AbstractStoryFactory):
    ...
    def create_parasha_sheets_stories(cls, iteration=1, k=3, **kwargs):
        def _create_parasha_sheet_story(parasha_obj, mustHave=None, **kwargs):
            ...
            cls.generate_story(
                sheet_ids=sheet_ids,
                title={
                    "en": "Sheets on " + cal["displayValue"]["en"],
                    "he": "גליונות על " + cal["displayValue"]["he"],
                },
                lead={"en": "Weekly Torah Portion", "he": "פרשת השבוע"},
                mustHave=mustHave,
                **kwargs
    ...
    def generate_featured_story(cls, **kwargs):
        return cls.generate_story(
            sheet_ids=cls._get_featured_ids(3),
            title={"en": "Popular", "he": "מומלץ"},
            **kwargs
        )
...
class TopicListStoryFactory(AbstractStoryFactory):
    ...
    def _data_object(cls, **kwargs):
        ...
        return {
            "topics": [
                {"en": topic["en"], "he": topic["he"], "slug": topic["slug"]}
                for topic in kwargs.get("topics")
            ],
            "title": kwargs.get(
                "title", {"en": "Trending Recently", "he": "נושאים עדכניים"}
            ),
    ...
    @classmethod
    def generate_parasha_topics_story(
        cls, parasha_obj, mustHave, iteration, k, **kwargs
    ):
        from sefaria.helper.topic import get_topic_by_parasha
        ...
        return cls.generate_story(
            ...
            title={
                "en": "Topics in " + cal["displayValue"]["en"],
                "he": "נושאים ב" + cal["displayValue"]["he"],
            },
            lead={"en": "Weekly Torah Portion", "he": "פרשת השבוע"},
            mustHave=mustHave or [],
            **kwargs
    ...
    def create_parasha_topics_stories(cls, iteration=1, k=6, **kwargs):
        def _create_parasha_topic_story(parasha_obj, mustHave=None, **kwargs):
            cls.generate_parasha_topics_story(
                parasha_obj, mustHave, iteration, k, **kwargs
            ).save()
...
class TopicTextsStoryFactory(AbstractStoryFactory):
    ...
    def _data_object(cls, **kwargs):
        ...
        if not trefs:
            trefs = [
                link.ref
                for link in topic.link_set(
                    _class="refTopic",
                    query_kwargs={"is_sheet": False},
                    limit=num,
                    sort=[("order.pr", -1)],
                )
            ]
...
def random_commentary_on(ref):
    commentary_refs = [
        l.ref_opposite(ref)
        for l in [x for x in ref.linkset() if x.type == "commentary"]
    ]
