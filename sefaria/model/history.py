...
def log_text(user, action, oref, lang, vtitle, old_text, new_text, **kwargs):
    ...
    if isinstance(new_text, list):
        if not isinstance(
            old_text, list
        ):  # is this necessary? the TextChunk should handle it.
            old_text = [old_text]
...
def _log_general(user, kind, old_dict, new_dict, rev_type, **kwargs):
    ...
    if kind == "note":
        ...
        if (new_dict and not new_dict.get("public")) or (
            old_dict and not old_dict.get("public")
        ):
            return
...
class History(abst.AbstractMongoRecord):
    collection = "history"
    required_attrs = ["rev_type", "user", "date"]
    optional_attrs = [
...
def process_version_title_change_in_history(ver, **kwargs):
    ...
    db.history.update(
        query, {"$set": {"version": kwargs["new"]}}, upsert=False, multi=True
    )
