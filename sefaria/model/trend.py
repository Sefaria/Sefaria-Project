...
def get_session_traits(request, uid=None):
    ...
    if uid is not None:
        traits.update(
            {
                "readsHebrew": Trend.get_user_trend_value(uid, "HebrewAbility") >= 0.5,
                "toleratesEnglish": Trend.get_user_trend_value(uid, "EnglishTolerance")
                >= 0.05,
...
class DateRange(object):
    ...
    def this_hebrew_year(cls):
        ...
        try:
            if this_gregorian_year_rh.date() > today:
                return cls(
                    "this_hebrew_year",
                    cls.new_years_dict[today.year - 1],
                    this_gregorian_year_rh,
                )
            else:
                return cls(
                    "this_hebrew_year",
            ...
            return cls(
                "this_hebrew_year", cls.new_years_dict[latest_year - 1], latest_year
            )
    ...
    def previous_hebrew_year(cls):
        ...
        try:
            if this_gregorian_year_rh.date() > today:
                return cls(
                    "previous_hebrew_year",
                    cls.new_years_dict[today.year - 2],
                    cls.new_years_dict[today.year - 1],
                )
            else:
                return cls(
            ...
            return cls(
                "this_hebrew_year",
                cls.new_years_dict[latest_year - 2],
                cls.new_years_dict[latest_year - 2],
            )
...
class Trend(abst.AbstractMongoRecord):
    ...
    optional_attrs = ["uid"]  # Required when scope is not "site"
...
def getAllUsersSheetUsage(daterange):
    pipeline = [
        {"$match": daterange.update_match({"secondary": False, "is_sheet": True})},
        {
            "$group": {"_id": "$uid", "cnt": {"$sum": 1}}
        },  # Sheet records never have num_times_read greater than 1.
    ]
...
def getAllUsersCategories(daterange, category):
    pipeline = [
        {
            "$match": daterange.update_match(
                {"secondary": False, "is_sheet": False, "categories.0": category}
            )
        },
        {"$group": {"_id": "$uid", "cnt": {"$sum": {"$max": ["$num_times_read", 1]}}}},
    ]
    results = db.user_history.aggregate(pipeline)
    return {d["_id"]: d for d in results}
...
def getAllUsersSheetCreation(daterange, publishedOnly=False):
    pipeline = [
        {
            "$match": daterange.update_match(
                {"status": "public"} if publishedOnly else {}, field="dateModified"
            )
        },  # is this correct
        {
            "$group": {"_id": "$owner", "cnt": {"$sum": 1}}
        },  # Sheet records never have num_times_read greater than 1.
    ]
...
def site_stats_data():
    ...
    for daterange in active_dateranges:
        d[daterange.key] = {
            "categoriesRead": {
                reverse_read_in_category_key(t.name): t.value
                for t in TrendSet(
                    {
                        "scope": "site",
                        "period": daterange.key,
                        "name": {
                            "$in": list(map(read_in_category_key, top_categories))
                        },
                    }
                )
            }
    ...
    for daterange in active_dateranges:
        ...
        usheet_views = db.user_history.aggregate(
            [
                ...
                {"$group": {"_id": "$sheet_id", "cnt": {"$sum": 1}}},
            ]
        )

        most_popular_sheet_ids = [
            s["_id"]
            for s in sorted(usheet_views, key=lambda o: o["cnt"], reverse=True)[:3]
        ]
        most_popular_sheets = []
        ...
        sheets_this_period = [
            ...
            if daterange.contains(
                datetime.strptime(s["created"], "%Y-%m-%dT%H:%M:%S.%f")
            )
        ]
        ...
        refs_viewed = db.user_history.aggregate(
            [
                {
                    "$match": daterange.update_match(
                        {"uid": uid, "secondary": False, "is_sheet": False}
                    )
                },
                {
                    "$group": {"_id": "$ref", "cnt": {"$sum": 1}}
                },  # Using $num_times_read isn't reliable.  It counts book views, but not text views.
            ]
        )
        most_viewed_trefs = [
            s["_id"]
            for s in sorted(refs_viewed, key=lambda o: o["cnt"], reverse=True)
            if s["cnt"] > 1 and "Genesis 1" not in s["_id"]
        ][:9]
        most_viewed_refs = [text.Ref(r) for r in most_viewed_trefs]
        most_viewed_ref_dicts = [
            {"en": r.normal(), "he": r.he_normal(), "book": r.index.title}
            for r in most_viewed_refs
        ]
        ...
        sheets_viewed = db.user_history.aggregate(
            [
                {
                    "$match": daterange.update_match(
                        {"uid": uid, "secondary": False, "is_sheet": True}
                    )
                },
                {"$group": {"_id": "$sheet_id", "cnt": {"$sum": 1}}},
            ]
        )
        most_viewed_sheets_ids = [
            s["_id"]
            for s in sorted(sheets_viewed, key=lambda o: o["cnt"], reverse=True)
            if s["cnt"] > 1 and s["_id"] not in usheet_ids
        ][:3]

        most_viewed_sheets = [
            Story.sheet_metadata(i, return_id=True) for i in most_viewed_sheets_ids
        ]
        ...
        user_stats_dict[daterange.key] = {
            "sheetsRead": user_profile.UserHistorySet(
                daterange.update_match(
                    {"is_sheet": True, "secondary": False, "uid": uid}
                )
            ).hits(),
            "textsRead": user_profile.UserHistorySet(
                daterange.update_match(
                    {"is_sheet": False, "secondary": False, "uid": uid}
                )
            ).hits(),
            "categoriesRead": {
                reverse_read_in_category_key(t.name): t.value
...
class HebrewAbilityFactory(TrendFactory):
    name = "HebrewAbility"
    desc = (
        "Value between 0 and 1 - 1 Being clear Hebrew ability, 0 being clear inability"
    )
    datatype = "float"  # int, float, str, bool, dict
    for_user = True
    for_group = False
    ...
    def getUsersWhoAreLearningSchedule(self):
        ...
        for user in self.getRelevantUserHistories():
            ...
            for history in user["history"]:
                ...
                while (
                    index < dateRefRangesTotal and keepCheckingRefAgainstBuckets == True
                ):
                    inRange, keepCheckingRefAgainstBuckets = self.dateRefRanges[
                        index
                    ].refIsInRange(history["ref"], history["datetime"])
                    if inRange:
                        refsInRangeCount += 1
...
class ParashaScheduleManager(ScheduleManager):
    ...
    def __init__(
        ...
        self.overallDateRange = DateRange(
            "overall",
            self.dateRefRanges[-1].dateRange.start,
            self.dateRefRanges[0].dateRange.end,
        )
    ...
    def createDateRefRanges(self):
        ...
        """
        query = {}
        query["date"] = {"$lte": self.dateRangeEnd + timedelta(days=self.daysToAdd)}
        query["diaspora"] = self.diaspora
        results = db.parshiot.find(query, limit=self.numberOfSegments).sort("date", -1)
        dateRefRanges = []
        for result in results:
            dateRefRanges.append(
                DateRefRange(
                    result["ref"],
                    result["date"] - self.varianceBack,
                    result["date"] + self.varianceBack,
                    result["parasha"],
                )
            )
    ...
    def getRelevantUserHistories(self):
        query = [
            {
                "$match": self.overallDateRange.update_match(
                    {"is_sheet": False, "book": self.dateRefRanges[0].ref.book}
                )
            },
            {"$sort": {"datetime": 1}},
            {
                "$group": {
                    "_id": {"uid": "$uid"},
                    "history": {
                        "$push": {"ref": "$ref", "datetime": "$datetime", "uid": "$uid"}
                    },
                }
            },
        ]
