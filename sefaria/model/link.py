...
class Link(abst.AbstractMongoRecord):
    ...
    def _validate(self):
        ...
        if (
            self.type == "essay"
        ):  # when type is 'essay', versionTitles should correspond to indices referred to in self.refs
            assert hasattr(self, "versions") and hasattr(
                self, "displayedText"
            ), "You must set versions and displayedText fields for type 'essay'."
            assert (
                "en" in self.displayedText[0]
                and "he" in self.displayedText[0]
                and "en" in self.displayedText[1]
                and "he" in self.displayedText[1]
        ...
        if hasattr(self, "charLevelData"):
            ...
            assert self.charLevelData[0]["versionTitle"] in [
                v["versionTitle"] for v in text.Ref(self.refs[0]).version_list()
    ...
    def _pre_save(self):
        if getattr(self, "_id", None) is None:
            ...
            if not samelink:
                ...
                if (
                    oref0.is_range()
                    and oref0.all_segment_refs() == section0.all_segment_refs()
                ):
                    samelink = Link().load(
                        {"$and": [{"refs": section0.normal()}, {"refs": self.refs[1]}]}
                    )
                elif oref0.is_section_level():
                    ranged0 = text.Ref(
                        f"{oref0.all_segment_refs()[0]}-{oref0.all_segment_refs()[-1]}"
                    )
                    samelink = Link().load(
                        {"$and": [{"refs": ranged0.normal()}, {"refs": self.refs[1]}]}
                    )
                elif (
                    oref1.is_range()
                    and oref1.all_segment_refs() == section1.all_segment_refs()
                ):
                    samelink = Link().load(
                        {"$and": [{"refs": section1.normal()}, {"refs": self.refs[0]}]}
                    )
                elif oref1.is_section_level():
                    ranged1 = text.Ref(
                        f"{oref1.all_segment_refs()[0]}-{oref1.all_segment_refs()[-1]}"
                    )
                    samelink = Link().load(
                        {"$and": [{"refs": ranged1.normal()}, {"refs": self.refs[0]}]}
                    )
            ...
            if samelink:
                if hasattr(self, "score") and hasattr(self, "charLevelData"):
                    ...
                    samelink.save()
                    raise DuplicateRecordError(
                        "Updated existing link with new type: {}".format(self.type)
                    )
                    ...
                    samelink.refs = (
                        self.refs
                    )  # in case the refs are reversed. switch them around
                    samelink.save()
                    raise DuplicateRecordError(
                        "Updated existing link with auto generation data {} - {}".format(
                            self.refs[0], self.refs[1]
                        )
                    )
                else:
                    raise DuplicateRecordError(
                        "Link already exists {} - {}. Try editing instead.".format(
                            self.refs[0], self.refs[1]
                        )
                    )
    ...
    def _set_available_langs(self):
        ...
        def lang_list(ref):
            return [
                ...
                if text.Ref(ref).is_text_fully_available(lang)
            ]
    ...
    def _set_expanded_refs(self):
        self.expandedRefs0 = [
            oref.normal() for oref in text.Ref(self.refs[0]).all_segment_refs()
        ]
        self.expandedRefs1 = [
            oref.normal() for oref in text.Ref(self.refs[1]).all_segment_refs()
        ]
...
class LinkSet(abst.AbstractMongoSet):
    ...
    def filter(self, sources):
        ...
        for source in sources:
            expanded_sources += (
                ...
                else text.library.get_indexes_in_category(source)
            )
        ...
        for source in self:
            if any(
                [re.match(regex, source.refs[0]) for regex in regexes]
                + [re.match(regex, source.refs[1]) for regex in regexes]
            ):
    ...
    def summary(self, relative_ref):
        ...
        for link in self:
            ref = (
                link.refs[0] if link.refs[1] == relative_ref.normal() else link.refs[1]
            )
            try:
                oref = text.Ref(ref)
            except:
...
def process_index_title_change_in_links(indx, **kwargs):
    ...
    for l in links:
        ...
        l.expandedRefs0 = [
            ...
            for r in l.expandedRefs0
        ]
        l.expandedRefs1 = [
            r.replace(kwargs["old"], kwargs["new"], 1)
            if re.search("|".join(patterns), r)
            else r
            for r in l.expandedRefs1
        ]
        try:
            ...
            logger.warning(
                "Deleting link that failed to save: {} - {}".format(
                    l.refs[0], l.refs[1]
                )
            )
            l.delete()
    ...
    if lang and available is not None:
        for link in links:
            ...
            if available:
                ...
                link.availableLangs[pos] = [
                    alang for alang in link.availableLangs[pos] if alang != lang
                ]
...
def get_link_counts(cat1, cat2):
    ...
    titles = []
    for c in [cat1, cat2]:
        ts = text.library.get_indexes_in_corpus(
            c
        ) or text.library.get_indexes_in_category(c)
        if len(ts) == 0:
            try:
    ...
    for title1 in titles[0]:
        for title2 in titles[1]:
            ...
            links = LinkSet(
                {"$and": [{"refs": {"$regex": re1}}, {"refs": {"$regex": re2}}]}
            )
            count = links.count()
            if count:
...
def get_category_category_linkset(cat1, cat2):
    ...
    for i, cat in enumerate([cat1, cat2]):
        queries += [
            {"$and": [{"categories": cat}, {"dependence": {"$in": [False, None]}}]}
        ]
        titles += [
            text.library.get_indexes_in_corpus(cat)
            or text.library.get_indexes_in_category(cat)
        ]
        if len(titles[i]) == 0:
...
def get_book_category_linkset(book, cat):
    ...
    :return:
    """
    titles = text.library.get_indexes_in_corpus(
        cat
    ) or text.library.get_indexes_in_category(cat)
    if len(titles) == 0:
        try:
    ...
    return LinkSet(
        {"$and": [{"refs": {"$regex": book_re}}, {"refs": {"$regex": cat_re}}]}
    )
