...
from sefaria.model.abstract import (
    AbstractMongoRecord,
    AbstractMongoSet,
...
from sefaria.system.exceptions import DuplicateRecordError, InputError, ManuscriptError
...
class Manuscript(SluggedAbstractMongoRecord):
    ...
    def normalize_slug_field(self, slug_field: str) -> str:
        ...
        duplicate = getattr(db, self.collection).find_one(
            {"slug": slug, "_id": {"$ne": mongo_id}}
        )
        if duplicate:
            raise DuplicateRecordError(
                f"Record with the title {slug_field} already exists"
            )
...
class ManuscriptPage(AbstractMongoRecord):
    ...
    def _pre_save(self):
        ...
        manuscript_id, page_id = getattr(self, "manuscript_slug", None), getattr(
            self, "page_id", None
        )
        if (
            manuscript_id is None or page_id is None
        ):  # important to check for None explicitly, page_id=0 is valid
            raise ManuscriptError("No manuscript_id or page_id")
        if self.is_new():
            duplicate = ManuscriptPage().load(
                {"manuscript_id": manuscript_id, "page_id": page_id}
            )
            if duplicate:
                raise DuplicateRecordError(
...
class ManuscriptPageSet(AbstractMongoSet):
    ...
    def load_by_ref(cls, oref):
        return cls(
            {"$or": cls.expanded_ref_query_for_ref(oref)}, hint="expanded_refs_1"
        )
...
def process_index_title_change_in_manuscript_links(indx, **kwargs):
    ...
    for o in objs:
        o.contained_refs = [
            r.replace(kwargs["old"], kwargs["new"], 1)
            if reg_reg.search("|".join(patterns), r)
            else r
            for r in o.contained_refs
        ]
        o.expanded_refs = [
            r.replace(kwargs["old"], kwargs["new"], 1)
            if reg_reg.search("|".join(patterns), r)
...
def process_slug_change_in_manuscript(man, **kwargs):
    ManuscriptPageSet({"manuscript_slug": kwargs["old"]}).update(
        {"manuscript_slug": kwargs["new"]}
    )
