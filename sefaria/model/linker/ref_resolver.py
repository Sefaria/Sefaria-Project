...
def _find_ordered_subset_indices(sequence: Sequence[str], targets: Sequence[str],
                                 disallowed: Optional[Set[int]] = None) -> Optional[List[int]]:
    """
    Return indices mapping each target to a unique matching entry in `sequence` in the given order.
    Skips entries in the optional `disallowed` set and ensures no index is reused.
    """
    disallowed = disallowed or set()
    used: Set[int] = set()
    matched_indices: List[int] = []
    for target in targets:
        # Find the first position whose slug equals `target` and that isn't already used or disallowed.
        match_idx = next(
            (idx for idx, value in enumerate(sequence)
             if value == target and idx not in used and idx not in disallowed),
            None,
        )
        if match_idx is None:
            return None
        # Reserve this index so later targets can't reuse it.
        used.add(match_idx)
