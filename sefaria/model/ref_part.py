...
SpanOrToken = Union[
    Span, Token
]  # convenience type since Spans and Tokens are very similar
...
class TrieEntry:
    ...
    key_is_id = (
        False  # is key an ID which shouldn't be manipulated with string functions?
    )
...
class RawRefPart(TrieEntry):
    ...
    def __init__(
        self,
        type: RefPartType,
        span: Optional[SpanOrToken],
        potential_dh_continuation: SpanOrToken = None,
    ) -> None:
        self.span = span
        self.type = type
        if potential_dh_continuation is not None:
            if (
                isinstance(potential_dh_continuation, Span)
                and len(potential_dh_continuation) > self.max_dh_continuation_len
            ):
                potential_dh_continuation = potential_dh_continuation[
                    : self.max_dh_continuation_len
                ]
    ...
    def dh_cont_text(self):
        return (
            ""
            if self.potential_dh_continuation is None
            else self.potential_dh_continuation.text
        )
    ...
    def realign_to_new_raw_ref(
        self, old_raw_ref_span: SpanOrToken, new_raw_ref_span: SpanOrToken
    ):
        """
...
class SectionContext(ContextPart):
    ...
    def __init__(
        self, addr_type: schema.AddressType, section_name: str, address: int
    ) -> None:
        """
...
class RawRef:
    ...
    def __init__(self, lang: str, raw_ref_parts: list, span: SpanOrToken) -> None:
        self.raw_ref_parts = self._merge_daf_amud_parts(
            lang, self._group_ranged_parts(raw_ref_parts)
        )
        self.parts_to_match = (
            self.raw_ref_parts
        )  # actual parts that will be matched. different when their are context swaps
        self.prev_num_parts_map = self._get_prev_num_parts_map(self.raw_ref_parts)
        self.span = span

    @staticmethod
    def _merge_daf_amud_parts(
        lang: str, raw_ref_parts: List["RawRefPart"]
    ) -> List["RawRefPart"]:
        """
        ...
        for ipart, part in enumerate(raw_ref_parts[:-1]):
            if part.type != RefPartType.NUMBERED:
                continue
            addr_talmud = schema.AddressTalmud(0)
            _, _, addr_classes = addr_talmud.get_all_possible_sections_from_string(
                lang, part.text, strip_prefixes=True
            )
            if schema.AddressTalmud not in addr_classes:
                continue
            _, _, addr_classes = schema.AddressInteger(
                0
            ).get_all_possible_sections_from_string(
                lang, part.text, strip_prefixes=True
            )
            if schema.AddressInteger in addr_classes:
                continue  # Don't consider if also matches AddressInteger. This is too ambiguous
            next_part = raw_ref_parts[ipart + 1]
            proposed_text = f"{part.text} {next_part.text}"
            _, _, addr_classes = addr_talmud.get_all_possible_sections_from_string(
                lang, proposed_text, strip_prefixes=True
            )
            if schema.AddressTalmud not in addr_classes:
                continue  # Only consider if still matches AddressTalmud with merged text
    ...
    def _group_ranged_parts(raw_ref_parts: List["RawRefPart"]) -> List["RawRefPart"]:
        ...
        for i, part in enumerate(raw_ref_parts):
            if part.type == RefPartType.RANGE_SYMBOL:
                ranged_symbol_ind = i
                break
        if ranged_symbol_ind is None or ranged_symbol_ind == len(raw_ref_parts) - 1:
        ...
        for i in range(ranged_symbol_ind + 1, len(raw_ref_parts)):
            if (
                i == len(raw_ref_parts) - 1
                or raw_ref_parts[i + 1].type != RefPartType.NUMBERED
            ):
        ...
        new_raw_ref_parts = (
            raw_ref_parts[: section_slice.start]
    ...
    @staticmethod
    def _get_prev_num_parts_map(
        raw_ref_parts: List[RawRefPart],
    ) -> Dict[RawRefPart, RawRefPart]:
        """
        ...
        """
        if len(raw_ref_parts) == 0:
            return {}
        prev_num_parts_map = {}
        prev_part = raw_ref_parts[0]
        for part in raw_ref_parts[1:]:
            if (
                prev_part.type == RefPartType.NUMBERED
                and part.type == RefPartType.NUMBERED
            ):
    ...
    def subspan(self, part_slice: slice) -> SpanOrToken:
        ...
        assert (
            subspan.text == parts[0].span.doc[start_token_i:end_token_i].text
        ), f"{subspan.text} != {parts[0].span.doc[start_token_i:end_token_i].text}"
        return subspan
    ...
    def map_new_indices(
        ...
        new_part_indices: List[Tuple[int, int]],
    ) -> None:
        """
        ...
        if self.span is None:
            raise InputError(
                f"${new_indices} don't match token boundaries. Using 'expand' alignment mode text is '{new_doc.char_span(*new_indices, alignment_mode='expand')}'"
            )
        doc_span = self.span.as_doc()
        for part, temp_part_indices in zip(self.raw_ref_parts, new_part_indices):
            part.span = doc_span.char_span(
                *[i - new_indices[0] for i in temp_part_indices]
            )
            if part.span is None:
                raise InputError(
                    f"{temp_part_indices} doesn't match token boundaries for part {part}. Using 'expand' alignment mode text is '{new_doc.char_span(*temp_part_indices, alignment_mode='expand')}'"
                )
...
class ResolvedRef:
    ...
    def __init__(
        self,
        raw_ref: RawRef,
        resolved_parts: List[RawRefPart],
        node,
        ref: text.Ref,
        context_ref: text.Ref = None,
        context_type: ContextType = None,
        _thoroughness=ResolutionThoroughness.NORMAL,
    ) -> None:
    ...
    def merge_parts(self, other: "ResolvedRef") -> None:
        for part in other.resolved_parts:
            if part in self.resolved_parts:
                continue
            if part.is_context:
    ...
    def has_prev_unused_numbered_ref_part_for_node(
        self, part: RawRefPart, lang: str, node: schema.SchemaNode
    ) -> bool:
        """
        ...
        """
        if (
            part.type != RefPartType.NUMBERED
            or not getattr(node, "numeric_equivalent", False)
            or not self.has_prev_unused_numbered_ref_part(part)
        ):
            return False
        try:
            (
                possible_sections,
                possible_to_sections,
                addr_classes,
            ) = schema.AddressInteger(0).get_all_possible_sections_from_string(
                lang, part.text, strip_prefixes=True
            )
            for sec, toSec, addr_class in zip(
                possible_sections, possible_to_sections, addr_classes
            ):
                if sec != node.numeric_equivalent:
                    continue
                if addr_class == schema.AddressInteger:
                    return True
        except KeyError:
            return False

    def _get_refined_matches_for_dh_part(
        self,
        lang,
        raw_ref_part: RawRefPart,
        refined_parts: List[RawRefPart],
        node: schema.DiburHamatchilNodeSet,
    ):
        """
        ...
        """
        if (
            self._thoroughness < ResolutionThoroughness.HIGH
            and self.ref.is_book_level()
        ):
            return []
        ...
        return [
            self.clone(
                resolved_parts=refined_parts.copy(),
                node=max_node,
                ref=text.Ref(max_node.ref),
            )
            for _, max_node, _ in best_matches
        ]

    def _get_refined_refs_for_numbered_part(
        self,
        raw_ref_part: RawRefPart,
        refined_parts: List[RawRefPart],
        node,
        lang,
        fromSections: List[RawRefPart] = None,
    ) -> List["ResolvedRef"]:
        if node is None:
            return []
        try:
            possible_sections, possible_to_sections, addr_classes = node.address_class(
                0
            ).get_all_possible_sections_from_string(
                lang, raw_ref_part.text, fromSections, strip_prefixes=True
            )
        ...
        for sec, toSec, addr_class in zip(
            ...
            if (
                self.has_prev_unused_numbered_ref_part(raw_ref_part)
                and addr_class == schema.AddressInteger
            ):
                """
            ...
            try:
                ...
                continue
        return [
            self.clone(resolved_parts=refined_parts, node=node, ref=refined_ref)
            for refined_ref in refined_refs
        ]

    def _get_refined_refs_for_numbered_context_part(
        self, sec_context: SectionContext, refined_parts: List[RawRefPart], node
    ) -> List["ResolvedRef"]:
    ...
    def _get_refined_matches_for_ranged_sections(
        self,
        sections: List["RawRefPart"],
        refined_parts: List[RawRefPart],
        node,
        lang,
        fromSections: list = None,
    ):
        ...
        for section_part in sections:
            ...
            for _ in range(queue_len):
                ...
                if not is_first_pass:
                    temp_resolved_raw_ref.node = (
                        temp_resolved_raw_ref.node.get_referenceable_child(
                            temp_resolved_raw_ref.ref
                        )
                    )
                is_first_pass = False
                next_resolved_raw_refs = (
                    temp_resolved_raw_ref._get_refined_refs_for_numbered_part(
                        section_part,
                        refined_parts,
                        temp_resolved_raw_ref.node,
                        lang,
    ...
    def _get_refined_matches_for_ranged_part(
        self,
        raw_ref_part: RangedRawRefParts,
        refined_parts: List[RawRefPart],
        node,
        lang,
    ) -> List["ResolvedRef"]:
        (
            section_resolved_raw_refs,
            incomplete_section_refs,
        ) = self._get_refined_matches_for_ranged_sections(
            raw_ref_part.sections, refined_parts, node, lang
        )
        toSection_resolved_raw_refs, _ = self._get_refined_matches_for_ranged_sections(
            raw_ref_part.toSections,
            refined_parts,
            node,
            lang,
            fromSections=[x.ref.sections for x in section_resolved_raw_refs],
        )
        ranged_resolved_raw_refs = []
        for section, toSection in product(
            section_resolved_raw_refs, toSection_resolved_raw_refs
        ):
            try:
                ranged_resolved_raw_refs += [
                    self.clone(
                        resolved_parts=refined_parts,
                        node=section.node,
                        ref=section.ref.to(toSection.ref),
                    )
                ]
            except InputError:
                continue
    ...
    def get_refined_matches(
        self, part: RawRefPart, node, lang: str
    ) -> List["ResolvedRef"]:
        refined_ref_parts = self.resolved_parts + [part]
        matches = []
        if (
            isinstance(node, schema.TitledTreeNode)
            and node.is_default()
            ...
            matches += [
                self.clone(
                    resolved_parts=self.resolved_parts, node=node, ref=node.ref()
                )
            ]
        # see NumberedTitledTreeNode.get_referenceable_child() for why we check if parent is None
        elif (
            part.type == RefPartType.NUMBERED
            and isinstance(node, schema.JaggedArrayNode)
            and node.parent is None
        ):
            if isinstance(part, SectionContext):
                matches += self._get_refined_refs_for_numbered_context_part(
                    part, refined_ref_parts, node
                )
            else:
                matches += self._get_refined_refs_for_numbered_part(
                    part, refined_ref_parts, node, lang
                )
        elif part.type == RefPartType.RANGE and isinstance(
            node, schema.JaggedArrayNode
        ):
            matches += self._get_refined_matches_for_ranged_part(
                part, refined_ref_parts, node, lang
            )
        elif (
            (
                part.type == RefPartType.NAMED
                and isinstance(node, schema.TitledTreeNode)
                or part.type == RefPartType.NUMBERED
                and isinstance(node, schema.ArrayMapNode)
            )
            or part.type == RefPartType.NUMBERED
            and isinstance(node, schema.SchemaNode)
        ):  # for case of numbered alt structs or schema nodes that look numbered (e.g. perakim and parshiot of Sifra)
            if node.ref_part_title_trie(lang).has_continuations(
                part.key(), key_is_id=part.key_is_id
            ) and not self.has_prev_unused_numbered_ref_part_for_node(part, lang, node):
                matches += [
                    self.clone(
                        resolved_parts=refined_ref_parts, node=node, ref=node.ref()
                    )
                ]
            ...
            if isinstance(node, schema.DiburHamatchilNodeSet):
                matches += self._get_refined_matches_for_dh_part(
                    lang, part, refined_ref_parts, node
                )
        # TODO sham and directional cases
        return matches

    def get_resolved_parts(
        self, include: Iterable[type] = None, exclude: Iterable[type] = None
    ) -> List[RawRefPart]:
        """
        ...
        for part in self.resolved_parts:
            if include is not None and not any(
                isinstance(part, typ) for typ in include
            ):
                continue
...
class AmbiguousResolvedRef:
    ...
    def __init__(self, resolved_refs: List[ResolvedRef]):
        ...
        self.raw_ref = resolved_refs[
            0
        ].raw_ref  # assumption is all resolved_refs share same raw_ref. expose at top level
...
class MatchTemplateTrie:
        ...
        for node in nodes:
            ...
            for match_template in node.get_match_templates():
                if (
                    not is_index_level
                    and self.scope != "any"
                    and match_template.scope != "any"
                    and self.scope != match_template.scope
                ):
                    continue
                ...
                for term in match_template.terms:
                    ...
                    for _ in range(len_curr_dict_queue):
                        ...
                        curr_dict_queue += self.__get_sub_tries_for_term(
                            term, curr_dict
                        )
                # add nodes to leaves
    ...
    def __get_sub_tries_for_term(
        self, term: NonUniqueTerm, curr_trie: dict
    ) -> List[dict]:
        sub_tries = []
        ...
        sub_tries += [
            self.__get_sub_trie_for_new_key(TermContext(term).key(), curr_trie)
        ]
        return sub_tries
    ...
    def has_continuations(self, key: str, key_is_id=False) -> bool:
        ...
        """
        return (
            self.get_continuations(key, default=None, key_is_id=key_is_id) is not None
        )
    ...
    def _get_continuations_recursive(
        self, key: str, prev_sub_tries=None, key_is_id=False
    ):
        ...
        for starti in starti_list:
            for endi in reversed(range(len(key) + 1)):
                ...
                if sub_key not in prev_sub_tries:
                    continue
                if endi == len(key):
                    next_sub_tries += [prev_sub_tries[sub_key]]
                    continue
                temp_sub_tries = self._get_continuations_recursive(
                    key[endi:], prev_sub_tries[sub_key]
                )
...
class MatchTemplateGraph:
    ...
    def __init__(self, nodes: List[schema.TitledTreeNode]):
        ...
        for node in nodes:
            for match_template in node.get_match_templates():
                if len(match_template.term_slugs) < 2:
                    continue
                terms = list(match_template.terms)
                for iterm, term in enumerate(terms[:-1]):
                    next_term = terms[iterm + 1]
                    if (
                        term.ref_part_role == "structural"
                        and next_term.ref_part_role == "structural"
                    ):
    ...
    def do_parents_share_child(self, parent1: str, parent2: str, child: str) -> bool:
        ...
        """
        return self.parent_has_child(parent1, child) and self.parent_has_child(
            parent2, child
        )

    def get_parent_for_children(
        self, context_match_templates: List[MatchTemplate], input_slugs: list
    ) -> Optional[str]:
    ...
    def get_shared_child(
        self, context_match_templates: List[MatchTemplate], input_slugs: List[str]
    ) -> Optional[str]:
        for template in context_match_templates:
            for i, context_slug in enumerate(template.term_slugs[:-1]):
                next_context_slug = template.term_slugs[i + 1]
                for input_slug in input_slugs:
                    if self.do_parents_share_child(
                        context_slug, input_slug, next_context_slug
                    ):
...
class TermMatcher:
    ...
    def match_term(self, ref_part: RawRefPart) -> List[NonUniqueTerm]:
        ...
        matches = []
        if ref_part.type != RefPartType.NAMED:
            return matches
        starti_inds = [0]
        if self.lang == "he":
...
class RefResolver:
    def __init__(
        self,
        raw_ref_model_by_lang: Dict[str, Language],
        raw_ref_part_model_by_lang: Dict[str, Language],
        ref_part_title_trie_by_lang: Dict[str, MatchTemplateTrie],
        ref_part_title_graph: MatchTemplateGraph,
        term_matcher_by_lang: Dict[str, TermMatcher],
    ) -> None:
        ...
        self._normalizer = NormalizerByLang(
            {
                ...
                "he": NormalizerComposer(
                    base_normalizer_steps + ["maqaf", "cantillation"]
                ),
            }
        )
    ...
    def _map_normal_output_to_original_input(
        self,
        lang: str,
        input: List[str],
        resolved: List[List[Union[ResolvedRef, AmbiguousResolvedRef]]],
    ) -> None:
        """
        ...
        for temp_input, temp_resolved in zip(input, resolved):
            ...
            mapping = self._normalizer.get_mapping_after_normalization(
                temp_input, lang=lang
            )
            conv = (
                self._normalizer.convert_normalized_indices_to_unnormalized_indices
            ...
            for (rr, (norm_raw_ref_start, _)) in zip(temp_resolved, norm_inds):
                unnorm_part_inds += [
                    conv(
                        [
                            [norm_raw_ref_start + i for i in part.char_indices]
                            for part in rr.raw_ref.raw_ref_parts
                        ],
                        mapping,
                    )
                ]
            for resolved_ref, temp_unnorm_inds, temp_unnorm_part_inds in zip(
                temp_resolved, unnorm_inds, unnorm_part_inds
            ):
                resolved_ref.raw_ref.map_new_indices(
                    unnorm_doc, temp_unnorm_inds, temp_unnorm_part_inds
                )

    def bulk_resolve_refs(
        self,
        lang: str,
        book_context_refs: List[Optional[text.Ref]],
        input: List[str],
        with_failures=False,
        verbose=False,
        reset_ibids_every_context_ref=True,
        thoroughness=ResolutionThoroughness.NORMAL,
            ...
            for raw_ref in raw_refs:
                ...
                if len(temp_resolved) == 0:
                    ...
                    if with_failures:
                        inner_resolved += [
                            ResolvedRef(
                                raw_ref, [], None, None, context_ref=book_context_ref
                            )
                        ]
    ...
    def _bulk_get_raw_refs(self, lang: str, input: List[str]) -> List[List[RawRef]]:
        ...
        ref_part_input = reduce(
            lambda a, b: a + [(sub_b.text, b[0]) for sub_b in b[1]],
            enumerate(all_raw_ref_spans),
            [],
        )
        all_raw_ref_part_spans = list(
            self._bulk_get_raw_ref_part_spans(lang, ref_part_input, as_tuples=True)
        )
        ...
        for input_idx, raw_ref_spans in enumerate(all_raw_ref_spans):
            ...
            raw_refs = []
            for ispan, (span, part_span_list) in enumerate(
                zip(raw_ref_spans, raw_ref_part_spans)
            ):
                raw_ref_parts = []
                for ipart, part_span in enumerate(part_span_list):
                    ...
                    if part_type == RefPartType.DH:
                        dh_cont = self._get_dh_continuation(
                            ispan, ipart, raw_ref_spans, part_span_list, span, part_span
                        )
    ...
    def _get_dh_continuation(
        ispan: int,
        ipart: int,
        raw_ref_spans: List[SpanOrToken],
        part_span_list: List[SpanOrToken],
        span: SpanOrToken,
        part_span: SpanOrToken,
    ) -> Optional[SpanOrToken]:
    ...
    def get_raw_ref_model(self, lang: str) -> Language:
        return self.__get_attr_by_lang(
            lang, self._raw_ref_model_by_lang, "No Raw Ref Model"
        )

    def get_raw_ref_part_model(self, lang: str) -> Language:
        return self.__get_attr_by_lang(
            lang, self._raw_ref_part_model_by_lang, "No Raw Ref Model"
        )

    def get_ref_part_title_trie(self, lang: str) -> MatchTemplateTrie:
        return self.__get_attr_by_lang(
            lang, self._ref_part_title_trie_by_lang, "No Raw Ref Part Title Trie"
        )

    def get_term_matcher(self, lang: str) -> TermMatcher:
        return self.__get_attr_by_lang(
            lang, self._term_matcher_by_lang, "No Term Matcher"
        )
    ...
    def _bulk_get_raw_ref_spans(
        self, lang: str, input: List[str], batch_size=150, **kwargs
    ) -> Generator[List[Span], None, None]:
        for doc in self.get_raw_ref_model(lang).pipe(
            input, batch_size=batch_size, **kwargs
        ):
    ...
    def _bulk_get_raw_ref_part_spans(
        self, lang: str, input: List[str], batch_size=None, **kwargs
    ) -> Generator[List[Span], None, None]:
        for doc in self.get_raw_ref_part_model(lang).pipe(
            input, batch_size=batch_size or len(input), **kwargs
        ):
            if kwargs.get("as_tuples", False):
                doc, context = doc
                yield doc.ents, context
    ...
    def split_non_cts_parts(lang, raw_ref: RawRef) -> List[RawRef]:
        ...
        for ipart, part in enumerate(raw_ref.raw_ref_parts):
            if part.type != RefPartType.NON_CTS:
                curr_parts += [part]
            if (
                part.type == RefPartType.NON_CTS
                or ipart == len(raw_ref.raw_ref_parts) - 1
            ):
                if len(curr_parts) == 0:
                    continue
                curr_part_end = ipart  # exclude curr part which is NON_CTS
                if ipart == len(raw_ref.raw_ref_parts) - 1:
                    curr_part_end = ipart + 1  # include curr part
                try:
                    raw_ref_span = raw_ref.subspan(
                        slice(curr_part_start, curr_part_end)
                    )
                    [
                        p.realign_to_new_raw_ref(raw_ref.span, raw_ref_span)
                        for p in curr_parts
                    ]
    ...
    def resolve_raw_ref(
        self, lang: str, book_context_ref: Optional[text.Ref], raw_ref: RawRef
    ) -> List[Union[ResolvedRef, AmbiguousResolvedRef]]:
        ...
        for i, temp_raw_ref in enumerate(split_raw_refs):
            ...
            context_swap_map = (
                ...
                if book_context_ref is None
                    ...
                    book_context_ref.index.nodes, "ref_resolver_context_swaps", None
                )
            )
            self._apply_context_swaps(lang, raw_ref, context_swap_map)
            unrefined_matches = self.get_unrefined_ref_part_matches(
                lang, book_context_ref, temp_raw_ref
            )
            if is_non_cts:
                # filter unrefined matches to matches that resolved previously
                resolved_titles = {r.ref.index.title for r in resolved_list}
                unrefined_matches = list(
                    filter(
                        lambda x: x.ref.index.title in resolved_titles,
                        unrefined_matches,
                    )
                )
                ...
                for match in unrefined_matches:
                    try:
                        match.ref = match.ref.subref(
                            book_context_ref.sections[
                                : -len(temp_raw_ref.raw_ref_parts)
                            ]
                        )
                    except (InputError, AttributeError):
                        continue
            temp_resolved_list = self.refine_ref_part_matches(
                lang, book_context_ref, unrefined_matches, temp_raw_ref
            )
    ...
    def get_unrefined_ref_part_matches(
        self, lang: str, book_context_ref: Optional[text.Ref], raw_ref: RawRef
    ) -> List["ResolvedRef"]:
        context_free_matches = self._get_unrefined_ref_part_matches_recursive(
            lang, raw_ref, ref_parts=raw_ref.parts_to_match
        )
        context_full_matches = []
        contexts = (
            (book_context_ref, ContextType.CURRENT_BOOK),
            (self._ibid_history.last_match, ContextType.IBID),
        )
        for context_ref, context_type in contexts:
            context_full_matches += (
                self._get_unrefined_ref_part_matches_for_graph_context(
                    lang, context_ref, context_type, raw_ref
                )
            )
        ...
        if len(matches) == 0:
            ...
            for context_ref, context_type in contexts:
                matches += self._get_unrefined_ref_part_matches_for_title_context(
                    lang, context_ref, raw_ref, context_type
                )
        return matches

    def _get_unrefined_ref_part_matches_for_title_context(
        self,
        lang: str,
        context_ref: Optional[text.Ref],
        raw_ref: RawRef,
        ...
        temp_ref_parts = raw_ref.parts_to_match + [
            TermContext(term) for term in longest_template.terms
        ]
        temp_matches = self._get_unrefined_ref_part_matches_recursive(
            lang, raw_ref, ref_parts=temp_ref_parts
        )
        matches += list(
            filter(lambda x: x.num_resolved(include={TermContext}), temp_matches)
        )
    ...
    def _get_unrefined_ref_part_matches_for_graph_context(
        self,
        lang: str,
        context_ref: Optional[text.Ref],
        context_type: ContextType,
        ...
        raw_ref_term_slugs = [
            term.slug
            for term in self.get_term_matcher(lang).match_terms(raw_ref.raw_ref_parts)
        ]
        context_parent = self._ref_part_title_graph.get_parent_for_children(
            context_match_templates, raw_ref_term_slugs
        )
        ...
        for context_slug in (context_parent,):
            ...
            temp_matches = self._get_unrefined_ref_part_matches_recursive(
                lang, raw_ref, ref_parts=raw_ref.parts_to_match + [term_context]
            )
            matches += list(
                filter(
                    lambda x: x.num_resolved(exclude={TermContext})
                    and x.num_resolved(include={TermContext}),
                    temp_matches,
                )
            )
            for match in matches:
    ...
    def _apply_context_swaps(
        self, lang: str, raw_ref: RawRef, context_swap_map: Dict[str, str] = None
    ):
        """
        ...
        for part in raw_ref.raw_ref_parts:
            ...
            for match in term_matches:
                if match.slug not in context_swap_map:
                    continue
                swapped_ref_parts += [
                    TermContext(NonUniqueTerm.init(slug))
                    for slug in context_swap_map[match.slug]
                ]
                found_match = True
                break
            if not found_match:
                swapped_ref_parts += [part]
        raw_ref.parts_to_match = swapped_ref_parts

    def _get_unrefined_ref_part_matches_recursive(
        self,
        lang: str,
        raw_ref: RawRef,
        title_trie: MatchTemplateTrie = None,
        ref_parts: list = None,
        prev_ref_parts: list = None,
    ) -> List[ResolvedRef]:
                ...
                for node in temp_title_trie[LEAF_TRIE_ENTRY]:
                    ...
                    matches += [
                        ResolvedRef(
                            ...
                            node,
                            ref,
                            _thoroughness=self._thoroughness,
                        )
                    ]
            temp_ref_parts = [temp_part for temp_part in ref_parts if temp_part != part]
            matches += self._get_unrefined_ref_part_matches_recursive(
                lang,
                raw_ref,
                temp_title_trie,
                ref_parts=temp_ref_parts,
                prev_ref_parts=temp_prev_ref_parts,
            )

        return self._prune_unrefined_ref_part_matches(matches)

    def refine_ref_part_matches(
        self,
        lang: str,
        book_context_ref: Optional[text.Ref],
        ref_part_matches: List[ResolvedRef],
        raw_ref: RawRef,
    ) -> List[ResolvedRef]:
        matches = []
        for unrefined_match in ref_part_matches:
            unused_parts = list(
                set(raw_ref.parts_to_match) - set(unrefined_match.resolved_parts)
            )
            matches += self._get_refined_ref_part_matches_recursive(
                lang, unrefined_match, unused_parts
            )
            ...
            context_ref_list = (
                [book_context_ref, self._ibid_history.last_match]
                if unrefined_match.context_ref is None
                else [unrefined_match.context_ref]
            )
            context_type_list = (
                [ContextType.CURRENT_BOOK, ContextType.IBID]
                if unrefined_match.context_ref is None
                else [unrefined_match.context_type]
            )
            for context_ref, context_type in zip(context_ref_list, context_type_list):
                matches += self._get_refined_ref_part_matches_for_section_context(
                    lang, context_ref, context_type, unrefined_match, unused_parts
                )
        return self._prune_refined_ref_part_matches(matches)

    @staticmethod
    def _get_section_contexts(
        context_ref: text.Ref, match_index: text.Index, common_index: text.Index
    ) -> List[SectionContext]:
        """
        ...
        def get_section_set(index: text.Index) -> Set[Tuple[str, str, bool]]:
            ...
            try:
                referenceable_sections = getattr(
                    root_node,
                    "referenceableSections",
                    [True] * len(root_node.addressTypes),
                )
                return set(
                    zip(
                        root_node.addressTypes,
                        root_node.sectionNames,
        ...
        referenceable_sections = getattr(
            context_node,
            "referenceableSections",
            [True] * len(context_node.addressTypes),
        )
        context_sec_list = list(
            zip(
                context_node.addressTypes,
                context_node.sectionNames,
                referenceable_sections,
            )
        )
        match_sec_set = get_section_set(match_index)
        common_sec_set = (
            get_section_set(common_index) & match_sec_set & set(context_sec_list)
        )
        if len(common_sec_set) == 0:
            return []
        sec_contexts = []
        for isec, sec_tuple in enumerate(context_sec_list):
            if sec_tuple in common_sec_set and isec < len(context_ref.sections):
                ...
                sec_contexts += [
                    SectionContext(addr_type, sec_name, context_ref.sections[isec])
                ]
        return sec_contexts

    @staticmethod
    def _get_refined_ref_part_matches_for_section_context(
        lang: str,
        context_ref: Optional[text.Ref],
        context_type: ContextType,
        ref_part_match: ResolvedRef,
        ref_parts: List[RawRefPart],
    ) -> List[ResolvedRef]:
        """
        ...
        if context_ref is None:
            return []
        context_titles = set(getattr(context_ref.index, "base_text_titles", [])) | {
            context_ref.index.title
        }
        match_titles = set(
            getattr(ref_part_match.ref.index, "base_text_titles", [])
        ) | {ref_part_match.ref.index.title}
        matches = []
        for common_base_text in context_titles & match_titles:
            common_index = text.library.get_index(common_base_text)
            sec_contexts = RefResolver._get_section_contexts(
                context_ref, ref_part_match.ref.index, common_index
            )
            matches += RefResolver._get_refined_ref_part_matches_recursive(
                lang, ref_part_match, ref_parts + sec_contexts
            )
        # remove matches which dont use context
        matches = list(
            filter(lambda x: x.num_resolved(include={SectionContext}), matches)
        )
        for match in matches:
            match.context_ref = context_ref
            match.context_type = context_type
    ...
    @staticmethod
    def _get_refined_ref_part_matches_recursive(
        lang: str, match: ResolvedRef, ref_parts: List[RawRefPart]
    ) -> List[ResolvedRef]:
        fully_refined = []
        ...
        for part in ref_parts:
            for child in children:
                ...
                for temp_match in temp_matches:
                    temp_ref_parts = list(
                        set(ref_parts) - set(temp_match.resolved_parts)
                    )
                    fully_refined += (
                        RefResolver._get_refined_ref_part_matches_recursive(
                            lang, temp_match, temp_ref_parts
                        )
                    )
    ...
    @staticmethod
    def _prune_unrefined_ref_part_matches(
        ref_part_matches: List[ResolvedRef],
    ) -> List[ResolvedRef]:
        ...
        for match in ref_part_matches:
            key = (
                match.node.title
                if isinstance(match.node, text.Index)
                else match.node.ref().normal()
            )
    ...
    def _prune_refined_ref_part_matches(
        self, resolved_refs: List[ResolvedRef]
    ) -> List[ResolvedRef]:
        """
        ...
        def filter_context_matches(match: ResolvedRef) -> bool:
            ...
            for part in match.get_resolved_parts():
                if (
                    not first_explicit_section
                    and part.type == RefPartType.NUMBERED
                    and not part.is_context
                ):
            ...
            to_match_explicit = {
                part for part in match.raw_ref.parts_to_match if not part.is_context
            }
            ...
            if match.context_type in CONTEXT_TO_REF_PART_TYPE.keys():
                ...
                for _ in range(num_parts_to_remove):
                    part = next(
                        (
                            p
                            for p in to_match_explicit
                            if p.type in CONTEXT_TO_REF_PART_TYPE[match.context_type]
                        ),
                        None,
                    )
                    if part is None:
        ...
        context_free_matches = list(
            filter(
                lambda m: m.context_ref is None
                and set(m.get_resolved_parts()) == set(m.raw_ref.parts_to_match),
                resolved_refs,
            )
        )
        ...
        for resolved_ref in resolved_refs:
            if resolved_ref.order_key != top_order_key:
                break
        ...
        if self._thoroughness >= ResolutionThoroughness.HIGH:
            ...
            max_resolved_refs = list(
                filter(lambda x: not x.ref.is_empty(), max_resolved_refs)
            )
