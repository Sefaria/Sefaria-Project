...
from sefaria.utils.hebrew import has_cantillation, is_hebrew, strip_cantillation
...
class WordForm(abst.AbstractMongoRecord):
    ...
    optional_attrs = ["c_form", "refs", "language_code", "generated_by"]
...
class Lexicon(abst.AbstractMongoRecord):
    collection = "lexicon"
    required_attrs = ["name", "language", "to_language", "text_categories"]
    ...
    def as_strings(self, with_headword=True):
        ...
        for sense in self.content["senses"]:
            if "grammar" in sense:
                ...
                next_line = (
                    "<br/>&nbsp;&nbsp;&nbsp;&nbsp;<strong>{}</strong> - ".format(
                        sense["grammar"]["verbal_stem"]
                    )
                )
                next_line += ", ".join(
                    [
                        '<strong dir="rtl">{}</strong>'.format(b)
                        for b in sense["grammar"]["binyan_form"]
                    ]
                )
                try:
...
class RashiDictionaryEntry(DictionaryEntry):
    required_attrs = DictionaryEntry.required_attrs + [
        "content",
        "orig_word",
        "orig_ref",
        "catane_number",
...
class HebrewDictionaryEntry(DictionaryEntry):
    ...
    def headword_string(self):
        headwords = [self.headword] + getattr(self, "alt_headwords", [])
        string = (
            ", ".join([f"<strong><big>{hw}</big></strong>" for hw in headwords])
            + "\xa0\xa0"
        )
        return string
...
class JastrowDictionaryEntry(DictionaryEntry):
    ...
    def headword_string(self):
        ...
        for hw in [self.headword] + getattr(self, "alt_headwords", []):
            hw = re.sub(
                r" [\u00B2\u00B3\u2074\u2075\u2076]", "", hw
            )  # Drop superscripts from presentation
    ...
    def _read_records(self):
        ...
        if self.records is None:
            ...
            for rec in self.raw_records:
                self.records.append(
                    LexiconEntrySubClassMapping.instance_from_record_factory(rec)
                )
...
class LexiconLookupAggregator(object):
    ...
    def lexicon_lookup(cls, input_str, **kwargs):
        ...
        if not results or kwargs.get("always_consonants", False):
            results += cls._single_lookup(
                strip_cantillation(input_str, True), lookup_key="c_form", **kwargs
            )
        if not kwargs.get("never_split", None) and (
            len(results) == 0 or kwargs.get("always_split", None)
        ):
        ...
        if len(results):
            ...
            query = (
                set()
            )  # TODO: optimize number of word form lookups? there can be a lot of duplicates... is it needed?
            for r in results:
