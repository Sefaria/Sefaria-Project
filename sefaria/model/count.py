...
class Count(abst.AbstractMongoRecord):
    ...
    required_attrs = ["textComplete", "percentAvailable", "availableCounts"]
    optional_attrs = [
        "categories",
        "availableTexts",
        "title",
    ...
    def _set_derived_attributes(self):
        ...
        if getattr(self, "title", None):
            try:
                ...
                logger.warning(
                    "Count object failed to get Index for {} : {} Normal right after Index name change.".format(
                        self.title, e
                    )
                )
    ...
    def section_length(self, section_numbers):
        ...
        """
        return self._allVersionCountsJA.sub_array_length(
            [s - 1 for s in section_numbers]
        )
