...
class RecommendationSource:
    ...
    def __str__(self):
        return "RecommendationSource: {}, {}".format(
            self.source, self.anchor_ref.normal()
        )

    def __repr__(self):
        return "{}({}, {})".format(
            self.__class__.__name__, self.source, self.anchor_ref
        ).encode("utf-8")
    ...
    def to_dict(self):
        return {"source": self.source, "anchor_ref": self.anchor_ref.normal()}


class Recommendation:
    def __init__(
        self, oref=None, relevance=0.0, score=None, novelty=None, sources=None
    ):
        self.ref = oref
    ...
    def __add__(self, other):
        ...
        return Recommendation(
            new_ref,
            relevance=self.relevance + other.relevance,
    ...
    def __str__(self):
        return "Recommendation: {}, {}, {}".format(
            self.ref.normal(), self.score, self.sources
        )

    def __repr__(self):
        return "{}({}, score={}, sources={})".format(
            self.__class__.__name__, self.ref, self.score, self.sources
        ).encode("utf-8")
    ...
    def score(self):
        if self._score is None:
            ...
            self.novelty = (
                ref_data.inverse_pagesheetrank() if ref_data is not None else 1.0
            )
    ...
    def sources_interesting(self):
        ...
        filt = [
            ...
            if (x.source.startswith("Sheet ") or x.source == "direct")
        ]
        return len(filt) >= 2
...
class RecommendationEngine:
    def __init__(
        ...
        tref,
        top=10,
        exclude_direct_commentary=True,
        limit_to_direct_link=False,
        cluster_max_dist=5,
    ):
        """
    ...
    def get_all_possible_recs(self):
        ...
        (
            link_recs,
            commentary_link_ref_set,
            direct_link_ref_set,
        ) = self.get_recs_thru_links(self.ref)
        ...
        def combine_cluster(cluster):
            if len(cluster) == 1:
                ...
                if cluster[0]["data"].ref.primary_category in ("Tanakh", "Talmud"):
                    ...
                    return Recommendation(
                        ranged_ref, score=max(scores), sources=list(set(sources))
                    )
                else:
                    argmax = max(
                        list(range(len(scores))), key=lambda i: scores[i]
                    )  # see here for this semi-readable hack for argmax() https://towardsdatascience.com/there-is-no-argmax-function-for-python-list-cd0659b05e49
                    return cluster[argmax]["data"]

        ref_list = [rec.ref for rec in self.recommendations]
        clusters = self.cluster_close_refs(
            ref_list, self.recommendations, self.cluster_max_dist
        )
    ...
    def get_recs_thru_links(oref):
        ...
        for section_ref in section_ref_list:
            ...
            filtered_links = [
                ...
                if len(
                    range_set & {r.normal() for r in Ref(l["anchorRef"]).range_list()}
                )
                > 0
            ]
            direct_links |= {
                (
                    l["ref"],
                    l["category"] in ("Commentary", "Modern Commentary"),
                    Ref(l["anchorRef"]),
                )
        ...
        for link_tref, is_comment, anchor_ref in direct_links:
            ...
            if is_comment and not link_tref.startswith("Steinsaltz on "):
                ...
                (
                    temp_commentary_links,
                    _,
                    _,
                    _,
                ) = RecommendationEngine.normalize_related_refs(
                    [x["ref"] for x in get_links(link_tref, with_text=False)],
                    None,
                    COMMENTARY_LINK_SCORE,
                )
                for commentary_link in temp_commentary_links:
                    if (
                        author is not None
                        and (commentary_link, author) in commentary_author_set
                    ):
                    ...
                    commentary_links += [
                        Recommendation(
                            ...
                            sources=[RecommendationSource(link_tref, anchor_ref)],
                        )
                    ]
        ...
        (
            ...
            [x[0] for x in direct_links], None, DIRECT_LINK_SCORE, other_data=other_data
        )
        ...
        final_rex = [
            Recommendation(
                Ref(x),
                relevance=DIRECT_LINK_SCORE,
                sources=[RecommendationSource("direct", anchor_ref)],
            )
            for x, anchor_ref in zip(direct_links, anchor_ref_list)
        ] + commentary_links
        commentary_ref_set = set(
            [x[0] for x in [x for x in zip(direct_links, is_comment_list) if x[1]]]
        )
    ...
    def get_recs_thru_sheets(oref):
        ...
        for section_ref in section_ref_list:
            ...
            sheets_cursor = db.sheets.find(
                query, {"includedRefs": 1, "owner": 1, "id": 1, "tags": 1, "title": 1}
            )
            sheets += [
                s for s in sheets_cursor if RecommendationEngine.is_interesting_sheet(s)
            ]
        included_ref_dict = {}
        for sheet in sheets:
            (
                ...
                sheet.get("includedRefs", []),
                range_set,
                SHEET_REF_SCORE,
                check_has_ref=True,
                count_steinsaltz=True,
            )
            ...
            for k in ref_owner_keys:
                if (
                    k in included_ref_dict
                    and included_ref_dict[k]["score"] < focus_range_factor
                ) or k not in included_ref_dict:
                    included_ref_dict[k] = {
                        "score": focus_range_factor,
                        "source": "Sheet " + str(sheet["id"]),
                        "anchor_ref": focus_ref_subref,
                    }
        ...
        return [
            Recommendation(
                Ref(r),
                relevance=d["score"],
                sources=[RecommendationSource(d["source"], d["anchor_ref"])],
            )
            for (r, _), d in list(included_ref_dict.items())
        ]
    ...
    def cluster_close_refs(ref_list, data_list, dist_threshold):
        ...
        for temp_oref, temp_data in item_list:
            ...
            if last_cluster is None or (
                not (-1 < last_cluster[-1]["ref"].distance(temp_oref) <= dist_threshold)
            ):
    ...
    def includes_section(oref):
        ...
        if oref.is_section_level():
            return True
        if (
            isinstance(oref.index_node, JaggedArrayNode)
            and oref.index.schema.get("depth", 0) == 2
        ):
            # doesn't work for dictionary entries and not relevant anyway
            range_set = {r.normal() for r in oref.all_segment_refs()}
            section_range_set = {
                r.normal()
                for r in Ref(next(iter(range_set))).section_ref().all_segment_refs()
            }
    ...
    def normalize_related_refs(
        ...
        focus_range_factor = (
            0.0  # multiplicative factor based on how big a range the focus_ref is in
        )
        final_refs = []
        ...
        for temp_tref, other_data_item in zip(related_refs, other_data):
            ...
            if temp_oref.is_range():
                ...
                in_common_set = (
                    set() if focus_ref_set is None else temp_range_set & focus_ref_set
                )
                if len(in_common_set) > 0:
                    temp_focus_range_factor = (
                        (len(in_common_set) * base_score) / len(temp_range_set)
                        if len(temp_range_set) < REF_RANGE_MAX
                        else 0.0
                    )
                    if temp_focus_range_factor > focus_range_factor:
                        focus_range_factor = temp_focus_range_factor
                    if focus_ref_subset is None or len(in_common_set) < len(
                        focus_ref_subset
                    ):
        ...
        if count_steinsaltz:
            ...
            filter_result = [
                x
                for x in zip(final_refs, final_other_data)
                if "Steinsaltz on " not in x[0]
            ]
            final_refs, final_other_data = reduce(
                lambda a, b: [a[0] + [b[0]], a[1] + [b[1]]], filter_result, [[], []]
            )

        focus_ref_subref = None
        if focus_ref_subset is not None:
            focus_ref_subref_list = sorted(
                [Ref(x) for x in list(focus_ref_subset)], key=lambda x: x.order_id()
            )
