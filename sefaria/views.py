...
def index_bulk_edit_api(request):
    ...
    # Validate all indices exist before making any changes
    for t in titles:
    ...
    for t in titles:
        try:
            ...
            for k, v in updates.items():
                # Handle nested keys (only allowed under dependence.*)
                if "." in k:
                    parent, child = k.split(".", 1)
                    if parent != "dependence":
                        return jsonResponse({"error": f'Nested key not allowed: {k}'})
                    if not hasattr(idx, 'dependence') or idx.dependence is None:
                        idx.dependence = {}
                    idx.dependence[child] = v
                else:
                    # Special handling for different field types
                    if k == "authors":
                        # Authors should be a list of strings (author names)
                        # If it's a string, assume comma-separated
                        if isinstance(v, str):
                            v = [author.strip() for author in v.split(",") if author.strip()]
                        elif not isinstance(v, list):
                            errors.append(f"Invalid authors format for {t}")
                            continue
                    
                    elif k in ["altTitles", "heAltTitles"]:
                        # Alternative titles should be lists
                        if isinstance(v, str):
                            v = [title.strip() for title in v.split(",") if title.strip()]
                        elif not isinstance(v, list):
                            errors.append(f"Invalid {k} format for {t}")
                            continue
                    
                    elif k in ["compDate", "pubDate"]:
                        # Date fields can be:
                        # - Single year (int): 1040
                        # - Year range (list): [1040, 1105]
                        # - String that can be parsed to int or list
                        if isinstance(v, str):
                            v = v.strip()
                            if v.startswith('[') and v.endswith(']'):
                                ...
                                try:
                                    ...
                                    if not isinstance(v, list) or len(v) != 2:
                                        errors.append(f"Date range must have exactly 2 elements for {t}")
                                        continue
                                    # Validate both elements are integers
                                    v = [int(v[0]), int(v[1])]
                                except (json.JSONDecodeError, ValueError):
                                    errors.append(f"Invalid date range format for {t}: {v}")
                                    continue
                            else:
                                # Parse as single year
                                try:
                                    v = int(v)
                                except ValueError:
                                    errors.append(f"Invalid date format for {t}: {v}")
                                    continue
                            ...
                            try:
                                ...
                                errors.append(f"Date range must contain integers for {t}")
                                continue
                        elif not isinstance(v, int):
                            errors.append(f"Invalid date type for {t}")
                            continue
                    
                    elif k == "category":
                        # Category should be a valid category path
                        # The frontend sends it as comma-separated, we keep it that way
                        if not isinstance(v, str):
                            errors.append(f"Category must be a string for {t}")
                            continue
            ...
            errors.append(f"Error updating {t}: {str(e)}")
