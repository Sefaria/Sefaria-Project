...
class SheetStats(object):
    ...
    def count_sheet(self, sheet):
        ...
        self.count_sources(
            sheet.get("sources", []), sheet.get("tags", []), sheet.get("id", -1)
        )
    ...
    def sort(self):
        ...
        self.sorted_categories = sorted(
            iter(self.categories.items()), key=lambda x: -x[1]
        )
        self.sorted_books = sorted(iter(self.books.items()), key=lambda x: -x[1])
        self.sorted_untrans_refs = sorted(
            iter(self.untrans_refs.items()), key=lambda x: -x[1]
        )
        self.sorted_untrans_texts = sorted(
            iter(self.untrans_texts.items()), key=lambda x: -x[1]
        )
        self.sorted_untrans_categories = sorted(
            iter(self.untrans_categories.items()), key=lambda x: -x[1]
        )
        self.sorted_fragments = sorted(
            iter(self.fragments.items()), key=lambda x: -len(x[1])
        )

        self.sorted_refs_by_tag = {}
        for ref in self.refs_by_tag:
            self.sorted_refs_by_tag[ref] = sorted(
                iter(self.refs_by_tag[ref].items()), key=lambda x: -x[1]
            )

        self.sorted_refs_by_category = {}
        for ref in self.refs_by_category:
            self.sorted_refs_by_category[ref] = sorted(
                iter(self.refs_by_category[ref].items()), key=lambda x: -x[1]
            )
    ...
    def print_stats(self):
        ...
        print(
            "%0.1f%% Bilingual"
            % (100 * self.languages["bilingual"] / float(self.total))
        )
        ...
        print("\n")
        print(
            "%d Potential Fragments (translations in sheets not saved in DB)"
    ...
    def save_top_sources_sheet(self):
        sheet = {
            "title": "Top Sources in All Source Sheets - %s"
    ...
    def save_top_sources_by_category(self):
        sheet = {
            ...
            "sources": [
                {"ref": self.sorted_refs_by_category[cat][0][0], "title": cat}
                for cat in self.sorted_refs_by_category
            ],
    ...
    def save_top_sources_by_tag(self):
        sheet = {
            ...
            "sources": [
                {"ref": self.sorted_refs_by_tag[tag["tag"]][0][0], "title": tag["tag"]}
                for tag in self.top_tags[:50]
            ],
            "options": {"numbered": 0, "divineNames": "noSub"},
        }
    ...
    def save_top_for_category(self, cat, collapse=False):
        ...
        sources = [
            {
                "comment": "Most frequently used tractates (full list below):<br>%s"
                % top_books
            }
        ]
...
def total_sheet_views_by_query(query):
    ...
    result = db.sheets.aggregate(
        [{"$match": query}, {"$group": {"_id": None, "total": {"$sum": "$views"}}}]
    )
...
def account_creation_stats():
    ...
    users = list(
        User.objects.all().values(
            "date_joined", "email", "first_name", "last_name", "id", "last_login"
        )
    )

    df = pd.DataFrame(users)
    month_joined = (
        df["date_joined"].groupby(df.date_joined.dt.to_period("M")).agg("count")
    )

    # Filter to users who have hebrew interface language, proxy for Israel
    hebrew_user_ids = db.profiles.find(
        {"settings.interface_language": "hebrew"}, {"id": 1, "settings": 1}
    ).distinct("id")
    hebrew_users = [user for user in users if user["id"] in hebrew_user_ids]
    df_hebrew = pd.DataFrame(hebrew_users)
    hebrew_month_joined = (
        df_hebrew["date_joined"]
        .groupby(df_hebrew.date_joined.dt.to_period("M"))
        .agg("count")
    )
...
def user_activity_stats(query={}, return_string=False):
        ...
        [
            {"$match": query},
            {
                "$project": {
                    ...
                    "uid": 1,
                    "date": {
                        "$dateToString": {"date": "$datetime", "format": "%Y-%m"}
                    },  # "year": {"$year": "$datetime"},
                }
            },
            {"$group": {"_id": "$date", "uids": {"$addToSet": "$uid"}}},
            {"$sort": {"_id": 1}},
        ]
    )
    ...
    for i in range(len(months)):
        ...
        returning_users = (
            len(set(months[i]["uids"]) & set(months[i - 1]["uids"])) if i != 0 else 0
        )
        months[i]["monthly retention"] = (
            int(100 * returning_users / len(months[i - 1]["uids"])) if i != 0 else 0
        )
    ...
    for month in months:
        results += "{}, {}, {}, {}, {}, {}, {}%\n".format(
            month["_id"],
            month["1 month active"],
            month["3 month active"],
            month["6 month active"],
            month["12 month active"],
            month["monthly returning"],
