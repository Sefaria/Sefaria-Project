...
def get_sheet(id=None):
    ...
    s["collections"] = [
        {"name": collection.name, "slug": collection.slug} for collection in collections
    ]
    return s
...
def get_sheet_metadata(id=None):
    ...
    s = db.sheets.find_one(
        {"id": int(id)},
        {"title": 1, "owner": 1, "summary": 1, "ownerImageUrl": 1, "via": 1},
    )
    return s
...
def get_sheet_metadata_bulk(id_list, public=True):
    ...
    return db.sheets.find(
        query,
        {"id": 1, "title": 1, "owner": 1, "summary": 1, "ownerImageUrl": 1, "via": 1},
    )
...
def get_sheet_node(sheet_id=None, node_id=None):
    ...
    s = db.sheets.find_one(
        {"id": int(sheet_id), "sources.node": int(node_id)}, {"sources.$": 1, "_id": 0}
    )
...
def public_sheets(
    sort=[["datePublished", -1]], limit=50, skip=0, lang=None, filtered=False
):
    if filtered:
    ...
    response = {"sheets": sheet_list(query=query, sort=sort, limit=limit, skip=skip)}
    return response
...
def sheet_to_dict(sheet):
    ...
    sheet_dict = {
        ...
        "tags": [
            t["asTyped"] for t in sheet.get("topics", [])
...
def trending_topics(days=7, ntags=14):
    ...
    results = add_langs_to_topics(
        [
            ...
            for topic in filter(lambda x: len(x["authors"]) > 1, topics)
        ],
        use_as_typed=False,
...
def save_sheet(sheet, user_id, search_override=False, rebuild_nodes=False):
    ...
    if "id" in sheet:
        ...
        old_media_urls = set(
            [source["media"] for source in existing.get("sources") if "media" in source]
        )
    ...
    if status_changed and not new_sheet:
        ...
        if sheet["status"] != "public":
            ...
            if SEARCH_INDEX_ON_SAVE and not search_override:
                es_index_name = search.get_new_and_current_index_names("sheet")[
                    "current"
                ]
...
def is_valid_source(source):
    if not (
        "ref" in source
        or "outsideText" in source
        or "outsideBiText" in source
        or "comment" in source
        or "media" in source
    ):
        return False
    return True
...
def bleach_text(text):
    ...
    return bleach.clean(
        text,
        tags=ok_sheet_tags,
        attributes=ok_sheet_attrs,
        styles=ok_sheet_styles,
        strip=True,
    )
    ...
    for sheet in sheets:
        ...
        db.sheets.update(
            {"_id": sheet["_id"]},
            {
                "$set": {
                    "includedRefs": refs,
                    "expandedRefs": model.Ref.expand_refs(refs),
                }
            },
        )
...
def get_sheets_for_ref(tref, uid=None, in_collection=None):
    ...
    mongo_user_profiles = list(
        db.profiles.find(
            {"id": {"$in": user_ids}}, {"id": 1, "slug": 1, "profile_pic_url_small": 1}
        )
    )
    ...
    for profile in user_profiles:
        ...
        try:
            user_profiles[profile]["profile_pic_url_small"] = mongo_user_profiles[
                profile
            ].get("profile_pic_url_small", "")
        except:
    ...
    for sheet in sheets:
        anchor_ref_list, anchor_ref_expanded_list = oref.get_all_anchor_refs(
            segment_refs, sheet.get("includedRefs", []), sheet.get("expandedRefs", [])
        )
...
def update_sheet_topics(sheet_id, topics, old_topics):
    ...
    normalized_topics = [
        {"asTyped": pair[0], "slug": pair[1]} for pair in normalized_slug_title_pairs
    ]
...
def add_langs_to_topics(
    topic_list: list, use_as_typed=True, backwards_compat_lang_fields: dict = None
) -> list:
    """
...
def public_tag_list(sort_by="alpha"):
    ...
    sort_keys = {
        ...
        "alpha-hebrew": lambda x: x["tag"]
        if len(x["tag"]) and x["tag"][0] in "אבגדהוזחטיכלמנסעפצקרשת0123456789"
        else "ת" + x["tag"],
    }
...
def make_sheet_from_text(
    text, sources=None, uid=1, generatedBy=None, title=None, segment_level=False
):
    """
    ...
    sheet = {
        ...
        + " with "
        + ", ".join([s.replace(" on " + text, "") for s in sources]),
        "sources": [],
        "status": 0,
    ...
    for leaf in leafs:
        ...
        if leaf.first_section_ref() != leaf.last_section_ref():
            ...
            if segment_level:
                refs += [
                    ref
                    for ref in leaf_spanning_ref.all_segment_refs()
                    if oref.contains(ref)
                ]
            else:  # section level
                refs += [
                    ref
                    for ref in leaf_spanning_ref.split_spanning_ref()
                    if oref.contains(ref)
                ]
        else:
...
def change_tag(old_tag, new_tag_or_list):
    ...
    new_tag_list = (
        [new_tag_or_list] if isinstance(new_tag_or_list, str) else new_tag_or_list
    )
...
def get_sheet_categorization_info(find_without, skip_ids=[]):
            ...
            [
                {
                    "$match": {
                        "categories": {"$in": [None, []]},
                        "sources.outsideText": {"$exists": True},
                        "id": {"$nin": skip_ids},
                        "noTags": {"$in": [None, False]},
                        "status": "public",
                    }
    ...
    categories_all = list(
        filter(lambda x: x != None, db.sheets.distinct("categories"))
    )  # this is slow; maybe add index or ...?
    categorize_props = {
...
def update_sheet_tags_categories(body, uid):
    ...
    db.sheets.update_one(
        {"id": body["sheetId"]},
        {
            "$set": {"categories": body["categories"], "noTags": noTags},
            "$push": {"moderators": {"uid": uid, "time": time}},
        },
    )
