...
def get_sheet(id=None):
    ...
    s["collections"] = [{"name": collection.name,"slug": collection.slug} for
...
def get_sheet_metadata(id=None):
    ...
    s = db.sheets.find_one(
        {"id": int(id)},{"title": 1,"owner": 1,"summary": 1,"ownerImageUrl": 1,
                         "via": 1}
        )
    return s
...
def get_sheet_metadata_bulk(id_list,public=True):
    ...
    return db.sheets.find(
        query,{"id": 1,"title": 1,"owner": 1,"summary": 1,"ownerImageUrl": 1,
               "via": 1}
        )
...
def get_sheet_node(sheet_id=None,node_id=None):
    ...
    if not s:
        return {
            "error": "Couldn't find node with sheet id: %s and node id: %s" % (
            sheet_id,node_id)}
    return s["sources"][0]
...
def sheet_to_dict(sheet):
    ...
    sheet_dict = {
        ...
        "title": strip_tags(
            sheet["title"]
            ) if "title" in sheet else "Untitled Sheet",
        ...
        "modified": dateutil.parser.parse(sheet["dateModified"]).strftime(
            "%m/%d/%Y"
            ),
...
def add_sheet_to_collection(sheet_id,collection,is_sheet_owner,
                            override_displayedCollection=False):
    ...
    if sheet["id"] not in collection.sheets:
        ...
        if is_sheet_owner and (not sheet.get(
                "displayedCollection",None
                ) or override_displayedCollection):
...
def trending_topics(days=7,ntags=14):
    ...
    topics = db.sheets.aggregate(
        [
            {"$match": query},
            {"$unwind": "$topics"},
            {"$group": {"_id": "$topics.slug","sheet_count": {"$sum": 1},
                        "authors": {"$addToSet": "$owner"}}},
            {"$project": {"_id": 0,"slug": "$_id",
                          "sheet_count": "$sheet_count",
                          "authors": "$authors"}}],cursor={}
    )
    ...
    results = add_langs_to_topics(
        ...
        } for topic in filter(lambda x: len(x["authors"]) > 1,topics_list)],
        use_as_typed=False,
        backwards_compat_lang_fields={'en': 'tag','he': 'he_tag'}
    )
...
def save_sheet(sheet,user_id,search_override=False,rebuild_nodes=False):
    ...
    if "id" in sheet:
        ...
        old_media_urls = set(
            [source["media"] for source in existing.get("sources") if
             "media" in source]
            )
        if len(old_media_urls) > 0:
            new_media_urls = set(
                [source["media"] for source in sheet.get("sources") if
                 "media" in source]
                )
                ...
                for url in deleted_media:
                    if url.startswith(GoogleStorageManager.BASE_URL):
                        GoogleStorageManager.delete_filename(
                            (re.findall(r"/([^/]+)$",url)[0]),
                            GoogleStorageManager.UGC_SHEET_BUCKET
                            )
        ...
        for source in sheet["sources"]:
            if "node" not in source:
                ...
                nextNode += 1
            if "media" in source and source["media"].startswith(
                ...
                duped_image_url = GoogleStorageManager.duplicate_file(
                    old_file,to_file,bucket_name
                    )
    ...
    if status_changed and not new_sheet:
        ...
        if sheet["status"] != "public":
            ...
            if SEARCH_INDEX_ON_SAVE and not search_override:
                es_index_name = \
                search.get_new_and_current_index_names("sheet")['current']
    ...
    if sheet[
        ...
        try:
            index_name = search.get_new_and_current_index_names("sheet")[
                'current']
...
def is_valid_source(source):
    if not (
            "ref" in source or "outsideText" in source or "outsideBiText" in source or "comment" in source or "media" in source):
        return False
    return True


def bleach_text(text):
    ok_sheet_tags = ['blockquote','p','a','ul','ol','nl','li','b','i','strong',
                     'em','small','big','span','strike',
                     'hr','br','div','table','thead','caption','tbody','tr',
                     'th','td','pre','sup','u','h1']

    ok_sheet_attrs = {'a': ['href','name','target','data-ref'],'img': ['src'],
    ...
    return bleach.clean(
        text,tags=ok_sheet_tags,attributes=ok_sheet_attrs,
        styles=ok_sheet_styles,strip=True
        )
...
def clean_source(source):
    if "ref" in source:
        ...
        source["outsideBiText"]["he"] = bleach_text(
            source["outsideBiText"]["he"]
            )
        source["outsideBiText"]["en"] = bleach_text(
            source["outsideBiText"]["en"]
            )
...
def test():
    ...
    for s in ss:
        ...
        if lang == "some hebrew":
            print(
                "{}\thttps://www.sefaria.org/sheets/{}".format(
...
def add_source_to_sheet(id,source,note=None):
    ...
    if note:
        sheet["sources"].append(
            {"outsideText": note,"options": {"indented": "indented-1"}}
            )
    db.sheets.replace_one({"_id": sheet["_id"]},sheet,upsert=True)
...
def add_ref_to_sheet(id,ref,request):
    ...
    if (sheet["owner"] != request.user.id):
        return jsonResponse(
            {"error": "user can only add refs to their own sheet"}
            )
    sheet["dateModified"] = datetime.now().isoformat()
...
def update_included_refs(query=None,hours=None,refine_refs=False):
    ...
    for sheet in sheets:
        ...
        db.sheets.update(
            {"_id": sheet["_id"]},{"$set": {"includedRefs": refs,
                                            "expandedRefs": model.Ref.expand_refs(
                                                refs
                                                )}}
            )
...
def extract_topic_slugs(base_ref_str: str,seg_refs: list[str]) -> list[str]:
    """
    Return every topic slug linked to *any* of the segment refs
    (or to the base ref itself).  Works for pasuk or chapter.
    """
    cursor = db.topic_links.find(
        {"class": "refTopic",
         "$or": [
             {"ref": {"$in": seg_refs + [base_ref_str]}},
             {"expandedRefs": {"$in": seg_refs + [base_ref_str]}}
         ]},
        {"toTopic": 1}
    )
    return list({d["toTopic"] for d in cursor if "toTopic" in d})


def calculate_combined_score(sheets: list,topic_slugs: list[str],
                             t_weight: float = 0.6,
                             c_weight: float = 0.4) -> list:
    """
    Calculate combined score for sheets based on topic relevance and creativity.
    Returns sheets with added combined_score field.
    """
    # Calculate topic scores for each sheet
    for sheet in sheets:
        topic_score = 0
        topic_scores = sheet.get("topic_scores",{})

        # Sum up scores for relevant topics
        for slug in topic_slugs:
            if slug in topic_scores:
                topic_score += topic_scores[slug]

        sheet["topic_score"] = topic_score

    # Filter sheets that have topic relevance and valid creativity scores
    relevant_sheets = []
    for sheet in sheets:
        creativity_score = sheet.get("creativity_score",0)
        topic_score = sheet.get("topic_score",0)

        # Only include sheets with topic relevance and valid creativity score
        if topic_score > 0 and isinstance(creativity_score,(int,float)):
            relevant_sheets.append(sheet)

    if not relevant_sheets:
        # If no relevant sheets, return original sheets with 0 combined_score
        for sheet in sheets:
            sheet["combined_score"] = 0
        return sheets

    # Calculate normalization factors
    topic_scores = [sheet["topic_score"] for sheet in relevant_sheets]
    creativity_scores = [sheet.get("creativity_score",0) for sheet in
                         relevant_sheets]

    t_max = max(topic_scores) if topic_scores else 1
    c_max = max(creativity_scores) if creativity_scores else 1
    c_min = min(creativity_scores) if creativity_scores else 0

    # Normalize and calculate combined scores
    for sheet in relevant_sheets:
        # Normalize topic score (0-1)
        topic_norm = sheet["topic_score"] / t_max if t_max > 0 else 0

        # Normalize creativity score (0-1)
        creativity_score = sheet.get("creativity_score",0)
        if c_max > c_min:
            creativity_norm = (creativity_score - c_min) / (c_max - c_min)
        else:
            creativity_norm = 0

        # Calculate weighted combined score
        combined_score = (topic_norm * t_weight) + (creativity_norm * c_weight)
        sheet["combined_score"] = combined_score

    # Set combined_score to 0 for sheets not in relevant_sheets
    relevant_sheet_ids = {sheet["id"] for sheet in relevant_sheets}
    for sheet in sheets:
        if sheet["id"] not in relevant_sheet_ids:
            sheet["combined_score"] = 0

    return sheets


def get_sheets_for_ref(tref,uid=None,in_collection=None,t_weight=0.6,
                       c_weight=0.4):
    """
    Returns a list of sheets that include ref, formatted as needed for the Client Sidebar.
    ...
    Now includes combined_score field based on topic relevance and creativity.
    """
    oref = model.Ref(tref)
    ...
    topic_slugs = extract_topic_slugs(oref.normal(),segment_refs)
    ...
    # Updated projection to include topic_scores and creativity_score for scoring
    sheetsObj = db.sheets.find(
        query,
        {"id": 1,"title": 1,"owner": 1,"viaOwner": 1,"via": 1,"dateCreated": 1,
         "includedRefs": 1,"expandedRefs": 1,"views": 1,"topics": 1,
         "status": 1,"summary": 1,"attribution": 1,"assigner_id": 1,"likes": 1,
         "displayedCollection": 1,"options": 1,"topic_scores": 1,
         "creativity_score": 1}
    ).sort([["views",-1]])
    ...
    sheets = calculate_combined_score(sheets,topic_slugs,t_weight,c_weight)
    ...
    mongo_user_profiles = list(
        db.profiles.find(
            {"id": {"$in": user_ids}},
            {"id": 1,"slug": 1,"profile_pic_url_small": 1}
        )
    )
    ...
    for profile in user_profiles:
        ...
        try:
            user_profiles[profile]["profile_pic_url_small"] = \
                mongo_user_profiles[profile].get("profile_pic_url_small",'')
        except:
            user_profiles[profile]["profile_pic_url_small"] = ""
    ...
    for sheet in sheets:
        ...
        ownerData = user_profiles.get(
            sheet["owner"],{'first_name': 'Ploni','last_name': 'Almoni',
                            'email': 'test@sefaria.org','slug': 'Ploni-Almoni',
                            'id': None,'profile_pic_url_small': ''}
        )
        ...
        for anchor_ref,anchor_ref_expanded in zip(
            ...
            sheet_data = {
                ...
                "ownerName": ownerData["first_name"] + " " + ownerData[
                    "last_name"],
                ...
                # Add combined_score to output
                # Optional: include component scores for debugging
                "topic_score": sheet.get("topic_score",0),
                "creativity_score": sheet.get("creativity_score",0),
            }
...
def update_sheet_topics(sheet_id,topics,old_topics):
    ...
    normalized_topics = [{"asTyped": pair[0],"slug": pair[1]} for pair in
                         normalized_slug_title_pairs]

    db.sheets.update_one(
        {"id": sheet_id},{"$set": {"topics": normalized_topics}}
        )
...
def choose_existing_topic_for_title(title):
    ...
    def is_title_primary(title,topic):
        all_primary_titles = [topic.get_primary_title(lang) for lang in
                              topic.title_group.langs]
...
def update_sheet_topic_links(sheet_id,new_topics,old_topics):
    ...
    status = db.sheets.find_one({"id": sheet_id},{"status": 1}).get(
        "status","unlisted"
        )
    if status != "public":
        return
...
def add_langs_to_topics(topic_list: list,use_as_typed=True,
                        backwards_compat_lang_fields: dict = None) -> list:
    """
    ...
    if len(topic_list) > 0:
        for topic in topic_list:
            ...
            default_title = topic['asTyped'] if use_as_typed else topic[
                'slug'].replace("-"," ").title()
            topic_titles = topic_map.get(
                topic['slug'],{"en": default_title,"he": default_title}
                )
            ...
            if backwards_compat_lang_fields is not None:
                for lang in ('en','he'):
                    new_topic[backwards_compat_lang_fields[lang]] = new_topic[
                        lang]
...
def public_tag_list(sort_by="alpha"):
    ...
    sort_keys = {
        ...
        "alpha-hebrew": lambda x: x["tag"] if len(x["tag"]) and x["tag"][
            0] in "אבגדהוזחטיכלמנסעפצקרשת0123456789" else "ת" + x["tag"],
    }
...
def get_sheets_by_topic(topic,public=True,proj=None,limit=0,page=0):
    ...
    sheets = db.sheets.find(query,proj).sort([["views",-1]]).limit(limit).skip(
        page * limit
        )
    return sheets
...
def add_visual_data(sheet_id,visualNodes,zoom):
	...
	"""
    db.sheets.update(
        {"id": sheet_id},{"$unset": {"visualNodes": "","zoom": ""}}
        )
    db.sheets.update(
        {"id": sheet_id},{
            "$push": {"visualNodes": {"$each": visualNodes},"zoom": zoom}}
        )
...
def make_sheet_from_text(text,sources=None,uid=1,generatedBy=None,title=None,
    ...
    oref = model.Ref(text)
    sheet = {
        "title": title if title else oref.normal() if not sources else oref.normal() + " with " + ", ".join(
            [s.replace(" on " + text,"") for s in sources]
            ),
        "sources": [],
        "status": 0,
    ...
    for leaf in leafs:
        ...
        if leaf.first_section_ref() != leaf.last_section_ref():
            leaf_spanning_ref = leaf.first_section_ref().to(
                leaf.last_section_ref()
                )
            assert isinstance(leaf_spanning_ref,model.Ref)
            if segment_level:
                refs += [ref for ref in leaf_spanning_ref.all_segment_refs() if
                         oref.contains(ref)]
            else:  # section level
                refs += [ref for ref in leaf_spanning_ref.split_spanning_ref()
                         if oref.contains(ref)]
        else:
