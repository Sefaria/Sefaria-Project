...
from sefaria.forms import (
    SefariaLoginForm,
    SefariaPasswordResetForm,
...
urlpatterns = [
    ...
    url(
        r"^search-autocomplete-redirecter/?$",
        reader_views.search_autocomplete_redirecter,
    ),
...
urlpatterns += [
    url(
        r"^explore(-(?P<topCat>[\w-]+)-and-(?P<bottomCat>[\w-]+))?(/(?P<book1>[A-Za-z-,\']+))?(/(?P<book2>[A-Za-z-,\']+))?(/(?P<lang>\w\w)/?)?/?$",
        reader_views.explore,
    ),
    url(
        r"^visualize/library/(?P<lang>[enh]*)/?(?P<cats>.*)/?$",
        reader_views.visualize_library,
    ),
...
urlpatterns += [
    ...
    url(
        r"^interface/(?P<language>english|hebrew)$",
        reader_views.interface_language_redirect,
    ),
    ...
    url(
        r"^api/profile/(?P<slug>[^/]+)/(?P<ftype>followers|following)$",
        reader_views.profile_follow_api,
    ),
    url(r"^api/user_history/saved$", reader_views.saved_history_for_ref),
    url(
        r"^api/interrupting-messages/read/(?P<message>.+)$",
        reader_views.interrupting_messages_read_api,
    ),
]
...
urlpatterns += [
    ...
    url(
        r"^contributors/(?P<username>[^/]+)(/(?P<page>\d+))?$",
        reader_views.profile_redirect,
    ),
]
...
urlpatterns += [
    ...
    url(
        r"^api/texts/version-status/tree/?(?P<lang>.*)?/?$",
        reader_views.version_status_tree_api,
    ),
    ...
    url(
        r"^api/texts/(?P<tref>.+)/(?P<lang>\w\w)/(?P<version>.+)$",
        reader_views.old_text_versions_api_redirect,
    ),
    ...
    url(
        r"^api/counts/words/(?P<title>.+)/(?P<version>.+)/(?P<language>.+)$",
        reader_views.word_count_api,
    ),
    ...
    url(
        r"^api/calendars/next-read/(?P<parasha>.+)$", reader_views.parasha_next_read_api
    ),
    ...
    url(
        r"^api/words/completion/(?P<word>.+)/(?P<lexicon>.+)$",
        reader_views.dictionary_completion_api,
    ),
    url(
        r"^api/words/completion/(?P<word>.+)$", reader_views.dictionary_completion_api
    ),  # Search all dicts
...
urlpatterns += [
    ...
    url(
        r"^api/sheets/(?P<parasha>.+)/get_aliyot$",
        sheets_views.get_aliyot_by_parasha_api,
    ),
    url(
        r"^api/sheets/(?P<sheet_id>\d+)/copy_source$",
        sheets_views.copy_source_to_sheet_api,
    ),
    ...
    url(
        r"^api/sheets/(?P<sheet_id>\d+)\.(?P<node_id>\d+)$", sheets_views.sheet_node_api
    ),
    ...
    url(
        r"^api/sheets/user/(?P<user_id>\d+)/((?P<sort_by>\w+)/(?P<limiter>\d+)/(?P<offset>\d+))?$",
        sheets_views.user_sheet_list_api,
    ),
    url(
        r"^api/sheets/modified/(?P<sheet_id>\d+)/(?P<timestamp>.+)$",
        sheets_views.check_sheet_modified_api,
    ),
    url(
        r"^api/sheets/create/(?P<ref>[^/]+)(/(?P<sources>.+))?$",
        sheets_views.make_sheet_from_text_api,
    ),
    ...
    url(
        r"^api/sheets/tag-list/user/(?P<user_id>\d+)?$", sheets_views.user_tag_list_api
    ),
    ...
    url(
        r"^api/sheets/all-sheets/(?P<limiter>\d+)/(?P<offset>\d+)$",
        sheets_views.all_sheets_api,
    ),
    url(
        r"^api/sheets/(?P<sheet_id>\d+)/export_to_drive$", sheets_views.export_to_drive
    ),
...
urlpatterns += [
    url(
        r"^api/collections/user-collections/(?P<user_id>\d+)$",
        sheets_views.user_collections_api,
    ),
    url(
        r"^api/collections/for-sheet/(?P<sheet_id>\d+)$",
        sheets_views.collections_for_sheet_api,
    ),
    url(r"^api/collections(/(?P<slug>[^/]+))?$", sheets_views.collections_api),
    url(
        r"^api/collections/(?P<slug>[^/]+)/set-role/(?P<uid>\d+)/(?P<role>[^/]+)$",
        sheets_views.collections_role_api,
    ),
    url(
        r"^api/collections/(?P<slug>[^/]+)/invite/(?P<uid_or_email>[^/]+)(?P<uninvite>\/uninvite)?$",
        sheets_views.collections_invite_api,
    ),
    url(
        r"^api/collections/(?P<slug>[^/]+)/(?P<action>(add|remove))/(?P<sheet_id>\d+)",
    ...
    url(
        r"^api/collections/(?P<slug>[^/]+)/pin-sheet/(?P<sheet_id>\d+)",
        sheets_views.collections_pin_sheet_api,
    ),
]
...
urlpatterns += [
    ...
    url(
        r"^api/search-path-filter/(?P<book_title>.+)$", reader_views.search_path_filter
    ),
]
...
urlpatterns += [
    ...
    url(
        r"^api/(?P<kind>(followers|followees))/(?P<uid>\d+)$",
        reader_views.follow_list_api,
    ),
]
...
urlpatterns += [
    ...
    url(
        r"^api/recommend/topics(/(?P<ref_list>.+))?", reader_views.recommend_topics_api
    ),
]
...
urlpatterns += [
    url(
        r"^api/history/(?P<tref>.+)/(?P<lang>\w\w)/(?P<version>.+)$",
        reader_views.texts_history_api,
    ),
...
urlpatterns += [
    url(
        r"^api/locks/set/(?P<tref>.+)/(?P<lang>\w\w)/(?P<version>.+)$",
        reader_views.set_lock_api,
    ),
    url(
        r"^api/locks/release/(?P<tref>.+)/(?P<lang>\w\w)/(?P<version>.+)$",
        reader_views.release_lock_api,
    ),
    url(
        r"^api/locks/check/(?P<tref>.+)/(?P<lang>\w\w)/(?P<version>.+)$",
        reader_views.check_lock_api,
    ),
]
...
urlpatterns += [
    url(
        r"^api/locktext/(?P<title>.+)/(?P<lang>\w\w)/(?P<version>.+)$",
        reader_views.lock_text_api,
    ),
    url(
        r"^api/version/flags/(?P<title>.+)/(?P<lang>\w\w)/(?P<version>.+)$",
        reader_views.flag_text_api,
    ),
]
...
urlpatterns += [
    ...
    url(
        r"^activity/(?P<tref>[^/]+)/(?P<lang>.{2})/(?P<version>.+)/(?P<page>\d+)$",
        reader_views.segment_history,
    ),
    url(
        r"^activity/(?P<tref>[^/]+)/(?P<lang>.{2})/(?P<version>.+)$",
        reader_views.segment_history,
    ),
    url(
        r"^api/revert/(?P<tref>[^/]+)/(?P<lang>.{2})/(?P<version>.+)/(?P<revision>\d+)$",
        reader_views.revert_api,
    ),
]
...
urlpatterns += [
    url(
        r"^login/?$",
        django_auth_views.LoginView.as_view(authentication_form=SefariaLoginForm),
        name="login",
    ),
...
urlpatterns += [
    url(
        r"^compare/?((?P<comp_ref>[^/]+)/)?((?P<lang>en|he)/)?((?P<v1>[^/]+)/)?(?P<v2>[^/]+)?$",
        sefaria_views.compare,
    )
]
...
urlpatterns += [
    ...
    url(
        r"^vgarden/custom/(?P<key>.*)$", reader_views.custom_visual_garden_page
    ),  # legacy.  Used for "maggid" and "ecology"
]

# Sefaria.js -- Packaged JavaScript
urlpatterns += [
    url(
        r"^data\.(?:(?:\d+)\.)?js$", sefaria_views.data_js
    ),  # Allow for regular data.js and also data.<timestamp>.js for caching
...
urlpatterns += [
    ...
    url(
        r"^download/version/(?P<title>.+) - (?P<lang>[he][en]) - (?P<versionTitle>.+)\.(?P<format>plain\.txt)",
        sefaria_views.text_download_api,
    ),
    url(
        r"^download/version/(?P<title>.+) - (?P<lang>[he][en]) - (?P<versionTitle>.+)\.(?P<format>json|csv|txt)",
        sefaria_views.text_download_api,
    ),
...
urlpatterns += [
    url(
        r"^api/file/upload$", sefaria_views.file_upload
    ),  # SEC-AUDIT: do we limit how many files users can upload?
]
...
urlpatterns += [
    ...
    url(
        r"^admin/rebuild/citation-links/(?P<title>.+)$",
        sefaria_views.rebuild_citation_links,
    ),
    url(
        r"^admin/delete/citation-links/(?P<title>.+)$",
        sefaria_views.delete_citation_links,
    ),
    ...
    url(
        r"^admin/descriptions/categories/update",
        sefaria_views.update_categories_from_sheet,
    ),
...
urlpatterns += [
    url(
        r"^(?P<tref>[^/]+)/(?P<lang>\w\w)/(?P<version>.*)$",
        reader_views.old_versions_redirect,
    ),
...
if DOWN_FOR_MAINTENANCE:
    ...
    urlpatterns = [
        ...
        url(r"^admin/?", include(admin.site.urls)),
        url(
            r"^healthz/?$", reader_views.application_health_api
        ),  # this oddly is returning 'alive' when it's not.  is k8s jumping in the way?
    ...
    urlpatterns += [url(r".*", sefaria_views.maintenance_message)]
