...
from sefaria.utils.util import get_lang_codes_for_territory, short_to_long_lang_code
...
class LanguageSettingsMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        profile = (
            UserProfile(id=request.user.id) if request.user.is_authenticated else None
        )
        # INTERFACE
        ...
        if not interface:
            ...
            interface = (
                request.COOKIES.get("interfaceLang")
            ...
            interface = (
                "english" if interface not in ("english", "hebrew") else interface
            )
        ...
        if domain_lang and domain_lang != interface:
            ...
            if any(
                [bot in request.META.get("HTTP_USER_AGENT", "") for bot in no_direct]
            ):
        ...
        content = (
            ...
            or request.COOKIES.get("contentLang")
        ...
        content = (
            ...
            if content not in ("english", "hebrew", "bilingual")
            else content
        )
        ...
        langs_in_country = get_lang_codes_for_territory(
            request.META.get("HTTP_CF_IPCOUNTRY", None)
        )
        translation_language_preference_suggestion = None
        trans_lang_pref_suggested = (
            profile is not None
            and profile.settings.get("translation_language_preference_suggested", False)
        ) or request.COOKIES.get("translation_language_preference_suggested", False)
        if translation_language_preference is None and not trans_lang_pref_suggested:
            supported_translation_langs = set(
        ...
        version_preferences_by_corpus_cookie = json.loads(
            unquote(request.COOKIES.get("version_preferences_by_corpus", "null"))
        )
        request.version_preferences_by_corpus = (
            (
                profile is not None
                and getattr(profile, "version_preferences_by_corpus", None)
            )
            or version_preferences_by_corpus_cookie
            or {}
        )
        ...
        request.translation_language_preference_suggestion = (
