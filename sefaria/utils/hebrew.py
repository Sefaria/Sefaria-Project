...
def heb_string_to_int(n):
    ...
    n = re.sub(
        '[\u05F4"\u201c\u201d\u200e]', "", n
    )  # remove gershayim, double quote, fancy double quote, LTR Control Character
...
def decode_hebrew_numeral(numeral: str):
    ...
    t = list(
        map(heb_string_to_int, split_thousands(numeral))
    )  # split and convert to numbers
    t = [
        pow(10, 3 * E_num[0]) * E_num[1] for E_num in enumerate(t)
    ]  # take care of thousands and add
    return sum(t)
...
def int_to_heb(integer):
    ...
    for num in range(500, 1200, 100):
        hebrew_numerals[num] = (
            hebrew_numerals[400] * (num // 400) + hebrew_numerals[num % 400]
        )

    if integer > 1100:
        raise KeyError(
            "Asked to convert individual integer {} above 1100; too large.".format(
                integer
            )
        )
...
def break_int_magnitudes(n, start=None):
    ...
    if start is not None:
        if not (start % 10 == 0 or start == 1):
            raise TypeError(
                "Argument 'start' must be 1 or divisible by 10, {} provided.".format(
                    start
                )
            )
    else:
    ...
    if start == 1:
        ...
        return [n // start * start] + break_int_magnitudes(
            n - n // start * start, start=start // 10
        )
...
def decompose_presentation_forms_in_str(orig_str):
    return presentation_re.sub(
        lambda match: decompose_presentation_forms(match.group()), orig_str
    )
...
def normalize_final_letters_in_str(orig_str):
    return final_letter_re.sub(
        lambda match: normalize_final_letters(match.group()), orig_str
    )
...
def swap_keyboards_for_string(orig_str):
    return re.sub(
        r".", lambda match: swap_keyboards_for_letter(match.group()), orig_str
    )
...
def get_abbr(
    abbr: str, unabbr: List[str], match=lambda x, y: x.startswith(y), lang="he"
):
    abbr = re.sub("[^א-ת]", "", abbr) if lang == "he" else re.sub("[^a-z]", "", abbr)
    indexes = [
        [index for index, letter in enumerate(abbr) if word[0] == letter]
        for w, word in enumerate(unabbr)
    ]
    ...
    for choice in choices:
        ...
        choice += [None]
        if all(
            match(temp_unabbr[n], abbr[choice[n] : choice[n + 1]])
            for n in range(len(temp_unabbr))
        ):
            return temp_unabbr
    return None
...
def hebrew_starts_with(he: str, other_he: str) -> False:
    ...
    while ihe < len(he_words) and iother < len(other_words):
        if iabbr < len(he_abbrs) and he_abbrs[iabbr].iabbr == ihe:
            ...
            continue
        if (
            i_other_abbr < len(other_abbrs)
            and other_abbrs[i_other_abbr].iabbr == iother
        ):
