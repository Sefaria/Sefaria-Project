...
class TestNikkudUtils:
    def test_strip_nikkud(self):
        ...
        assert (
            h.strip_nikkud("הַמּוֹצִיא בְמִסְפָּר צְבָאָם לְכֻלָּם בְּשֵׁם יִקְרָא")
...
@pytest.mark.parametrize(
    ("hebrew", "other_hebrew", "should_match"),
    [
        ...
        (
            "אע״ג שרבי שמעול בן גמליאל שמר שבית שאשרעש לגכשג",
            "אף על גב שרשב״ג שמר שבשאש״ל",
            True,
        ),  # multiple abbrevs
        (
            "אע״ג שרבי שמעול בן גמליאל שמר שבית שארעש לגכשג",
            "אף על גב שרשב״ג שמר שבשאש״ל",
            False,
        ),  # diff multiple abbrevs
    ],
)
    ...
    [
        ...
        pytest.param(
            ...
            marks=pytest.mark.xfail(
                reason="cannot handle non-sofit letters in abbrev matching sofit in unabbr"
            ),
        ),
