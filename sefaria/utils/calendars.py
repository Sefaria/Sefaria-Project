...
def daf_yomi(datetime_obj):
    ...
    if "displayValue" in daf:
        daf_en, daf_he = daf["displayValue"].get("en", None), daf["displayValue"].get(
            "he", None
        )
        ...
        daf_yomi.append(
            {
                ...
                "displayValue": {
                    "en": daf_en if daf_en else rf.normal(),
                    "he": daf_he if daf_he else rf.he_normal(),
                },
                "url": rf.url(),
                "ref": rf.normal(),
                "order": 3,
...
def daily_mishnayot(datetime_obj):
    ...
    daily_mishnahs = db.daily_mishnayot.find({"date": {"$eq": datetime_obj}}).sort(
        [("date", 1)]
    )
    ...
    items.append(
        {
            ...
            "displayValue": {
                "en": display_en.replace("Arukh HaShulchan, ", ""),
                "he": display_he.replace("ערוך השולחן, ", ""),
            },
            "url": rf.url(),
            "ref": rf.normal(),
            "order": 10,
        ...
        rambam_items.append(
            {
                "title": {
                    ...
                    "he": 'הרמב"ם היומי {}'.format("(3 פרקים)"),
                },
                "displayValue": {"en": display_en, "he": display_he},
                "url": rf.url(),
                "ref": rf.normal(),
...
def make_haftarah_by_custom_response_from_calendar_entry(
    db_haftara, custom, add_custom_to_display
):
    shorthands = {
        "ashkenazi": {"en": "A", "he": "א"},
        "sephardi": {"en": "S", "he": "ס"},
    ...
    for h in db_haftara:
        ...
        if add_custom_to_display:
            for lang in haftara["title"]:
                haftara["title"][lang] = "{} ({})".format(
                    haftara["title"][lang], shorthands[custom][lang]
                )
...
def make_haftarah_response_from_calendar_entry(db_parasha, custom=None):
    ...
    if isinstance(db_parasha["haftara"], list):  # backwards compatability
        haftarah_objs += make_haftarah_by_custom_response_from_calendar_entry(
            db_parasha["haftara"], "ashkenazi", False
        )
    elif len(list(db_parasha["haftara"].keys())) == 1 or not len(
        db_parasha["haftara"].get(custom, [])
    ):
        haftarah_objs += make_haftarah_by_custom_response_from_calendar_entry(
            db_parasha["haftara"].get("ashkenazi"), "ashkenazi", False
        )
    elif custom:
        haftarah_objs += make_haftarah_by_custom_response_from_calendar_entry(
            db_parasha["haftara"].get(custom, []), custom, True
        )
    else:
        for key in db_parasha["haftara"]:
            haftarah_objs += make_haftarah_by_custom_response_from_calendar_entry(
                db_parasha["haftara"].get(key), key, True
            )
    return haftarah_objs
...
@graceful_exception(logger=logger, return_value=[])
def parashat_hashavua_and_haftara(
    datetime_obj, diaspora=True, custom=None, parasha=None, ret_type="list"
):
    ...
    if ret_type == "list":
        ...
        he_date_in_english, he_date_in_hebrew = get_hebrew_date(
            db_parasha.get("date", None)
        )
        parasha_items = {
            "parasha": parasha_item[0],
            "haftarah": haftarah_item,
            "date": db_parasha.get("date", None),
            "he_date": {"en": he_date_in_english, "he": he_date_in_hebrew},
        }
...
def hok_leyisrael(datetime_obj, diaspora=True):
    def get_hok_parasha(datetime_obj, diaspora=diaspora):
        ...
        parasha_term = Term().load(
            {"category": "Torah Portions", "titles": {"$elemMatch": {"text": parasha}}}
        )
        if not parasha_term:
            parasha_term = get_hok_parasha(
                datetime_obj + datetime.timedelta(7), diaspora=diaspora
            )
        return parasha_term
...
def get_all_calendar_items(datetime_obj, diaspora=True, custom="sephardi"):
    if not SITE_SETTINGS["TORAH_SPECIFIC"]:
        return []
    cal_items = []
    cal_items += parashat_hashavua_and_haftara(
        datetime_obj, diaspora=diaspora, custom=custom
    )
...
def get_todays_calendar_items(diaspora=True, custom=None):
    return get_all_calendar_items(
        timezone.localtime(timezone.now()), diaspora=diaspora, custom=custom
    )
