...
def delete_user_account(uid, confirm=True):
    ...
    if confirm:
        print(
            "Are you sure you want to delete the account of '%s' (%s)?"
...
def merge_user_accounts(
    from_uid, into_uid, fill_in_profile_data=True, override_profile_data=False
):
    ...
    print(
        ...
        % (from_user.full_name, from_user.email, into_user.full_name, into_user.email)
    )
    ...
    print(
        "Content from %s moved into %s's account." % (from_user.email, into_user.email)
    )
...
def generate_api_key(uid):
    ...
    key = (
        base64.b64encode(
            hashlib.sha256(
                bytes(str(random.getrandbits(256)), encoding="utf-8")
            ).digest(),
            random.choice([b"rA", b"aZ", b"gQ", b"hH", b"hG", b"aR", b"DD"]),
        )
        .rstrip(b"==")
        .decode("utf-8")
    )
    ...
    print(
        "API Key for %s (uid: %d, email: %s): %s"
        % (user.full_name, uid, user.email, key)
    )
