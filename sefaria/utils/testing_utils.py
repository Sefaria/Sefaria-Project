        ...
        # category
            ...
            if sub_result:
                for path in sub_result:
                    new_path = (
                        [toc_elem["category"]] + path
                        if isinstance(path, list)
                        else [toc_elem["category"]]
                    )
...
def verify_title_existence_in_toc(title, expected_toc_location=None, toc=None):
    ...
    assert (
        len(locations_in_toc) == num_appearances_in_toc
    ), "title appears %d times" % len(locations_in_toc)
    if expected_toc_location:
        assert toc_path_to_string(expected_toc_location) == toc_path_to_string(
            locations_in_toc[0]
        ), locations_in_toc
...
def verify_existence_across_tocs(title, expected_toc_location=None):
    verify_title_existence_in_toc(
        title, expected_toc_location, toc=model.library.get_toc()
    )
