...
def cleanup_and_format_text(text, language):
    ...
    text = text.replace("—", "-")
    text = text.replace("\u05BE", " ")  # replace hebrew dash with ascii

    strip_cantillation_vowel_regex = re.compile(
        "[^\u05d0-\u05f4\s^\x00-\x7F\x80-\xFF\u0100-\u017F\u0180-\u024F\u1E00-\u1EFF\u2000-\u206f]"
    )
...
def generate_image(
    text="", category="System", ref_str="", lang="he", platform="twitter"
):
    text_color = palette[category][1]
    bg_color = palette[category][0]

    font = ImageFont.truetype(
        font="static/fonts/Amiri-Taamey-Frank-merged.ttf",
        size=platforms[platform]["font_size"],
    )
    ...
    if lang == "en":
        ...
        ref_font = ImageFont.truetype(
            font="static/fonts/Roboto-Regular.ttf",
            size=platforms[platform]["ref_font_size"],
        )
        ...
        ref_font = ImageFont.truetype(
            font="static/fonts/Heebo-Regular.ttf",
            size=platforms[platform]["ref_font_size"],
        )
    ...
    text = textwrap.fill(
        text=text, width=calc_letters_per_line(text, font, int(img.size[0] - padding_x))
    )
    text = get_display(
        text
    )  # Applies BIDI algorithm to text so that letters aren't reversed in PIL.
    ...
    # category line
    draw.line(cat_border_pos, fill=palette[category][0], width=int(width * 0.02))

    # header white
    draw.line(
        (0, int(height * 0.05), img.size[0], int(height * 0.05)),
        fill=(255, 255, 255),
        width=int(height * 0.1),
    )
    draw.line(
        (0, int(height * 0.1), img.size[0], int(height * 0.1)),
        fill="#CCCCCC",
    ...
    # write ref
    draw.text(
        xy=(img.size[0] / 2, img.size[1] - padding_y / 2),
        text=get_display(ref_str.upper()),
        font=ref_font,
        spacing=spacing,
        align=align,
        fill=text_color,
    ...
    logo.thumbnail((width, int(height * 0.06)))
...
def make_img_http_response(text, category, ref_str, lang, platform):
    try:
        ...
        logo_padded.paste(
            logo,
            (int(width / 2 - logo.size[0] / 2), int(height / 2 - logo.size[1] / 2)),
        )
        img = Image.alpha_composite(img, logo_padded)
