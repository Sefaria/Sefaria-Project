...
- **Library Assistant opt-out setting** — whether the assistant is *offered* to a given user, LAS-001 → LAS-062. Runs under the `la-setup` + `la-setting` projects, which log in as their own seeded cohort accounts rather than the shared QA profiles, and appear only once `scripts/dev/seed_library_assistant_e2e_users.py` has written `e2e-tests/.la-e2e-users.json`. See [library-assistant-setting/README.md](library-assistant-setting/README.md).
...
# Library Assistant opt-out setting. Requires the seeded cohorts —
