...
22. **The Strapi content suite (`tests/strapi-*.spec.js`, project `chrome-strapi`) is a documented exception to rules 1, 4 and 5.** It uses bare `page.goto`, raw CSS locators and no `PageManager` **on purpose** — do not "fix" it to match the house style. `goToPageWithLang` / `goToPageWithUser` call `installOverlaySuppression`, which short-circuits `/api/strapi/graphql-cache` with an empty payload and marks every `modal_*`/`banner_*` key as already-seen (see rule 3). That is precisely the content those specs assert on, so routing them through the standard entry points does not merely change style — it deletes the subject of the test. The surfaces under test (`#bannerMessage`, `#interruptingMessageBox`, `.sidebarPromo`) also have no roles or labels to target, so CSS is the only available anchor. These specs never contact live Strapi. Every spec builds its payload in code (`routeWithStrapiPayload` + `support/strapi-payload-factory.js`); the fourteen originally-recorded scenarios serve byte-equal factory replicas from `tests/strapi.scenario-payloads.js`, proven against their frozen `.har` recordings by the fidelity spec (HAR replay itself was retired 2026-08-31 — `routeFromHAR` matched on the POST body, so every GraphQL query change invalidated all recordings at once; the `.har` files remain committed as reference and as the payload contract's schema oracle). Specs carry an `afterEach` guard (`expectStrapiServed`) that turns "the payload never reached the page" into an explicit failure instead of a silently passing absence assertion — keep it when editing. Before touching any of this read [tests/README.md](tests/README.md), which covers when to record versus synthesize, the recording workflow, the faked clock (`install()` alone is **not** enough — it keeps running with real time; `pauseAt` is what freezes it), `waitForTimerArmed` for `showDelay` boundaries, the `cf-ipcountry` country lever, the stale-fixture guard, and the payload contract spec that holds the factory to the real recorded shape.
...
| Strapi-driven content (banners, modals, sidebar ads) — HAR replay, **exception to §2.1/4/5, see §2.22** | `tests/` | `chrome-strapi` | `SANDBOX_URL` verbatim |
...
| Strapi content suite: recording workflow, levers, traps | [tests/README.md](tests/README.md) |
