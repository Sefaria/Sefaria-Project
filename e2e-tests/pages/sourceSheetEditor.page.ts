...
export class SourceSheetEditorPage {
  ...
  async assertSaveState(state: any, language = LANGUAGES.EN) {
    ...
    await expect(this.statusMessage()).toHaveText(text, { timeout: t(5000) });
    await this.statusIndicator().hover();
    await expect(this.statusIndicator()).toHaveAttribute('aria-label', tooltip, { timeout: t(2000) });
  ...
  async waitForAutosave() {
    ...
    await this.page.waitForTimeout(t(3000));
    ...
    }, null, { timeout: t(5000) }); // optional timeout
  }
