...
| [`strapi-sidebar-ad-page-type.spec.js`](strapi-sidebar-ad-page-type.spec.js) | *(synthetic)* Page-type targeting: one shows-up test per targetable page type on its real page (incl. the voices-module host), null/unknown `pageType` boundaries, the category_toc + multi-word-keyword conjunction on a real collection TOC, author-vs-topic-vs-portal pages (a portal classifies exclusively as portal_page), the no-flash guarantee while topic data loads, and the legacy-Strapi one-shot retry. Needs mongod + seeded topic pools (see `../support/seed_topic_pools.py`); voices/auth tests skip by name when unavailable. |
