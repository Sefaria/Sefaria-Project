...
- `tasks.py` is invoked via `dispatch_chatbot_opt_in_webhook(email, opt_in, interface_language)`, called from `sefaria.helper.library_assistant.notify_crm_of_change` whenever a user's effective Library Assistant setting genuinely changes; it does not go through `CrmMediator`.
