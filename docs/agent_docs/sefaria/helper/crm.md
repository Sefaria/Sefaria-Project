...
- `tasks.py` is entered via `dispatch_chatbot_opt_in_webhook(email, opt_in, interface_language)`, which has no callers in the application: the task is kept as a deactivated working reference behind `CHATBOT_OPT_IN_WEBHOOK_DEACTIVATED`. Reactivating it means flipping that flag and calling the dispatch from wherever the signal is wanted. It does not go through `CrmMediator`.
