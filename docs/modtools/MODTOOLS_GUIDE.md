...
- Endpoints: `GET /api/version-indices`, `POST /api/version-bulk-edit`, `PATCH /api/version-rename`, `POST /api/version-bulk-delete`
...
| `/api/version-rename` | PATCH | Rename `versionTitle` for a single index (called per-index by the client to rename across many indices) | Staff |
