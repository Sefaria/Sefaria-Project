...
// FUNCTION
		function makeMediaEmbedLink(mediaURL, target) {
		...
		if ((m = re.exec(mediaURL)) !== null) {
			if (m.index === re.lastIndex) {
				...
				if (m.length>0) {
					...
					addMediaToSheet(embedHTML, target);
				}
		}
    ...
    // Adding YUTorah capacities
		else if ((mediaURL).match(/.*?yutorah.org.*?cfm\/(.*?)\//i) != null) {

			var re = /.*?yutorah.org.*?cfm\/(.*?)\//i;
			var m;
			var shiurJSON = 'https://www.yutorah.org//sidebar//getLectureDataJSON.cfm?shiurID=';

			// console.log("YU Torah");

			// Run an exec regex match to extract
			// the shiur ID and append it on to the url
			// with the shiurJSON
			if ((m = re.exec(mediaURL)) !== null) {
				if (m.index === re.lastIndex) {
					re.lastIndex++;
					...
					if (m.length>0) {
						shiurJSON += m[m.length-1];
					}
			}

			//console.log(shiurJSON)

			// fetch the shiur JSON data
      fetch(shiurJSON)
            ...
            .then(data => {
							...
							const embedURL = data["downloadURL"];
							console.log("Embed URL: " + embedURL);
							embedHTML = '<audio src="' + embedURL + '" type="audio/mpeg" controls>Your browser does not support the audio element.</audio>';
							console.log(embedHTML);


							//parse the json
							const teacherName = data["shiurTeacherFullName"];
							const shiurTitle = data["shiurTitle"]

							console.log(teacherName + " - \'" + shiurTitle + "\'. Retrieved from: " + mediaURL);

							// Create a mediaCaption
							var caption = "<b>" + shiurTitle + "</b><br>By " + teacherName + " <br> <a href= \"" + mediaURL + "\">" + mediaURL + "</a>";
							var mediaCaption = "<div class='mediaCaption enOnly'><div class='mediaCaptionInner'>" +
												"<div class='en'>" + caption + "</div>" +
												"<div class='he'> </div>" +
											 	"</div></div>";


							// append the media caption to the HTML
							embedHTML += mediaCaption;
	...
	function addMediaToSheet(embedHTML, target){
		if (embedHTML != false) {
...
;
  ...
  if (sjs.is_owner||sjs.can_edit||sjs.can_add) {
    ...
    $("#addmediaDiv").on("click", ".button:first", function(e) {
      ...
      var embedHTML = makeMediaEmbedLink($("#inlineAddMediaInput").val(), $target);
			console.log(embedHTML);
    });
