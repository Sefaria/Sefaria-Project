...
function SefariaEditor(props) {
    ...
    function renderBlock(props, editor, next) {
        ...
        switch (node.type) {
            ...
            case 'SheetSource':
                return (
                    <div className="SheetSource" {...attributes} onClick={(event) => checkForBorderClick(event, props)}>
                        {children}
                    </div>
                );
    ...
    function isEndOfSheetItemBlockContent(v) {
        ...
        const anchorBlock = v.anchorBlock;

        if (!anchorBlock) {return false}

        const nextNodeBlock = v.document.getNextNode(anchorBlock.key);
        if (!nextNodeBlock) {return true}
        else {
            return (v.selection.focus.isAtEndOfNode(anchorBlock) && nextNodeBlock.get("type") !== "paragraph")
        }
    }

    function checkForBorderClick(event, props) {
        console.log(event)
        console.log(props.editor.value.fragment.toJSON())
        console.log(this)

        if(  event.offsetX <= parseInt($(this).css('borderLeftWidth'))){
            alert('clicked on the left border!');
        }


    //     if (!editor.value.selection.anchor.path) {return next()}
    //     const fragment = editor.value.fragment;
    //     const curSelectedRefs = fragment.getBlocksByType('TextRef');
    //     if (curSelectedRefs.size == 1) {
    //         const textRefNode = fragment.getNode(curSelectedRefs.get(0).key)
    //         console.log(textRefNode.toJSON())
    //         console.log(textRefNode)
    //         console.log(textRefNode.key)
    //         console.log(textRefNode.anchor)
    //         console.log(textRefNode.focus)
    //         editor.moveToRangeOfNode(textRefNode);
    //     }
    }
