...
/* =========  Tiny autosave helper  ========= */
(function() {
  /* --- settings you might tweak --- */
  const DEBOUNCE_MS        = 800;          // wait time after keystroke
  const STORAGE_PREFIX     = "sjs_draft_"; // namespace in localStorage
  const SELECTOR_EDIT_BOX  = "textarea, [contenteditable='true']";

  /* --- internal helpers --- */
  function keyForPage() {
    // If the editor knows the ref, prefer that; else fall back to URL-path.
    let ref = (sjs && sjs.current && sjs.current.sectionRef) || location.pathname;
    return STORAGE_PREFIX + ref;
  }

  function getBox() {
    // Grabs whichever editable element is currently visible.
    return $(SELECTOR_EDIT_BOX).filter(":visible").first();
  }

  function saveDraft() {
    const $box = getBox();
    if (!$box.length) { return; }
    const val  = $box.is("textarea") ? $box.val() : $box.html();
    try       { localStorage.setItem(keyForPage(), val); }
  ...
  function loadDraft() {
    ...
    const saved  = localStorage.getItem(keyForPage());
    if (saved && !($box.val ? $box.val() : $box.html()).trim()) {
      // Only inject if the field is still empty
      if ($box.is("textarea")) { $box.val(saved); }
      else                     { $box.html(saved); }

      // Optional: little toast so users know what happened
      if (sjs && sjs.alert && sjs.alert.message) {
        sjs.alert.message("Restored an unsaved draft for you.");
      }
    }
  }
  ...
  /* --- wire-up --- */
  let t;
  $(document).on("input", SELECTOR_EDIT_BOX, function () {
    clearTimeout(t);
    t = setTimeout(saveDraft, DEBOUNCE_MS);
  });

  // Just in case the user closes the tab faster than debounce time.
  $(window).on("beforeunload", saveDraft);

  // Kill the draft once a real save or cancel happens
  $(document).on("click", "#saveNote, #cancelNote, .btn-primary.save, #addSourceSave, #saveText",
