...
Sefaria = extend(Sefaria, {
  ...
  _adaptApiResponse: function(versionsResponse) {
    ...
    if (!translation.sources.every(source => source === translation.sources[0])) {
        versionsResponse.sources = translation.sources;
    }
    if (!primary.sources.every(source => source === primary.sources[0])) {
  ...
  for (let i = 0; i < initialPanels.length; i++) {
      ...
      if (panel.text) {
        ...
        if(panel.text?.versions?.length){
            let versions = Sefaria.saveVersions(panel.text.sectionRef, panel.text.versions);
