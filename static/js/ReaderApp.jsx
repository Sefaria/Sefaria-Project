    ...
    for (var i = 0; i < panels.length; i++) {
      ...
      if (state.menuOpen) {
        ...
        var filter    = state.filter.length ? state.filter :
                          (state.connectionsMode in {"Sheets": 1, "Notes": 1, "Versions": 1, "Version Open": 1, "About": 1, "extended notes" : 1, "See Also": 1} ? [state.connectionsMode] : ["all"]);
        ...
        var filter    = state.filter.length ? state.filter :
                          (state.connectionsMode in {"Sheets": 1, "Notes": 1, "Versions": 1, "Version Open": 1, "About": 1, "See Also": 1} ? [state.connectionsMode] : ["all"]);
        hist.sources  = filter.join("+");
        if (state.connectionsMode === "Version Open" && state.versionFilter.length) {
          hist.versionFilter = state.versionFilter[0];
        }
