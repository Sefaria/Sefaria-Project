...
Argo post-promotion analysis (prod)
  -> repository_dispatch (prod-rollout-succeeded, carries `version`)
...
- `.github/workflows/prod-release-notes.yaml` — listens for that dispatch
  (or a manual `workflow_dispatch`), resolves the version, runs
...
1. **Dry-run the whole pipeline without a real deploy.** Run:
   ...
   ```
   gh workflow run "Prod Release Notes" -f version=<a past prod version> -f dry_run=true
   ```
...
3. Watch for the existing terse Slack ping from `rollout-complete-prod`
