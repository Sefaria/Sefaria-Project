...
yq -i '.spec.template.spec.containers[0].args = ["-c", "python /app/build/ci/cleanup_test_data.py && pip3 install pytest-django pytest-timeout && pytest -v --timeout=600 --reuse-db -m \"not deep and not failing\" ./sefaria ./sso ./reader ./powered_by"]' job.yaml
