...
$K scale deploy/authpoc-mint --replicas=0; $K wait --for=delete pod -l app=mint-authpoc --timeout=60s
echo DURING; watch_for 360 $H "${A[@]}"
$K scale deploy/authpoc-mint --replicas=1; $K rollout status deploy/authpoc-mint; sleep 20
