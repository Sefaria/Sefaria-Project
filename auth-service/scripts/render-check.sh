...
E=${ENV:-authpoc}
assert "$(count Deployment "^$E-auth-service")" 2 "<env>-auth-service + <env>-auth-service-postgres deployments"
...
assert "$(yq 'select(.kind == "Deployment" and .metadata.name == "'"$E"'-auth-service-postgres") | .spec.template.spec.containers[0].volumeMounts[] | select(.mountPath == "/docker-entrypoint-initdb.d") | .name' "$R")" initdb "schema mounted into docker-entrypoint-initdb.d"
