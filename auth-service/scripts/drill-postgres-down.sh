...
$K scale deploy/authpoc-auth-service-postgres --replicas=0; $K wait --for=delete pod -l app=auth-service-postgres-authpoc --timeout=60s
...
$K scale deploy/authpoc-auth-service-postgres --replicas=1; $K rollout status deploy/authpoc-auth-service-postgres
