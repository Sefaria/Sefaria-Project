...
H=authpoc.cauldron.sefaria.org; A=(-H 'x-api-key: sfr_alpha000000000000000000000000001')
...
bash scripts/keyadmin-run.sh >/dev/null 2>&1 || true; source scripts/keyadmin-run.sh; run init-schema --schema /schema.sql >/dev/null; run --push redis create --project proj_alpha --tier developer --key sfr_alpha000000000000000000000000001 >/dev/null
