...
class TestFilterByRefs:
    def test_empty_refs_skips_query(self):
        with patch("semantic_search.models.SemanticTextChunk.objects") as mock_objects:
...
class TestLinkedRefEnhancement:
    ...
    def test_excludes_original_result_refs(self):
        chunks = [
            make_chunk(ref="Genesis 1:1", chunked_from_ref="Genesis 1", linked_refs=["Genesis 1:1", "Genesis 1", "Ref B"]),
    ...
    def test_depth_two_fetches_and_counts_next_hop_links(self):
        first_hop = make_chunk(ref="Ref B", linked_refs=["Ref D", "Ref E"])
        chunk_store = MagicMock()
        chunk_store.filter_by_refs.return_value = [first_hop]
        chunks = [
            make_chunk(ref="Genesis 1:1", linked_refs=["Ref B", "Ref D"]),
        ]
    ...
    def test_invalid_params_return_no_appended_refs(self):
        chunk = make_chunk(linked_refs=["Ref B"])
...
class TestSemanticSearch:
    ...
    @patch("semantic_search.search.settings")
    def test_calls_embed_query_then_search_by_embedding(self, mock_settings, mock_embed, mock_chunk_cls):
    ...
    @patch("semantic_search.search.embed_query")
    @patch("semantic_search.search.settings")
    def test_forwards_filters_and_limit(self, mock_settings, mock_embed, mock_chunk_cls):
        mock_settings.GEMINI_API_KEY = "test-key"
    ...
    @patch("semantic_search.search.embed_query")
    @patch("semantic_search.search.settings")
    def test_returns_search_results(self, mock_settings, mock_embed, mock_chunk_cls):
        mock_settings.GEMINI_API_KEY = "test-key"
