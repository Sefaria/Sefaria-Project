...
class SefariaTestCase(TestCase):
    def make_test_user(self):
        user = User.objects.create_user(
            username="test@sefaria.org", email="test@sefaria.org", password="!!!"
        )
        user.set_password("!!!")
    ...
    def not_in_cache(self, title):
        ...
        self.assertFalse(
            any(key.startswith(title) for key, value in Ref._raw_cache().items())
        )
...
class PagesTest(SefariaTestCase):
    ...
    def test_text_history(self):
        response = c.get(
            "/activity/Genesis_12/en/The_Holy_Scriptures:_A_New_Translation_(JPS_1917)"
        )
        self.assertEqual(200, response.status_code)
...
class ApiTest(SefariaTestCase):
    ...
    def test_api_get_text_talmud_commentary(self):
        ...
        self.assertEqual(
            data["categories"],
            ["Talmud", "Bavli", "Commentary", "Tosafot", "Seder Moed"],
        )
    ...
    def test_api_get_text_bad_text(self):
        ...
        self.assertEqual(
            data["error"],
            "Failed to parse sections for ref Protocols_of_the_Elders_of_Zion.13.13",
        )  # "Unrecognized Index record: Protocols of the Elders of Zion.13.13")
    ...
    def test_api_get_text_too_many_hyphens(self):
        ...
        data = json.loads(response.content)
        self.assertEqual(
            data["error"], "Couldn't understand ref 'Genesis.9-4-5' (too many -'s)."
        )
                ...
                "nodes": [
                    {
                        ...
                        "depth": 1,
                        "titles": [
                            {"lang": "en", "text": "Idrah Rabbah", "primary": True},
                            {"lang": "he", "text": "אידרה רבה", "primary": True},
                        ],
...
class PostIndexTest(SefariaTestCase):
    ...
    def tearDown(self):
        ...
        job.nodes.title_group.titles = [
            variant
            for variant in job.nodes.title_group.titles
            if variant["text"] != "Boj"
        ]
        job.save()
...
class PostTextNameChange(SefariaTestCase):
    ...
    def tearDown(self):
        ...
        HistorySet(
            {"version": "The Name Change Test Edition", "rev_type": "add text"}
        ).delete()
        HistorySet(
            {"new.refs": {"$regex": "Name Change Test"}, "rev_type": "add link"}
        ).delete()
        HistorySet(
            {"new.ref": {"$regex": "Name Change Test"}, "rev_type": "add note"}
        ).delete()
        HistorySet({"rev_type": "add index", "title": "Name Changed"}).delete()
        HistorySet({"ref": {"$regex": "Name Changed"}, "rev_type": "add text"}).delete()
        HistorySet(
            {"new.ref": {"$regex": "Name Changed"}, "rev_type": "add note"}
        ).delete()
        HistorySet(
            {"new.refs": {"$regex": "Name Changed"}, "rev_type": "add link"}
        ).delete()
        HistorySet(
            {"old.ref": {"$regex": "Name Changed"}, "rev_type": "delete note"}
        ).delete()
        HistorySet(
            {"old.refs": {"$regex": "Name Changed"}, "rev_type": "delete link"}
        ).delete()
    ...
    def test_change_index_name(self):
        ...
        self.assertEqual(
            1,
            HistorySet({"rev_type": "add index", "title": "Name Change Test"}).count(),
        )
        ...
        link1 = {"refs": ["Name Change Test 1.1", "Genesis 1:5"], "type": "reference"}
        link2 = {
        ...
        self.assertEqual(
            0,
            HistorySet(
                {"ref": {"$regex": "Name Change Test"}, "rev_type": "add text"}
            ).count(),
        )
        self.assertEqual(
            0,
        ...
        self.assertEqual(
            1, HistorySet({"rev_type": "add index", "title": "Name Changed"}).count()
        )
        self.assertEqual(
            1,
            HistorySet(
                {"ref": {"$regex": "Name Changed"}, "rev_type": "add text"}
            ).count(),
        )
        self.assertEqual(
            1,
        ...
        tutils.verify_title_existence_in_toc(
            new["title"], expected_toc_location=orig["categories"]
        )
...
class PostTextTest(SefariaTestCase):
    ...
    def test_post_new_text(self):
        ...
        toc = json.loads(c.get("/api/index").content)
        tutils.verify_title_existence_in_toc(
            index["title"], expected_toc_location=index["categories"]
        )
    ...
    def test_post_to_default_node(self):
        ...
        response = c.post(
            "/api/texts/Chofetz_Chaim,_Part_One,_The_Prohibition_Against_Lashon_Hara,_Principle_1",
...
class PostCategory(SefariaTestCase):
    ...
    def test_orphan_rejected(self):
        cat = {
            "path": ["Tanakh", "Other Stuff", "New Works"],
            "titles": [
                {"lang": "en", "text": "New Works", "primary": True},
                {"lang": "he", "text": "חידושים", "primary": True},
            ],
        }
        response = c.post("/api/category", {"json": json.dumps(cat)})
        self.assertEqual(200, response.status_code)
    ...
    def test_incomplete_record_rejected(self):
        ...
        cat = {
            "path": ["Tanakh", "New Works"],
            "titles": [{"lang": "en", "text": "New Works", "primary": True}],
        }
        response = c.post("/api/category", {"json": json.dumps(cat)})
        self.assertEqual(200, response.status_code)
        data = json.loads(response.content)
        ...
        cat = {
            "path": ["Tanakh", "New Works"],
            "titles": [{"lang": "he", "text": "חידושים", "primary": True}],
        }
        response = c.post("/api/category", {"json": json.dumps(cat)})
        self.assertEqual(200, response.status_code)
        data = json.loads(response.content)
...
class PostLinks(SefariaTestCase):
    ...
    def tearDown(self):
        LinkSet(
            {
                "refs": {"$regex": "Meshekh Chokhmah"},
                "anchorText": {"$exists": 1, "$ne": ""},
            }
        ).delete()
    ...
    def test_post_new_links(self):
        ...
        for i in range(1, 61):
            for j in range(1, 11):
                link_obj = {
                    "type": "commentary",
                    "refs": [
                        "Meshekh Chokhmah %d:%d" % (i, j),
                        "%s 1:1" % rand.choice(bible_books),
                    ],
        ...
        self.assertEqual(200, response.status_code)
        self.assertNotEqual(
            600, LinkSet({"refs": {"$regex": "Meshekh Chokhmah"}}).count()
        )
        # Delete links
        LinkSet(
            {
...
class SheetPostTest(SefariaTestCase):
    ...
    def test_post_sheet(self):
        ...
        response = c.post(
            "/api/sheets/{}/add_ref".format(sheet_id), {"ref": "Mishnah Peah 1:1"}
        )
        self.assertEqual(200, response.status_code)
        data = json.loads(response.content)
        self.assertTrue("error" not in data)
...
class UserSyncTest(SefariaTestCase):
    ...
    def test_save_delete(s):
        ...
        response = c.post(
            "/api/profile/sync", {"last_sync": s.d(0), "user_history": s.d([hist1])}
        )
        response = c.post(
            "/api/profile/sync", {"last_sync": s.d(0), "user_history": s.d([hist1save])}
        )
        ...
        response = c.post(
            "/api/profile/sync",
            {"last_sync": s.d(0), "user_history": s.d([hist1delete])},
        )
        response = c.get("/api/profile/user_history?saved=1")
        data = s.l(response.content)
