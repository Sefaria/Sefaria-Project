...
from sefaria.settings import USE_VARNISH, USE_NODE, NODE_HOST, DOMAIN_LANGUAGES, MULTISERVER_ENABLED, SEARCH_ADMIN, \
    RTC_SERVER, MULTISERVER_REDIS_SERVER, MULTISERVER_REDIS_PORT, MULTISERVER_REDIS_DB, DISABLE_AUTOCOMPLETER, \
    ENABLE_LINKER
...
def get_connections_mode(filter):
    ...
    sidebarModes = (
        ...
        "extended notes", "Topics", "Torah Readings", "manuscripts", "Lexicon")
...
def add_new_topic_api(request):
    if request.method == "POST":
        ...
        t = Topic({'slug': "", "isTopLevelDisplay": data["category"] == "Main Menu", "data_source": "sefaria",
                   "numSources": 0})
...
def topics_api(request, topic, v2=False):
    ...
    if request.method == "GET":
        ...
        response = get_topic(v2, topic, with_links, annotate_links, with_refs, group_related, annotate_time_period,
                             ref_link_type_filters, with_indexes)
        return jsonResponse(response, callback=request.GET.get("callback", None))
    elif request.method == "POST":
        if not request.user.is_staff:
            return jsonResponse({
        ...
        if (topic_data["category"] == "Main Menu") != getattr(topic_obj, "isTopLevelDisplay",
                                                              False):  # True when topic moved to top level or moved from top level
            needs_save = True
            topic_obj.isTopLevelDisplay = topic_data["category"] == "Main Menu"

        if topic_data["origDescription"] != topic_data["description"] or topic_data.get("origCatDescription",
                                                                                        "") != topic_data.get(
            "catDescription", ""):
...
def _topic_data(topic):
    ...
    response = get_topic(True, topic, with_links=True, annotate_links=True, with_refs=True, group_related=True,
                         annotate_time_period=False, ref_link_type_filters=ref_link_type_filters, with_indexes=True)
    return response
