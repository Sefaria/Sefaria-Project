...
from sefaria.history import (
    ...
    record_index_deletion,
    record_version_deletion,
    text_at_revision,
    text_history,
    top_contributors,
)
...
from sefaria.system.decorators import (
    catch_error_as_json,
...
def render_template(
    ...
    if (
        app_props
    ):  # We are rendering the ReaderApp in Node, otherwise its jsut a Django template view with ReaderApp set to headerMode
...
def render_react_component(component, props):
    ...
    try:
        ...
        if isinstance(e, socket.timeout) or (
            hasattr(e, "reason") and isinstance(e.reason, socket.timeout)
        ):
...
def base_props(request):
    ...
    if request.user.is_authenticated:
        ...
        user_data = {
            ...
            "is_editor": UserWrapper(user_obj=request.user).has_permission_group(
                "Editors"
            ),
            ...
            "calendars": get_todays_calendar_items(
                **_get_user_calendar_params(request)
            ),
            "notificationCount": profile.unread_notification_count(),
            "notifications": profile.recent_notifications().client_contents(),
            "saved": {
                "loaded": False,
                "items": profile.get_history(
                    saved=True, secondary=False, serialized=True, annotate=False
                ),
            ...
            "last_place": profile.get_history(
                last_place=True, secondary=False, sheets=False, serialized=True
            ),
            "interruptingMessage": InterruptingMessage(
                attrs=interrupting_message_dict, request=request
            ).contents(),
        }
    else:
        user_data = {
            ...
            "last_place": [],
            "interruptingMessage": InterruptingMessage(
                attrs=GLOBAL_INTERRUPTING_MESSAGE, request=request
            ).contents(),
        }
    user_data.update(
        {
            "last_cached": library.get_last_cached_time(),
            "multiPanel": not request.user_agent.is_mobile
            and not "mobile" in request.GET,
            ...
            "initialSettings": {
                ...
                "punctuationTalmud": request.COOKIES.get(
...
def make_panel_dict(oref, versionEn, versionHe, filter, versionFilter, mode, **kwargs):
    ...
    if oref.is_book_level():
        index_details = library.get_index(oref.normal()).contents(
            with_content_counts=True
        )
        index_details["relatedTopics"] = get_topics_for_book(
            oref.normal(), annotate=True
        )
        if kwargs.get("extended notes", 0) and (
            versionEn is not None or versionHe is not None
        ):
        ...
        panel = {
            ...
            else [r.normal() for r in oref.split_spanning_ref()],
            "currVersions": {
                "en": versionEn,
                "he": versionHe,
            },
        ...
        settings_override = {}
        panelDisplayLanguage = (
            kwargs.get("connectionsPanelDisplayLanguage", None)
            if mode == "Connections"
            else kwargs.get("panelDisplayLanguage", None)
        )
        ...
        if panelDisplayLanguage:
            settings_override.update(
                {"language": short_to_long_lang_code(panelDisplayLanguage)}
            )
        if aliyotOverride:
        ...
        if mode != "Connections" and oref != None:
            try:
                text_family = TextFamily(
                    oref,
                    version=panel["currVersions"]["en"],
                    lang="en",
                    version2=panel["currVersions"]["he"],
                    lang2="he",
                    commentary=False,
                    context=True,
                    pad=True,
                    alts=True,
                    wrapLinks=False,
                    translationLanguagePreference=kwargs.get(
                        "translationLanguagePreference", None
                    ),
                ).contents()
            ...
            if oref.index.categories == ["Tanakh", "Torah"]:
                panel[
                    ...
                    oref.index.contents()
                )  # Included for Torah Parashah titles rendered in text

            if (
                oref.is_segment_level() or oref.is_range()
            ):  # we don't want to highlight "Genesis 3" but we do want "Genesis 3:4" and "Genesis 3-5"
                panel["highlightedRefs"] = [
                    subref.normal() for subref in oref.range_list()
                ]
...
def make_sheet_panel_dict(sheet_id, filter, **kwargs):
    ...
    if highlighted_node:
        ref = next(
            (
                element["ref"]
                for element in sheet["sources"]
                if element.get("ref") and element["node"] == int(highlighted_node)
            ),
    ...
    if filter is not None and ref is not None:
        panels += [
            make_panel_dict(Ref(ref), None, None, filter, None, "Connections", **kwargs)
        ]
        return panels
    else:
        return panels
...
def make_panel_dicts(
    oref, versionEn, versionHe, filter, versionFilter, multi_panel, **kwargs
):
    """
    ...
    if filter is not None and multi_panel:
        panels += [
            make_panel_dict(
                oref, versionEn, versionHe, filter, versionFilter, "Text", **kwargs
            )
        ]
        panels += [
            make_panel_dict(
...
def text_panels(request, ref, version=None, lang=None, sheet=None):
    ...
    filter = (
        request.GET.get("with").replace("_", " ").split("+")
    ...
    if sheet == None:
        versionFilter = (
            [request.GET.get("vside").replace("_", " ")]
            if request.GET.get("vside")
            else []
        )

        if versionEn and not Version().load(
            {"versionTitle": versionEn, "language": "en"}
        ):
            raise Http404
        if versionHe and not Version().load(
            {"versionTitle": versionHe, "language": "he"}
        ):
            raise Http404
        versionEn, versionHe = override_version_with_preference(
            oref, request, versionEn, versionHe
        )
        ...
        if filter is not None:
            ...
            if lang2:
                kwargs["connectionsPanelDisplayLanguage"] = (
                    lang2
                    if lang2 in ["en", "he"]
                    else lang1
                    if lang1 in ["en", "he"]
                    else request.interfaceLang[0:2]
                )
        if request.GET.get("aliyot", None):
            kwargs["aliyotOverride"] = (
                "aliyotOn" if int(request.GET.get("aliyot")) == 1 else "aliyotOff"
            )
        ...
        panels += make_panel_dicts(
            oref, versionEn, versionHe, filter, versionFilter, multi_panel, **kwargs
        )

    elif sheet == True:
        panels += make_sheet_panel_dict(
            ref,
            filter,
            **{
                "panelDisplayLanguage": request.GET.get("lang", request.contentLang),
    ...
    while True:
        ...
        if not ref:
            break
        if ref == "search":
            panelDisplayLanguage = request.GET.get(
                "lang{}".format(i), request.contentLang
            )
            panels += [
                make_search_panel_dict(
                    request.GET, i, **{"panelDisplayLanguage": panelDisplayLanguage}
                )
            ]
            ...
            panelDisplayLanguage = request.GET.get("lang", request.contentLang)
            panels += make_sheet_panel_dict(
                sheet_id, None, **{"panelDisplayLanguage": panelDisplayLanguage}
            )
            ...
            versionEn = (
                request.GET.get("ven{}".format(i)).replace("_", " ")
                if request.GET.get("ven{}".format(i))
                else None
            )
            versionHe = (
                request.GET.get("vhe{}".format(i)).replace("_", " ")
                if request.GET.get("vhe{}".format(i))
                else None
            )
            if not versionEn and not versionHe:
                ...
                if language == "en":
                    versionEn = (
                        request.GET.get("v{}".format(i)).replace("_", " ")
                        if request.GET.get("v{}".format(i))
                        else None
                    )
                else:  # he
                    versionHe = (
            ...
            filter = (
                request.GET.get("w{}".format(i)).replace("_", " ").split("+")
                if request.GET.get("w{}".format(i))
                else None
            )
            filter = [] if filter == ["all"] else filter
            versionFilter = (
            ...
            if request.GET.get("aliyot{}".format(i), None):
                kwargs["aliyotOverride"] = (
                    "aliyotOn"
                    if int(request.GET.get("aliyot{}".format(i))) == 1
                    else "aliyotOff"
                )
            ...
            kwargs["selectedNamedEntityText"] = request.GET.get(
                f"namedEntityText{i}", None
            )
            if (
                versionEn
                and not Version().load({"versionTitle": versionEn, "language": "en"})
            ) or (
                versionHe
                and not Version().load({"versionTitle": versionHe, "language": "he"})
            ):
                i += 1
                continue  # Stop processing all panels?
                # raise Http404
    ...
    if sheet == None:
        title = (
            primary_ref.he_normal()
            if request.interfaceLang == "hebrew"
            else primary_ref.normal()
        )
        ...
        if primary_ref.is_book_level():
            ...
            read = _(
                "Read the text of %(book)s online with commentaries and connections."
            ) % {"book": book}
            desc = desc + " " + read if desc else read

        else:
            segmentIndex = (
                primary_ref.sections[-1] - 1 if primary_ref.is_segment_level() else 0
            )
            try:
                enText = _reduce_ranged_ref_text_to_first_section(
                    panels[0]["text"].get("text", [])
                )
                heText = _reduce_ranged_ref_text_to_first_section(
                    panels[0]["text"].get("he", [])
                )
                enDesc = (
                    enText[segmentIndex] if segmentIndex < len(enText) else ""
                )  # get english text for section if it exists
                heDesc = (
                    heText[segmentIndex] if segmentIndex < len(heText) else ""
                )  # get hebrew text for section if it exists
                if request.interfaceLang == "hebrew":
                ...
                desc = (
                    desc[:160].rsplit(" ", 1)[0] + "..."
                )  # truncate as close to 160 characters as possible while maintaining whole word. Append ellipses.

            except (IndexError, KeyError):
                desc = _(
                    "Explore 3,000 years of Jewish texts in Hebrew and English translation."
                )
        ...
        desc = unescape(
            sheet.get(
                "summary",
                _("A source sheet created with Sefaria's Source Sheet Builder"),
            )
        )
    ...
    if (
        len(panels) > 0
        and panels[0].get("refs") == []
        and panels[0].get("mode") == "Text"
    ):
...
def texts_category_list(request, cats):
    ...
    if cats == "recent":
        ...
        cat_string = (
            ...
            else ", ".join([hebrew_term(cat) for cat in cats])
        )
        catDesc = (
            getattr(tocObject, "enDesc", "")
            if request.interfaceLang == "english"
            else getattr(tocObject, "heDesc", "")
        )
        catShortDesc = (
            getattr(tocObject, "enShortDesc", "")
            if request.interfaceLang == "english"
            else getattr(tocObject, "heShortDesc", "")
        )
        catDefaultDesc = _(
            "Read %(categories)s texts online with commentaries and connections."
...
def topics_category_page(request, topicCategory):
    ...
    title = (
        topic_obj.get_primary_title(short_lang)
        + " | "
        + _(
...
def get_search_params(get_dict, i=None):
    ...
    def get_filters(prefix, filter_type):
        return (
            ...
            if get_dict.get(get_param(prefix + filter_type + "Filters", i), "")
            else []
        )
    ...
    return {
        ...
        if get_dict.get(get_param("tvar", i))
        else "",
        "textSort": get_dict.get(get_param("tsort", i), None),
        "textFilters": text_filters,
        "textFilterAggTypes": [
            None for _ in text_filters
...
def override_version_with_preference(oref, request, versionEn, versionHe):
    vpref_dict = get_version_preferences_from_dict(
        oref, request.version_preferences_by_corpus
    )
    if vpref_dict is None:
        return versionEn, versionHe
    for lang, vtitle in vpref_dict.items():
        if Version().load(
            {"versionTitle": vtitle, "language": lang, "title": oref.index.title}
        ):
...
def search(request):
    ...
    return render_template(
        ...
        props,
        {
            "title": (search_params["query"] + " | " if search_params["query"] else "")
            + _("Sefaria Search"),
            "desc": _(
                "Search 3,000 years of Jewish texts in Hebrew and English translation."
            ),
        },
    )
...
def collection_page(request, slug):
    ...
    authenticated = request.user.is_authenticated and collection.is_member(
        request.user.id
    )
    ...
    props["collectionData"] = collection.contents(
        with_content=True, authenticated=authenticated
    )
...
def edit_collection_page(request, slug=None):
    ...
    return render_template(
        request, "edit_collection.html", None, {"initialData": collectionData}
    )
...
def translations_page(request, slug):
    ...
    return render_template(
        ...
        props,
        {
            "title": title_dictionary[slug]["title"]
            if "title" in title_dictionary[slug]
            else "Jewish Texts in " + title_dictionary[slug]["name"]
            if slug in title_dictionary
            else "Jewish Texts in" + " " + slug,
            "desc": title_dictionary[slug]["desc"]
            if "desc" in title_dictionary[slug]
            else "The largest free library of Jewish texts available to read online in Hebrew, "
            + title_dictionary[slug]["name"]
            + ", and English including Torah, Tanakh, Talmud, Mishnah, Midrash, commentaries and more.",
            "noindex": False,
        },
    )
...
def _get_user_calendar_params(request):
    if request.user.is_authenticated:
        ...
        custom = (
            "ashkenazi"  # this is default because this is the most complete data set
        )
...
def saved(request):
    ...
    props = {
        "saved": {
            "loaded": True,
            "items": profile.get_history(
                saved=True, secondary=False, serialized=True, annotate=True, limit=20
            ),
        }
    }
...
def user_history(request):
    if request.user.is_authenticated:
        ...
        uhistory = (
            profile.get_history(
                secondary=False, serialized=True, annotate=True, limit=20
            )
            if profile.settings.get("reading_history", True)
            else []
        )
    else:
...
def updates(request):
    ...
    desc = _(
        "See texts, translations and connections that have been recently added to Sefaria."
    )
...
def sheet_crumbs(request, sheet=None):
    ...
    breadcrumbJsonList = [
        ...
        _crumb(
            2, f"/topics/{main_topic.slug}", main_topic.get_primary_title(short_lang)
        ),
...
def ld_cat_crumbs(request, cats=None, title=None, oref=None):
    ...
    for i, c in enumerate(cats):
        ...
        breadcrumbJsonList += [
            _crumb(nextPosition, "/texts/" + "/".join(cats[0 : i + 1]), name)
        ]
        nextPosition += 1
    ...
    if title:
        ...
        breadcrumbJsonList += [
            _crumb(nextPosition, "/" + title.replace(" ", "_"), name)
        ]
        nextPosition += 1
        ...
        if oref and oref.index_node != oref.index.nodes:
            for snode in oref.index_node.ancestors()[1:] + [oref.index_node]:
                if snode.is_default():
                    continue
                name = (
                    snode.primary_title("he")
                    if request.interfaceLang == "hebrew"
                    else snode.primary_title("en")
                )
                breadcrumbJsonList += [
                    _crumb(nextPosition, "/" + snode.ref().url(), name)
                ]
                nextPosition += 1
        ...
        if oref and getattr(oref.index_node, "depth", None) and not oref.is_range():
            ...
            for i in range(len(oref.sections)):
                if request.interfaceLang == "english":
                    name = (
                        oref.index_node.sectionNames[i]
                        + " "
                        + oref.normal_section(i, "en")
                    )
                else:
                    name = (
                        hebrew_term(oref.index_node.sectionNames[i])
                        + " "
                        + oref.normal_section(i, "he")
                    )
                breadcrumbJsonList += [
                    _crumb(
                        nextPosition, "/" + oref.context_ref(depth - i - 1).url(), name
                    )
                ]
                nextPosition += 1
    ...
    if ref is not None:
        try:
            ...
            if index:
                ref = None
                initJSON = json.dumps(
                    {"mode": "add new", "newTitle": index.contents()["title"]}
                )
    else:
        ...
        if not (request.user.is_staff or user_started_text(request.user.id, title)):
            return render_template(
                ...
                None,
                {
                    ...
                    "content": "The Text Info for %s is locked.<br><br>Please email hello@sefaria.org if you believe edits are needed."
                    % title,
                },
            )
...
def terms_editor(request, term=None):
    ...
    if term is not None:
        ...
        data = (
            existing_term.contents() if existing_term else {"name": term, "titles": []}
        )
    else:
        return render_template(
            request,
...
def modify_bulk_text_api(request, title):
    ...
    if request.method == "POST":
        ...
        version = Version().load(
            {"title": title, "versionTitle": versionTitle, "language": language}
        )
        if version is None:
            return jsonResponse(
                {
        ...
        def modify(uid):
            error_map = tracker.modify_bulk_text(
                uid,
                version,
                t["text_map"],
        ...
        if not request.user.is_authenticated:
            ...
            if not key:
                return jsonResponse(
                    {"error": "You must be logged in or use an API key to save texts."}
                )
            apikey = db.apikeys.find_one({"key": key})
            if not apikey:
...
def texts_api(request, tref):
    ...
    if request.method == "GET":
        ...
        if (
            uref and tref != uref
        ):  # This is very similar to reader.reader_redirect subfunction, above.
            url = "/api/texts/" + uref
            response = redirect(iri_to_uri(url), permanent=True)
            params = request.GET.urlencode()
            response["Location"] += "?%s" % params if params else ""
        ...
        versionEn = request.GET.get("ven", None)
        firstAvailableRef = bool(
            int(request.GET.get("firstAvailableRef", False))
        )  # use first available ref, which may not be the same as oref
        if firstAvailableRef:
            temp_oref = oref.first_available_section_ref()
            oref = (
                temp_oref or oref
            )  # don't overwrite oref if first available section ref fails
        if versionEn:
            versionEn = versionEn.replace("_", " ")
        versionHe = request.GET.get("vhe", None)
        if versionHe:
        ...
        multiple = int(
            request.GET.get("multiple", 0)
        )  # Either undefined, or a positive integer (indicating how many sections forward) or negative integer (indicating backward)
        translationLanguagePreference = request.GET.get(
            "transLangPref", None
        )  # as opposed to vlangPref, this refers to the actual lang of the text
        fallbackOnDefaultVersion = bool(
            int(request.GET.get("fallbackOnDefaultVersion", False))
        )
        ...
        def _get_text(
            ...
            try:
                text["next"] = (
                    oref.next_section_ref().normal()
                    if oref.next_section_ref()
                    else None
                )
                text["prev"] = (
                    oref.prev_section_ref().normal()
            ...
            text["sheets"] = (
                get_sheets_for_ref(tref) if int(request.GET.get("sheets", 0)) else []
            )
            ...
            if layer_name:
                ...
                layer_content = [
                    format_note_object_for_client(n) for n in layer.all(tref=tref)
                ]
    ...
    if request.method == "DELETE":
        ...
        if versionEn:
            versionEn = versionEn.replace("_", " ")
            v = Version().load(
                {"title": tref, "versionTitle": versionEn, "language": "en"}
            )
        ...
        if versionHe:
            versionHe = versionHe.replace("_", " ")
            v = Version().load(
                {"title": tref, "versionTitle": versionHe, "language": "he"}
            )
...
def social_image_api(request, tref):
    ...
    if lang == "bi":
        lang = "en"
    version = (
        request.GET.get("ven", None) if lang == "en" else request.GET.get("vhe", None)
    )
...
def search_autocomplete_redirecter(request):
    ...
    elif object_data is not None and object_data.get("type", "") in {
        "Topic",
        "PersonTopic",
        "AuthorTopic",
    }:
...
def opensearch_suggestions_api(request):
    ...
    ret_data = [query, completions_dict["completions"]]
...
def text_titles_api(request):
    return jsonResponse(
        {"books": library.full_title_list()}, callback=request.GET.get("callback", None)
    )
...
def index_api(request, title, raw=False):
    ...
    if request.method == "GET":
        ...
        i = library.get_index(title).contents(
            raw=raw, with_content_counts=with_content_counts
        )
    ...
    if request.method == "POST":
        ...
        if not request.user.is_authenticated:
            ...
            if not apikey:
                return jsonResponse({"error": "Unrecognized API key."})
            return jsonResponse(
                func(apikey["uid"], Index, j, method="API", raw=raw).contents(raw=raw)
            )
        else:
            ...
            try:
                ...
                if not request.user.is_staff and not user_started_text(
...
def word_count_api(request, title, version, language):
    if request.method == "GET":
        counts = VersionSet(
            {"title": title, "versionTitle": version, "language": language}
        ).word_count()
        resp = jsonResponse(
            {"wordCount": counts}, callback=request.GET.get("callback", None)
        )
        return resp
...
def counts_api(request, title):
    ...
    if request.method == "GET":
        return jsonResponse(
            StateNode(title).contents(), callback=request.GET.get("callback", None)
        )
...
def shape_api(request, title):
    ...
    if request.method == "GET":
        ...
        # Category
            ...
            if len(cat_list) == 1:
                # try as corpus
                indexes = library.get_indexes_in_corpus(
                    cat_list[0], include_dependant=include_dependents, full_records=True
                )
            if len(indexes) == 0:
                cat = library.get_toc_tree().lookup(
                    cat_list
                )  # just used for validating that the cat exists
                if not cat:
                    res = {
                        "error": "No index or category found to match {}".format(title)
                    }
                    return jsonResponse(res, callback=request.GET.get("callback", None))
                indexes = library.get_indexes_in_category_path(
                    cat_list, include_dependant=include_dependents, full_records=True
                )

            res = [
                _simple_shape(jan)
                for index in indexes
                for jan in index.nodes.get_leaf_nodes()
            ]
...
def text_preview_api(request, title):
    ...
    if not oref.index_node.has_children():
        ...
        r = (
            oref.index_node.get_default_child().ref()
        )  # Get ref through ref() to get default leaf node and avoid getting parent node
...
def links_api(request, link_id_or_ref=None):
    ...
    if request.method == "GET":
        ...
        return jsonResponse(
            get_links(
                link_id_or_ref, with_text=with_text, with_sheet_links=with_sheet_links
            ),
            callback,
        )
        ...
        if isinstance(j, list):
            ...
            for i in j:
                try:
                    ...
                    res.append(
                        {
                            "status": "ok. Link: {} | {} Saved".format(
                                retval["ref"], retval["anchorRef"]
                            )
                        }
                    )
                except Exception as e:
                    res.append(
                        {
                            "error": "Link: {} | {} Error: {}".format(
                                i["refs"][0], i["refs"][1], str(e)
                            )
                        }
                    )
    ...
    if request.method == "DELETE":
        ...
        try:
            ...
            if ObjectId.is_valid(link_id_or_ref):
                ...
                return jsonResponse(
                    {
                        "error": "{} is neither a valid Ref nor a valid Mongo ObjectID. {}".format(
                            link_id_or_ref, e
                        )
                    }
                )
        ...
        for l in ls:
            ...
            try:
                ...
                if "error" in retval:
                    ...
                    results.append(
                        {"status": "ok. Link: {} | {} Deleted".format(refs[0], refs[1])}
                    )
            except Exception as e:
                results.append(
                    {
...
def notes_api(request, note_id_or_ref):
    ...
    if request.method == "POST":
        ...
        if not request.user.is_authenticated:
            ...
            def protected_note_post(req):
                resp = format_object_for_client(func(req.user.id, Note, note))
                return resp
    ...
    if request.method == "DELETE":
        ...
        return jsonResponse(tracker.delete(request.user.id, Note, note_id_or_ref))
...
def all_notes_api(request):
    ...
    if private:
        if not request.user.is_authenticated:
            ...
            res = [
                note.contents(with_string_id=True)
                for note in NoteSet({"owner": request.user.id}, sort=[("_id", -1)])
            ]
    else:
    ...
    if request.GET.get("private", False) and request.user.is_authenticated:
        ...
        response = {
            "links": get_links(
                tref,
                with_text=False,
                with_sheet_links=request.GET.get("with_sheet_links", False),
            ),
...
def version_status_api(request):
    ...
    for v in VersionSet():
        try:
            ...
            pass
    return jsonResponse(
        sorted(res, key=lambda x: x["title"] + x["version"]),
        callback=request.GET.get("callback", None),
    )
...
def visualize_links_through_rashi(request):
    ...
    json_file = (
        "../static/files/torah_rashi_torah.json"
        if level == 1
        else "../static/files/tanach_rashi_tanach.json"
    )
    return render_template(
        request, "visualize_links_through_rashi.html", None, {"json_file": json_file}
    )
...
def talmudic_relationships(request):
    ...
    return render_template(
        request, "talmudic_relationships.html", None, {"json_file": json_file}
    )
...
def sefer_hachinukh_mitzvot(request):
    ...
    return render_template(
        request, "sefer_hachinukh_mitzvot.html", None, {"csv": csv_file}
    )
...
def flag_text_api(request, title, lang, version):
    ...
    if not request.user.is_authenticated:
        ...
        if not key:
            return jsonResponse(
                {
                    "error": "You must be logged in or use an API key to perform this action."
                }
            )
        apikey = db.apikeys.find_one({"key": key})
        if not apikey:
        ...
        version = version.replace("_", " ")
        vobj = Version().load(
            {"title": title, "language": lang, "versionTitle": version}
        )
    ...
    if request.method == "GET":
        if not path or path == "index":
            ...
            if not topic:
                ...
                links = topic.link_set(
                    query_kwargs={"linkType": "displays-under", "toTopic": slug}
                )
    ...
    if request.method == "POST":
        ...
        if not request.user.is_authenticated:
            ...
            if not key:
                return jsonResponse(
                    {
                        "error": "You must be logged in or use an API key to add or delete categories."
                    }
                )
            apikey = db.apikeys.find_one({"key": key})
            if not apikey:
            ...
            if not user.is_staff:
                return jsonResponse(
                    {"error": "Only Sefaria Moderators can add or delete categories."}
                )
            uid = apikey["uid"]
            kwargs = {"method": "API"}
        elif request.user.is_staff:
        ...
        if Category().load({"path": j["path"]}):
            return jsonResponse(
                {"error": "Category {} already exists.".format(", ".join(j["path"]))}
            )
        if not Category().load({"path": j["path"][:-1]}):
            return jsonResponse(
                {
                    "error": "No parent category found: {}".format(
                        ", ".join(j["path"][:-1])
                    )
                }
            )
...
def calendars_api(request):
    if request.method == "GET":
        ...
        try:
            ...
            return jsonResponse(
                {"error": "Unknown 'timezone' value: '%s'." % zone_name}
            )
        ...
        try:
            ...
            datetimeobj = datetime.datetime(
                year, month, day, tzinfo=pytz.timezone("UTC")
            )
        except Exception as e:
        ...
        if diaspora not in ["0", "1"]:
            ...
            calendars = get_all_calendar_items(
                datetimeobj, diaspora=diaspora, custom=custom
            )
            return jsonResponse(
                {
                    "date": datetimeobj.date().isoformat(),
    ...
    if request.method == "GET":
        ...
        return jsonResponse(
            parashat_hashavua_and_haftara(
                datetimeobj, request.diaspora, parasha=parasha, ret_type="dict"
            )
        )
...
def terms_api(request, name):
    ...
    if request.method == "GET":
        ...
        if term is None:
            ...
            return jsonResponse(
                term.contents(), callback=request.GET.get("callback", None)
            )
    ...
    if request.method in ("POST", "DELETE"):
        ...
        def _internal_do_post(request, uid):
            ...
            if request.method == "POST":
                ...
                func = (
                    tracker.update if request.GET.get("update", False) else tracker.add
                )
        ...
        if not request.user.is_authenticated:
            ...
            if not key:
                return jsonResponse(
                    {
                        "error": "You must be logged in or use an API key to add, edit or delete terms."
                    }
                )
            apikey = db.apikeys.find_one({"key": key})
            if not apikey:
            ...
            if not user.is_staff:
                return jsonResponse(
                    {"error": "Only Sefaria Moderators can add or edit terms."}
                )
            uid = apikey["uid"]
            kwargs = {"method": "API"}
        elif request.user.is_staff:
...
def get_name_completions(name, limit, ref_only, topic_override=False):
    ...
    completer = (
        library.ref_auto_completer(lang)
        if ref_only
        else library.full_auto_completer(lang)
    )
    object_data = None
    ref = None
    topic = None
    if topic_override:
        topic_set = TopicSet(
            {"titles.text": re.compile(rf"^{re.escape(name)}$", flags=re.IGNORECASE)},
            sort=[("numSources", -1)],
            limit=1,
        )
    ...
    try:
        ...
        if (
            inode.is_virtual
            and inode.parent
            and getattr(inode.parent, "lexiconName", None)
            in library._lexicon_auto_completer
        ):
            base_title = inode.parent.full_title()
            lexicon_ac = library.lexicon_auto_completer(inode.parent.lexiconName)
            t = [
                base_title + ", " + t[1]
                for t in lexicon_ac.items(inode.word)[: limit or None]
            ]
            completions = list(OrderedDict.fromkeys(t))  # filter out dupes
            completion_objects = [
                o for n in completions for o in lexicon_ac.get_data(n)
            ]
        ...
        t = [
            e.base_title + ", " + t[1]
            for t in lexicon_ac.items(e.word)[: limit or None]
        ]
        completions = list(OrderedDict.fromkeys(t))  # filter out dupes
...
def name_api(request, name):
    ...
    if ref:
        ...
        if inode.has_numeric_continuation():
            ...
            d["addressExamples"] = [
                t.toStr("en", 3 * i + 3) for i, t in enumerate(inode._addressTypes)
            ]
            d["heAddressExamples"] = [
                t.toStr("he", 3 * i + 3) for i, t in enumerate(inode._addressTypes)
            ]
    elif topic:
...
def dictionary_completion_api(request, word, lexicon=None):
    ...
    if lexicon is None:
        ...
        result = [
            [r, r] for r in rs
        ]  # ac.title_trie[ac.normalizer(r)][0]["key"] - this was when we wanted the first option with nikud
    else:
        matches = [
            (item[0], x)
            for item in library.lexicon_auto_completer(lexicon).items(word)[:LIMIT]
            for x in item[1]
        ]
...
def stories_api(request, gid=None):
    ...
    if request.method == "GET":
        ...
        if admin_feed:
            ...
            stories = SharedStorySet.for_traits(
                traits, limit=page_size, page=page
            ).contents()
            count = len(stories)
        else:
            stories = UserStorySet.recent_for_user(
                request.user.id, traits, limit=page_size, page=page
            ).contents()
            count = len(stories)
            stories = addDynamicStories(stories, user, page)

        return jsonResponse(
            {"stories": stories, "page": page, "page_size": page_size, "count": count}
        )
        ...
        if not request.user.is_authenticated:
            ...
            if not user.is_staff:
                return jsonResponse(
                    {"error": "Only Sefaria Moderators can add stories."}
                )
...
def addDynamicStories(stories, user, page):
    ...
    if page == 0:
        ...
        if most_recent:
            if getattr(most_recent, "is_sheet", None):
                ...
                stry = TextPassageStoryFactory().generate_from_user_history(
                    most_recent, lead={"en": "Keep Reading", "he": "המשך לקרוא"}
                )
    ...
    if page == 1:
        ...
        if len(saved) > 2:
            ...
            stry = TextPassageStoryFactory().generate_from_user_history(
                saved_item, lead={"en": "Take Another Look", "he": "קרא עוד"}
            )
            stories = [stry.contents()] + stories
...
def updates_api(request, gid=None):
    ...
    if request.method == "GET":
        ...
        if not request.user.is_authenticated:
            ...
            if not user.is_staff:
                return jsonResponse(
                    {"error": "Only Sefaria Moderators can add announcements."}
                )
...
def notifications_api(request):
    ...
    if not request.user.is_authenticated:
        return jsonResponse(
            {"error": "You must be logged in to access your notifications."}
        )

    page = int(request.GET.get("page", 0))
    page_size = int(request.GET.get("page_size", 10))

    notifications = NotificationSet().recent_for_user(
        request.user.id, limit=page_size, page=page
    )
...
def messages_api(request):
    ...
    if request.method == "POST":
        ...
        Notification({"uid": j["recipient"]}).make_message(
            sender_id=request.user.id, message=j["message"]
        ).save()
        return jsonResponse({"status": "ok"})
...
def add_new_topic_api(request):
    if request.method == "POST":
        ...
        t = Topic(
            {
                "slug": "",
                "isTopLevelDisplay": data["category"] == "Main Menu",
                "data_source": "sefaria",
                "numSources": 0,
            }
        )
        ...
        ):  # not Top Level so create an IntraTopicLink to category
            new_link = IntraTopicLink(
                {
                    "toTopic": data["category"],
                    "fromTopic": t.slug,
...
def topics_api(request, topic, v2=False):
    ...
    if request.method == "GET":
        ...
        ref_link_type_filters = set(
            filter(
                lambda x: len(x) > 0,
                request.GET.get("ref_link_type_filters", "").split("|"),
            )
        )
        response = get_topic(
            v2,
        ...
        if topic_data["origCategory"] != topic_data["category"]:
            ...
            origLink = IntraTopicLink().load(
                {
                    "fromTopic": topic_obj.slug,
            ...
            if topic_data["category"] == "Main Menu":
                ...
                child = IntraTopicLink().load(
                    {"linkType": "displays-under", "toTopic": topic_obj.slug}
                )
                if child is None:
                ...
                linkToItself = {
                    "fromTopic": topic_obj.slug,
                    "toTopic": topic_obj.slug,
                    "dataSource": "sefaria",
                    "linkType": "displays-under",
                }
                if (
                    getattr(topic_obj, "numSources", 0) > 0
                    and IntraTopicLink().load(linkToItself) is None
                ):
        ...
        needs_save = (
            False  # will get set to True if isTopLevelDisplay or description is changed
        )

        if (topic_data["category"] == "Main Menu") != getattr(
        ...
        if topic_data["origDescription"] != topic_data["description"] or topic_data.get(
            "origCatDescription", ""
        ) != topic_data.get("catDescription", ""):
            topic_obj.change_description(
                topic_data["description"], topic_data.get("catDescription", "")
            )
            needs_save = True
...
def topic_graph_api(request, topic):
    ...
    if topic_obj is None:
        ...
        topics, links = topic_obj.topics_and_links_by_link_type_recursively(
            linkType=link_type, max_depth=max_depth
        )
        response = {
...
def _topic_data(topic):
    ...
    ref_link_type_filters = _CAT_REF_LINK_TYPE_FILTER_MAP.get(
        cat, ["about", "popular-writing-of"]
    )
    response = get_topic(
        True,
        topic,
        ...
        with_refs=True,
        group_related=True,
        annotate_time_period=False,
        ref_link_type_filters=ref_link_type_filters,
        with_indexes=True,
    )
    return response
...
def global_activity(request, page=1):
    ...
    activity, page = get_maximal_collapsed_activity(
        query=q, page_size=page_size, page=page, filter_type=filter_type
    )

    next_page = page + 1 if page else None
    next_page = "/activity/%d" % next_page if next_page else None
    next_page = (
        "%s?type=%s" % (next_page, filter_type)
        if next_page and filter_type
        else next_page
    )
...
def segment_history(request, tref, lang, version, page=1):
    ...
    version = version.replace("_", " ")
    version_record = Version().load(
        {"title": oref.index.title, "versionTitle": version, "language": lang}
    )
    ...
    next_page = page + 1 if page else None
    next_page = (
        "/activity/%s/%s/%s/%d" % (nref, lang, version, next_page)
        if next_page
        else None
    )
    next_page = (
    ...
    return render_template(
        ...
        None,
        {
            ...
            "versionTitleInHebrew": getattr(
                version_record, "versionTitleInHebrew", version_record.versionTitle
            ),
            "email": email,
            "filter_type": filter_type,
            "next_page": next_page,
...
def chat_message_api(request):
    ...
    if request.method == "GET":
        ...
        messages = MessageSet(
            {"room_id": room_id}, sort=[("timestamp", -1)], limit=limit, skip=skip
        ).client_contents()
...
def user_profile(request, username):
    ...
    desc = _(
        "%(full_name)s is on Sefaria. Follow to view their public source sheets, notes and translations."
...
def profile_api(request):
    ...
    if not request.user.is_authenticated:
        return jsonResponse(
            {"error": _("You must be logged in to update your profile.")}
        )
...
def profile_follow_api(request, ftype, slug):
    if request.method == "GET":
        profile = UserProfile(slug=slug)
        follow_set = (
            FollowersSet(profile.id)
            if ftype == "followers"
            else FolloweesSet(profile.id)
        )
        response = [
            UserProfile(id=uid).to_api_dict(basic=True) for uid in follow_set.uids
        ]
        return jsonResponse(response)
    return jsonResponse({"error": "Unsupported HTTP method."})
...
def profile_upload_photo(request):
    ...
    if request.method == "POST":
        ...
        old_big_pic_filename = GoogleStorageManager.get_filename_from_url(
            profile.profile_pic_url
        )
        old_small_pic_filename = GoogleStorageManager.get_filename_from_url(
        ...
        profile.update(
            {"profile_pic_url": big_pic_url, "profile_pic_url_small": small_pic_url}
        )
        profile.save()
...
def profile_sync_api(request):
    ...
    if request.method == "POST":
        ...
        for field in syncable_fields:
            ...
            if field == "settings":
                settings_time_stamp = field_data.pop(
                    "time_stamp"
                )  # don't save time_stamp as a field of profile
                try:
                    ...
                    settings_time_stamp = (
                        0 if settings_time_stamp is None else int(settings_time_stamp)
                    )
                ...
                for hist in reversed(field_data):
                    if "ref" not in hist:
                        logger.warning(
                            f"Ref not in hist. History item: {hist}. User ID: {request.user.id}"
                        )
                        continue
                    try:
        ...
        if not no_return:
            ...
            if (
                post.get("last_sync", None) == "undefined"
            ):  # in certain rare sitatuations, last_sync is literally 'undefined'. This should be equivalent to sending '0'.
                last_sync = 0
            else:
                last_sync = json.loads(
                    post.get("last_sync", str(profile.last_sync_web))
                )

            uhs = UserHistorySet(
                {"uid": request.user.id, "server_time_stamp": {"$gt": last_sync}},
                hint="uid_1_server_time_stamp_1",
            )
            ret["last_sync"] = now
            ret["user_history"] = [
                uh.contents(for_api=True, annotate=False) for uh in uhs.array()
            ]
...
def delete_user_account_api(request):
    ...
    try:
        ...
        response = jsonResponse(
            {"error": "There was an error deleting the account", "user": user_email}
        )

    EmailMultiAlternatives(
        email_subject,
        email_msg,
        from_email="Sefaria System <dev@sefaria.org>",
...
def saved_history_for_ref(request):
    ...
    if request.method == "GET":
        ...
        return jsonResponse(
            UserHistory.get_user_history(oref=oref, saved=True, serialized=True)
        )
    return jsonResponse({"error": "Unsupported HTTP method."})
...
def user_history_api(request):
    ...
    if request.method == "GET":
        if not request.user.is_authenticated:
            ...
            user = UserProfile(id=request.user.id)
            if (
                "reading_history" in user.settings
                and not user.settings["reading_history"]
                and not saved
            ):
                return jsonResponse([])
            skip = int(request.GET.get("skip", 0))
...
def edit_profile(request):
    ...
    sheets = db.sheets.find(
        {"owner": profile.id, "status": "public"}, {"id": 1, "datePublished": 1}
    ...
    return render_template(
        ...
        None,
        {
            ...
            "lang_names_and_codes": zip(
                [
                    Locale(lang).languages[lang].capitalize()
            ...
            "translation_language_preference": (
                profile is not None
                and profile.settings.get("translation_language_preference", None)
            )
...
def community_preview(request):
    ...
    return community_page(
        request, props={"community": community, "communityPreview": date}
    )
...
def community_reset(request):
    ...
    if MULTISERVER_ENABLED:
        server_coordinator.publish_event(
            "in_memory_cache", "set", ["community-page-data-english", None]
        )
        server_coordinator.publish_event(
            "in_memory_cache", "set", ["community-page-data-hebrew", None]
        )
    ...
    community = get_community_page_items(
        date=date, language=request.interfaceLang, refresh=True
    )
...
def digitized_by_sefaria(request):
    ...
    return render_template(
        request,
        "static/digitized-by-sefaria.html",
        None,
        {
            "texts": texts,
        },
...
def random_ref(categories=None, titles=None):
    ...
    if categories is not None or titles is not None:
        ...
        all_indexes = [
            x
            for x in library.all_index_records()
            if x.title in titles or (x.get_primary_category() in categories)
        ]
    else:
...
def random_text_api(request):
    ...
    response = redirect(
        iri_to_uri("/api/texts/" + random_ref(categories, titles))
        + "?commentary=0&context=0",
        permanent=False,
    )
    return response
    ...
    for my_index in texts:
        if my_index["title"] not in titles:
            if len(my_index["index"]) > 0:
                ...
                to_add = {}
                if (
                    "dependence" in my_index_info
                    and "collective_title" in my_index_info
                    and my_index_info["dependence"] == "Commentary"
                    and lang in bundle_commentaries_langs
                ):
                    if (
                        len(
                            list(
                                filter(
                                    lambda x: True
                                    if x["title"] == my_index_info["collective_title"]
                                    else False,
                                    cur,
                                )
                            )
                        ...
                        try:
                            ...
                            categories_to_add = categories[
                                : categories.index(my_index_info["collective_title"])
                                + 1
                            ]
                            to_add["url"] = "/texts/" + "/".join(categories_to_add)
                        except:
                            print(
                                "failed to find author page for "
                                + my_index_info["collective_title"]
                                + ": "
                                + my_index_info["title"]
                            )
                    ...
                    to_add[
                        "url"
                    ] = f'/{my_index["vstate"][0]["first_section_ref"].replace(":", ".")}?{"ven=" + my_index["versionTitle"] if my_index["language"] == "en" else "vhe=" + my_index["versionTitle"]}&lang=bi'
...
def random_by_topic_api(request):
    ...
    resp = jsonResponse(
        {"ref": tref, "topic": random_topic.contents(), "url": url}, callback=cb
    )
...
def search_wrapper_api(request):
    if request.method == "POST":
        ...
        search_obj = Search(using=es_client, index=j.get("type")).params(
            request_timeout=5
        )
        search_obj = get_query_obj(
            search_obj=search_obj, **{k: v for k, v in list(j.items())}
        )
        response = search_obj.execute()
        if response.success():
            return jsonResponse(
                response.to_dict(), callback=request.GET.get("callback", None)
            )
        return jsonResponse(
            {
...
def serve_static_by_lang(request, page):
    ...
    """
    return render_template(
        request, "static/{}/{}.html".format(request.LANGUAGE_CODE, page), None, {}
    )
...
def annual_report(request, report_year):
    ...
    return render(
        request,
        template_name="static/annualreport.html",
...
def explore(request, topCat, bottomCat, book1, book2, lang=None):
    ...
    template_vars = {
        ...
        "topCat": topCat,
        "bottomCat": bottomCat,
        "topCatTitle": categories[topCat]["heTitle"]
        if request.interfaceLang == "hebrew"
        else categories[topCat]["title"],
        "bottomCatTitle": categories[bottomCat]["heTitle"]
        if request.interfaceLang == "hebrew"
        else categories[bottomCat]["title"],
        "urlRoot": urlRoot,
    }
...
def _get_sheet_tag_garden(tag):
    ...
    if not g:
        g = Garden(
            {
                "key": garden_key,
                "title": "Sources from Sheets Tagged {}".format(tag),
    ...
    return jsonResponse(
        {
            "applinks": {
                "apps": [],
                "details": [
                    {"appID": "{}.{}".format(teamID, bundleID), "paths": ["*"]}
                ],
            }
        }
    )
