...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-i", "--ios", dest="ios", action="store_true", help="Run tests on Appium/iOS"
    )
    parser.add_option(
        "-a",
        "--android",
        dest="android",
        action="store_true",
        help="Run tests on Appium/Android",
    )
    parser.add_option(
        "-c", "--chrome", dest="chrome", action="store_true", help="Run tests on Chrome"
    )
    parser.add_option(
        "-m",
        "--chrome-mobile",
        dest="chrome_mobile",
        action="store_true",
        help="Run tests on Chrome Mobile",
    )
    parser.add_option(
        "-f",
        "--firefox",
        dest="firefox",
        action="store_true",
        help="Run tests on Firefox",
    )
    parser.add_option(
        "-s", "--safari", dest="safari", action="store_true", help="Run tests on Safari"
    )
    parser.add_option(
        "-p",
        "--parallel",
