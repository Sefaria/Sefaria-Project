...
def person_link(person):
    ...
    """
    link = '<a href="/person/{}"><span class="int-en">{}</span><span class="int-he">{}</span></a>'.format(
        person.key, person.primary_name("en"), person.primary_name("he")
    )
    return mark_safe(link)
...
def group_link(group_name):
    return mark_safe(
        "<a href='/groups/%s'>%s</a>" % (group_name.replace(" ", "-"), group_name)
    )
...
def sheet_link(value):
    ...
    if "error" in sheet:
        ...
        safe = "<a href='/sheets/%d' data-id='%d'>%s</a>" % (
            value,
            value,
            strip_tags_func(sheet["title"]),
        )
    return mark_safe(safe)
...
def discussion_link(discussion):
    ...
    if getattr(discussion, "first_ref", None):
        ...
        safe = (
            "<a href='/Genesis.1?layer="
            + discussion.urlkey
            + "'>Unstarted Discussion</a>"
        )
    return mark_safe(safe)
...
def sheet_via_absolute_link(sheet_id):
    return mark_safe(absolute_link('<a href="/sheets/{}">a sheet</a>'.format(sheet_id)))
