...
from sefaria.model.collection import (
    Collection,
    CollectionSet,
...
def get_user_collections_for_sheet(uid, sheet_id):
    ...
    collections = CollectionSet(
        {
            "$or": [
                {"admins": uid, "sheets": sheet_id},
                {"members": uid, "sheets": sheet_id},
            ]
        }
    )
    collections = [collection.listing_contents(uid) for collection in collections]
...
def make_sheet_class_string(sheet):
    ...
    if o.get("numbered", False):
        classes.append("numbered")
    if o.get("boxed", False):
        classes.append("boxed")
...
def view_sheet(request, sheet_id, editorMode=False):
    ...
    sheet_collections = (
        get_user_collections_for_sheet(request.user.id, sheet_id)
        if sheet["owner"] == request.user.id
        else None
    )
    displayed_collection = (
        Collection().load({"slug": sheet["displayedCollection"]})
    ...
    canonical_url = (
        request.get_full_path().replace("?embed=1", "").replace("&embed=1", "")
    )
...
def delete_sheet_api(request, sheet_id):
    ...
    if not request.user.is_authenticated:
        ...
        if not key:
            return jsonResponse(
                {"error": "You must be logged in or use an API key to delete a sheet."}
            )
...
def collections_api(request, slug=None):
    if request.method == "GET":
        ...
        if not request.user.is_authenticated and request.method == "POST":
            ...
            if not key:
                return jsonResponse(
                    {"error": _("You must be logged in to create a new collection.")}
                )
            apikey = db.apikeys.find_one({"key": key})
            if not apikey:
                return jsonResponse({"error": "Unrecognized API key."})
...
def collections_get_api(request, slug=None):
    ...
    collection_obj = Collection().load(
        {"$or": [{"slug": uslug}, {"privateSlug": uslug}]}
    )
    if not collection_obj:
        return jsonResponse({"error": "No collection with slug '{}'".format(slug)})
    is_member = request.user.is_authenticated and collection_obj.is_member(
        request.user.id
    )
    collection_content = collection_obj.contents(
        with_content=True, authenticated=is_member
    )
    ...
    if request.method == "POST":
        ...
        if "slug" in collection_data:
            ...
            if not collection:
                return jsonResponse(
                    {
                        "error": "Collection with slug `{}` not found.".format(
                            collection["slug"]
                        )
                    }
                )
            ...
            collection.save()
        return jsonResponse(
            {"status": "ok", "collection": collection.listing_contents(request.user.id)}
        )
        ...
        if existing:
            ...
            return jsonResponse(
                {"error": "Collection with the slug `{}` does not exist".format(slug)}
            )
...
def collections_inclusion_api(request, slug, action, sheet_id):
    ...
    if not collection.is_member(request.user.id):
        return jsonResponse(
            {"error": "Only members of this collection my change its contents."}
        )
    sheet_id = int(sheet_id)
    ...
    if action == "remove":
        if sheet_id in collection.sheets:
            ...
            if (
                request.user.id == sheet["owner"]
                and sheet.get("displayedCollection", None) == collection.slug
            ):
            ...
            return jsonResponse(
                {
                    "error": "Sheet with id {} is not in this collection.".format(
                        sheet_id
                    )
                }
            )
    if action == "add":
        if sheet_id not in collection.sheets:
            ...
            if request.user.id == sheet["owner"] and not sheet.get(
    ...
    sheet_listing = annotate_user_collections([sheet_to_dict(sheet)], request.user.id)[
        0
    ]
    return jsonResponse(
        {
...
def collections_role_api(request, slug, uid, role):
    ...
    if request.user.id not in collection.admins:
        if not (
            uid == request.user.id and role == "remove"
        ):  # non admins can remove themselves
            return jsonResponse(
                {"error": "You must be a collection owner to change contributor roles."}
            )
    ...
    if (
        uid == request.user.id
        and collection.admins == [request.user.id]
        and role != "admin"
    ):
        return jsonResponse(
            {
...
def collections_invite_api(request, slug, uid_or_email, uninvite=False):
    ...
    if not user.exists():
        ...
        if is_new_member:
            ...
            notification.make_collection_add(
                adder_id=request.user.id, collection_slug=collection.slug
            )
            notification.save()
    ...
    if request.method == "POST":
        if not request.user.is_authenticated:
            ...
            if not key:
                return jsonResponse(
                    {
                        "error": "You must be logged in or use an API key to save.",
                        "errorAction": "loginRedirect",
                    }
                )
            apikey = db.apikeys.find_one({"key": key})
            if not apikey:
        ...
        if apikey:
            if "id" in sheet:
                sheet["lastModified"] = get_sheet(sheet["id"])[
                    "dateModified"
                ]  # Usually lastModified gets set on the frontend, so we need to set it here to match with the previous dateModified so that the check in `save_sheet` returns properly
            user = User.objects.get(id=apikey["uid"])
        else:
            user = request.user
        ...
        if "id" in sheet:
            ...
            if (
                ...
                and not can_edit(user, existing)
                and not can_add(request.user, existing)
            ):
        ...
        if sheet.get("displayedCollection", None):
            ...
            if sheet["displayedCollection"] not in [
                g["slug"] for g in get_user_collections(user.id)
            ]:
                sheet["displayedCollection"] = None
...
def bulksheet_api(request, sheet_id_list):
    if request.method == "GET":
        ...
        response = jsonResponse(
            {
                s["sheet_id"]: s
                for s in sheet_list_to_story_list(request, sheet_id_list, only_public)
            },
            cb,
        )
        return response
...
def user_sheet_list_api(request, user_id, sort_by="date", limiter=0, offset=0):
    ...
    return jsonResponse(
        user_sheets(user_id, sort_by, private=private, limit=limiter, skip=offset),
...
def add_source_to_sheet_api(request, sheet_id):
    ...
    def remove_footnotes(txt):
        ...
        return bleach.clean(
            str(soup),
            tags=Version.ALLOWED_TAGS,
            attributes=Version.ALLOWED_ATTRS,
            strip=True,
        )
    ...
    def get_correct_text_from_source_obj(source_obj, ref_obj, lang):
        ...
        if (
            lang in source_obj
        ):  # if there's actual content passed in, that tkaes precedence as the source text
            ...
            lang_tc = (
                TextChunk(ref_obj, lang, source["version-" + lang])
                if source.get("version-" + lang, None)
                else TextChunk(ref_obj, lang)
            )
...
def sheet_to_story_dict(request, sid):
    ...
    if request.user.is_authenticated:
        d["publisher_followed"] = (
            d["publisher_id"] in following.FolloweesSet(request.user.id).uids
        )
...
def make_sheet_from_text_api(request, ref, sources=None):
    ...
    sheet = make_sheet_from_text(
        ref, sources=sources, uid=request.user.id, generatedBy=None, title=None
    )
...
def export_to_drive(request, credential, sheet_id):
    ...
    user_info_service = build(
        "oauth2", "v2", credentials=credential, cache_discovery=False
    )
    ...
    media = MediaIoBaseUpload(
        BytesIO(html_string), mimetype="text/html", resumable=True
    )
...
def upload_sheet_media(request):
    ...
    if request.method == "POST":
        ...
        if imghdr.what(img_file_in_mem) == "gif":
            img_url = GoogleStorageManager.upload_file(
                img_file_in_mem, f"{request.user.id}-{uuid.uuid1()}.gif", bucket_name
            )
