...
def update_version_title(old, new, language):
    ...
    """
    query = {"versionTitle": old, "language": language}
    db.texts.update(
        query,
        {"$set": {"versionTitle": new}},
        upsert=False,
        multi=True,
        writeConcern={"w": 1},
    )
...
update_version_title(
    default_version_title_cantillaiton, rename_version_title_cantillation, language
)
print("changing old taamei hamikra history")
update_version_title_in_history(
    default_version_title_cantillaiton, rename_version_title_cantillation, language
)
...
update_version_title(
    default_version_title_vowels, rename_version_title_vowels, language
)
...
update_version_title(
    new_version_title_cantillaiton, default_version_title_cantillaiton, language
)
update_version_title_in_history(
    new_version_title_cantillaiton, default_version_title_cantillaiton, language
)
...
update_version_title_in_history(
    new_version_title_vowels, default_version_title_vowels, language
)

print("renaming Leningrad consonants to an old style default name")
update_version_title(
    new_version_title_consonants, default_version_title_consonants, language
)
update_version_title_in_history(
    new_version_title_consonants, default_version_title_consonants, language
)
...
model.VersionSet(
    {
        "versionTitle": rename_version_title_cantillation,
        "language": language,
        "title": {"$nin": ["Genesis", "Exodus", "Leviticus", "Numbers", "Deuteronomy"]},
    }
).delete()
model.VersionSet(
    {"versionTitle": rename_version_title_vowels, "language": language}
).delete()
...
db.texts.update(
    ...
    {"$set": {"status": "locked"}, "$unset": {"priority": ""}},
    upsert=False,
    multi=True,
    writeConcern={"w": 1},
)
...
db.texts.update(
    {"versionTitle": default_version_title_cantillaiton, "language": language},
    {"$set": {"status": "locked", "priority": 1}},
    upsert=False,
    multi=True,
    writeConcern={"w": 1},
)
