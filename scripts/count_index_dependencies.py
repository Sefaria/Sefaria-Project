...
def dep_counts(name):
    ...
    ret = {
        ...
        "version title match commentor": text.VersionSet(
            {"title": {"$regex": ref_patterns["commentor"]}}
        ).count(),
        "version title match commentee": text.VersionSet(
            {"title": {"$regex": commentee_title_pattern}}
        ).count(),
        "history title exact match": history.HistorySet({"title": name}).count(),
        "history title match commentor": history.HistorySet(
            {"title": {"$regex": ref_patterns["commentor"]}}
        ).count(),
        "history title match commentee": history.HistorySet(
            {"title": {"$regex": commentee_title_pattern}}
        ).count(),
    }
    ...
    for pname, pattern in list(ref_patterns.items()):
        ret.update(
            {
                "note match "
                + pname: note.NoteSet({"ref": {"$regex": pattern}}).count(),
                "link match "
                + pname: link.LinkSet({"refs": {"$regex": pattern}}).count(),
                "history refs match "
                + pname: history.HistorySet({"ref": {"$regex": pattern}}).count(),
                "history new refs match "
                + pname: history.HistorySet({"new.refs": {"$regex": pattern}}).count(),
            }
        )
