...
def migrate_versions_of_text(versions, mappings, orig_title, new_title, base_index):
    for i, version in enumerate(versions):
        ...
        for mapping in mappings:
            ...
            add_links_from_text(
                dRef, new_version.language, new_tc.text, new_version._id, 8646
            )
            if i == 0:  # links are the same across versions
                migrate_links_of_ref(orRef, dRef)
            # version history
            text_hist = HistorySet(
                {"ref": {"$regex": orRef.regex()}, "version": version.versionTitle}
            )
            for h in text_hist:
                new_h = h.copy()
...
def migrate_links_of_ref(orRef, destRef):
    query = {
        "$and": [
            {"refs": {"$regex": orRef.regex(), "$options": "i"}},
    ...
    for link in ref_links:
        ...
        curLinkRef = (
            linkRef1 if orRef.contains(linkRef1) else linkRef2
        )  # make sure we manipulate the right ref
        tranlsatedLinkRef = translate_ref(curLinkRef, orRef, destRef)
        newrefs = (
            [tranlsatedLinkRef.normal(), linkRef2.normal()]
            if linkRef1 == curLinkRef
            else [linkRef1.normal(), tranlsatedLinkRef.normal()]
        )
...
def make_link_history_record(curLinkRef, tranlsatedLinkRef):
    link_history = HistorySet(
        {"new.refs": curLinkRef.normal(), "rev_type": {"$regex": "link"}}
    )
    for h in link_history:
        new_h = h.copy()
        new_h.new["refs"] = [
            r.replace(curLinkRef.normal(), tranlsatedLinkRef.normal(), 1)
            for r in h.new["refs"]
        ]
