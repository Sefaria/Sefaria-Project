...
for book in torah:
    parshiot_in_book = [
        "Parashat " + p["sharedTitle"] for p in book.alt_structs["Parasha"]["nodes"]
    ]
        ...
        for ref in refs:
            ...
            writer.writerow(
                [
                    ...
                    "{0:#0{1}x}".format(gematria(text.as_string()), 8).replace(
                        "0x", "#"
                    ),
                    "{0:#0{1}x}".format(gematria(text.as_string()) * 18, 8).replace(
                        "0x", "#"
                    ),
                ]
            )
