...
for o in c:
    dappim += [
        {
            "daf": o["daf"].replace("Eiruvin", "Eruvin"),
            "date": datetime.strptime(o["date"], "%m/%d/%Y"),
        }
    ]
...
stats_lookup = {
    (row["Ref"], datetime.strptime(row["Date"], "%Y-%m-%d")): row for row in rows
}
...
for daf in dappim:
    ...
    for date in dates:
        try:
            days += [
                (
                    int(stats_lookup[(a_side, date)]["Users"]),
                    int(stats_lookup[(b_side, date)]["Users"]),
                )
            ]
