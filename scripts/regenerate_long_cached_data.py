...
def regenerate_version_status_tree():
    for lang in [None, "he", "en"]:
        django_cache(action="set", cache_type="persistent", cache_prefix='version_status_tree_api')(library.simplify_toc)(lang=lang if lang else "")
    if USE_VARNISH:
        for lang in ["he", "en"]:
...
def regenerate_bare_links_api(cat1, cat2):
    ...
    for c1idx in cat1idxs:
        ...
        django_cache(action="set", cache_type="persistent", cache_prefix='bare_link_api')(get_book_link_collection)(book=c1idx, cat=cat2)
    for c2idx in cat2idxs:
        print("bare_link_api:, Book: {}, Category: {}".format(c2idx, cat1))
        django_cache(action="set", cache_type="persistent", cache_prefix='bare_link_api')(get_book_link_collection)(book=c2idx, cat=cat1)


def regenerate_link_count_api(cat1, cat2):
    print("link_count_api:, Category1: {}, Category2: {}".format(cat1, cat2))
    django_cache(action="set", cache_type="persistent", cache_prefix='link_count_api')(get_link_counts)(cat1=cat1, cat2=cat2)
