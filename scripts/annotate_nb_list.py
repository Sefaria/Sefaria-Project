...
with open(
    "data/private/nationbuilder-people-export-2017-09-01-1120.csv", "r"
) as csv_in:
    reader = csv.reader(csv_in)
    with open(
        "data/private/nationbuilder-people-export-2017-09-01-1120-annotated.csv", "wb"
    ) as csv_out:
        ...
        for row in reader:
            ...
            public_sheets_count = db.sheets.find(
                {"owner": profile.id, "status": "public"}
            ).count()
            sheet_views = sum(
                [sheet["views"] for sheet in sheet_list(query={"owner": profile.id})]
            )
