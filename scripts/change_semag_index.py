...
def add_rabbinic(title):
    ...
    for r_count in range(len(rabbinic_nodes)):
        en_name = [x["text"] for x in rabbinic_nodes[r_count].get_titles_object() if x["primary"] is True and x["lang"] == "en"][0]
        he_name = [x["text"] for x in rabbinic_nodes[r_count].get_titles_object() if x["primary"] is True and x["lang"] == "he"][0]
