...
def run_job(test=True, board_id="", idList_mapping={}, members_mapping={}):
    ...
    webpages_without_websites_days = (
        sites_that_may_have_removed_linker_days  # same timeline is relevant
    )
    ...
    sites["Site uses linker but is not whitelisted"] = find_webpages_without_websites(
        test=test,
        hit_threshold=50,
    ...
    flag = 500
    print(
        "Looking for websites where the same Ref appears in at least {} pages...".format(
            flag
        )
    )
    sites["Websites that may need exclusions set"] = find_sites_to_be_excluded_relative(
        relative_percent=3
    )
            ...
            if len(comment) > 0:
                ...
                if not already_on_trello:
                    card = board.create_card(
                        site_name_in_DB, idList_mapping[kind], members_mapping[kind]
                    )
...
class TrelloBoard:
    ...
    def get_lists(self):
        ...
        response = requests.request(
            "GET", board_url, headers={"Accept": "application/json"}
        )
        if response.status_code == 200:
            for l in json.loads(response.content):
                ...
                response = requests.request(
                    "GET", list_url, headers={"Accept": "application/json"}
                )
                if response.status_code == 200:
...
def delete_bad_refs(BOARD_ID, TRELLO_KEY, TRELLO_TOKEN):
    ...
    for list_on_board in json.loads(response.content):
        if (
            list_on_board["name"]
            == "Todo: After setting exclusions, delete old, bad refs and delete web page if it has 0 refs"
        ):
            for card in board.lists[list_on_board["id"]]:
                ...
                for bad_ref in refs:
                    ws = WebPageSet(
                        {"url": {"$regex": domains}, "refs": bad_ref}
                    ).array()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--delete",
        default="no",
        help="Use this option to delete bad refs instead of running the default job.",
    )
    args = parser.parse_args()
    members_mapping = {
        "Linker uninstalled": ["53c2c1b503849ae6a6e56870", "5f0c575790c4a913b3992da2"],
        "Site uses linker but is not whitelisted": [
            "53c2c1b503849ae6a6e56870",
            "5f0c575790c4a913b3992da2",
        ],
    ...
    lists = [
        "Linker uninstalled",
        "Site uses linker but is not whitelisted",
        "Websites that may need exclusions set",
    ]
