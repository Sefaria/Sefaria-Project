...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-v", "--version", help="version title to be copied", default=""
    )
    ...
    if not args.version or not args.language or not args.title:
        ...
        print(
            "Language must be either English ('en') or Hebrew ('he').  The language you specified: "
            + args.language
            + " is not 'he' or 'en'."
        )
    else:
        ...
        for count in range(len(find_strings)):
            find_and_replace_in_text(
                title, vtitle, lang, find_strings[count], replace_strings[count], uid
            )
