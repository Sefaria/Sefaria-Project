...
class ServerTextCopier(object):
    ...
    def load_objects(self):
        ...
        if not self._index_obj:
            raise AttributeError(
                "No record found for {}".format(self._title_to_retrieve)
            )
        self._version_objs = []
        if self._versions_to_retrieve:
            if self._versions_to_retrieve == "all":
                self._version_objs = VersionSet(
                    {"title": self._title_to_retrieve}
                ).array()
            else:
                for version in self._versions_to_retrieve:
                    ...
                    if not vs:
                        print(
                            "Warning: No version object found for  lang: {} version title: {}. Skipping.".format(
        ...
        if self._post_links:
            if self._post_links == 1:  # only manual
                query = {
                    "$and": [
                        {"refs": {"$regex": Ref(self._index_obj.title).regex()}},
                        {
                            "$or": [
                                {"auto": False},
                                {"auto": 0},
                    ...
                    if node_num == len(content_nodes):
                        ...
                        self._make_post_request_to_server(
                            self._prepare_text_api_call(
                                node.full_title(force_update=True), count_after=True
                            ),
                            version_payload,
                        )
                        # except:
            ...
            if flags:
                self._make_post_request_to_server(
                    self._prepare_version_attrs_api_call(
                        ver.title, ver.language, ver.versionTitle
                    ),
                    flags,
                )
    ...
    def _handle_categories(self):
        ...
        try:
            dest_category = requests.get(
                "{}/api/category/{}".format(self._dest_server, "/".join(categories))
            ).json()
        except ValueError:
            return
        ...
        if dest_category.get("error") == "Category not found":
            ...
            for i in range(cat_index + 1, len(categories) + 1):
                ...
                if c is None:
                    raise IndexError(
                        "Necessary category for this index is missing. "
    ...
    def _upload_term(self, name):
        ...
        if t is None:
            raise AttributeError(
                "Necessary Term {} not Present on this Environment".format(name)
            )
        self._make_post_request_to_server(
            "api/terms/{}".format(urllib.parse.quote(name)), t.contents()
        )
    ...
    def _prepare_text_api_call(self, terminal_ref, count_after=False):
        return "api/texts/{}?count_after={}&index_after=0".format(
            urllib.parse.quote(terminal_ref.replace(" ", "_").encode("utf-8")),
            int(count_after),
        )

    def _prepare_version_attrs_api_call(self, title, lang, vtitle):
        return "api/version/flags/{}/{}/{}".format(
            urllib.parse.quote(title),
            urllib.parse.quote(lang),
            urllib.parse.quote(vtitle),
        )
    ...
    def _make_post_request_to_server(self, url, payload):
        ...
        try:
            ...
            if "prof" in full_url:
                filename = "/var/tmp/prof_mdt_{}_{}.txt".format(
                    payload["versionTitle"][:5], payload["language"]
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-v",
        "--versionlist",
        help="pipe separated version list: lang:versionTitle. To copy all versions, simply input 'all'",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=-1,
        type=int,
        help="Enter step size for link posting.  Size of 400 means links are posted 400 at a time.",
    )
