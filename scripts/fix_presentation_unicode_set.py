...
from sefaria.utils.hebrew import (
    decompose_presentation_forms,
    decompose_presentation_forms_in_str,
)
                ...
                if len(chars) > 0:
                    ...
                    print(
                        "[{}] {}:{}.{}.{} - {}".format(
                            ...
                            vs,
                            cm,
                            new_comment.encode("utf-8"),
                        )
                    )
                    print("Characters found ({}): {}".format(len(chars), chars))
                    num_extra_length = sum(
                        [(len(decompose_presentation_forms(x)) - len(x)) for x in chars]
                    )
