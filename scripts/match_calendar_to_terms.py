...
for calendar_item in calendar_items:
    if not is_hebrew(
        hebrew_parasha_name(calendar_item["parasha"]), True
    ) and not Term().load_by_title(calendar_item["parasha"]):
    ...
    term = Term(
        {
            "name": missing_name,
            "titles": [
                {"lang": "en", "text": missing_name, "primary": True},
                {"lang": "he", "text": heb_term, "primary": True},
            ],
            "scheme": "Holidays",
        }
    ).save()
