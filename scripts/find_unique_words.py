...
books = set(
    flatten(
        [
            (library.get_dependant_indices(book_title=r, dependence_type="Commentary"))
            for r in library.get_indexes_in_category("Tanakh")
        ]
    )
)
...
for book in books:
    ...
    text = [
        TextChunk(sectionRef, "he").text for sectionRef in book_index.all_section_refs()
    ]
...
for commentator in commentator_words:
    if len(commentator_words[commentator]) > cutoff_length:
        ...
        special_words = [
            word for word in list(unique_words) if all_words_used[word] < 2
        ]
        print(
            "%s\t%d\t%f\t%d\t%d"
            % (
                commentator,
                len(unique_words),
                len(unique_words) / float(cutoff_length),
                len(special_words),
                len(commentator_words[commentator]),
            )
        )
