...
if __name__ == "__main__":
    ...
    for row in csv_reader:
        id, old, new = row
        id = int(id)
        if id not in actual_sources.keys():
            actual_sources[id] = {}
    ...
    for sheet in sheets:
        if "id" not in sheet.keys():
            ...
            id = sheet["id"]
        if "id" not in sheet.keys() or id not in actual_sources.keys():
            continue
        print id
        sources = sheet.get("sources", [])
        for source_n, source in enumerate(sources):
            if "ref" in source.keys() and source["ref"].startswith("Yad Ramah on Bava Batra"):
