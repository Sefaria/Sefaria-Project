...
def fix_links(commentator):
    ...
    yoreh_ref = Ref(u"Shulchan Arukh, Yoreh De'ah")
    version_title = u"Shulchan Arukh, Yoreh De'ah Lemberg PlaceHolder VersionTitle"
    if version_title == u"Shulchan Arukh, Yoreh De'ah Lemberg PlaceHolder VersionTitle":
        print u"\033[91mUsing the placeholder versionTitle - this needs to be fixed ASAP!!!\033[0m"

    for segment in yoreh_ref.all_segment_refs():
        try:
            if segment.sections[0] % 20 == 1 and segment.sections[1] == 1:
                print segment.sections[0],
        except IndexError:
            pass
        ...
        total_links = sorted([l[0] for l in LinkSet(segment).refs_from(Ref(commentator), as_link=True)],
                             key=link_sort_key)
        soup = BeautifulSoup(u'<root>{}</root>'.format(segment_text.text), 'xml')
        ...
        if len(total_links) != len(total_itags):
            ...
            if len(total_links) != len(total_itags):
                print u"Problem with Ba'er Hetev at {}. Skipping".format(segment.normal())
                continue

        if not is_sorted(total_links, key=link_sort_key):
            print u"Links not in order at {}. Skipping".format(segment.normal())
            continue
        ...
        if is_sorted(total_itags, key=lambda x: int(x['data-order'])):
            ...
            print u"itags out of order at {}".format(segment.normal())


if __name__ == '__main__':
    # make sure index is set up properly
    baer = library.get_index(u"Ba'er Hetev on Shulchan Arukh, Yoreh De'ah")
    baer.collective_title = u"Ba'er Hetev"
    baer.dependence = u"Commentary"
    baer.base_text_titles = [u"Shulchan Arukh, Yoreh De'ah"]
    baer.save()

    fix_links(u"Ba'er Hetev on Shulchan Arukh, Yoreh De'ah")
