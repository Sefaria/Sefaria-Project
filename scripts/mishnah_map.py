...
def compile_refs(row):
    ...
    m_ref = "Mishnah {} {}:{}".format(
        row["Book"], row["Mishnah Chapter"], row["Start Mishnah"]
    )
...
with open("../data/Mishnah Map.csv") as mmap:
    ...
    for row_num, map_row in enumerate(map_rows):
        ...
        ls = LinkSet(
            {
                "refs": {"$all": [refs["m_ref"], refs["t_ref"]]},
        ...
        if ls.count() == 0:
            ...
            if ls.count() == 0:
                ...
                print(
                    "{}, {} exists with incorrect attributes".format(
                        refs["m_ref"], refs["t_ref"]
                    )
                )
                for l in ls:
