...
def compile_refs(row):
    ...
    m_ref = u'Mishnah {} {}:{}'.format(row['Book'], row['Mishnah Chapter'], row['Start Mishnah'])
    if row['Start Mishnah'] != row['End Mishnah']:
        m_ref += u'-{}'.format(row['End Mishnah'])

    t_ref = u'{} {}:{}'.format(row['Book'], row['Start Daf'], row['Start Line'])
    if row['End Daf'] != row['Start Daf']:
        t_ref += u'-{}:{}'.format(row['End Daf'], row['End Line'])
    elif row['Start Line'] != row['End Line']:
        t_ref += u'-{}'.format(row['End Line'])
...
with open('../data/Mishnah Map.csv') as mmap:
    ...
    for row_num, map_row in enumerate(map_rows):
        ...
        if ls.count() == 0:
            counts += 1
            print 'on line {}: '.format(row_num+2),
            ls = LinkSet({'refs': {'$all': [refs['m_ref'], refs['t_ref']]}})
            if ls.count() == 0:
