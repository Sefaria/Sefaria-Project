...
def get_ref_from_link(mishnah_talmud_link):
    mishnah_ref, talmud_ref = (
        mishnah_talmud_link.refs
        if "Mishnah" in mishnah_talmud_link.refs[0]
        else reversed(mishnah_talmud_link.refs)
    )
...
def get_german_text(talmud_ref):
    german_text = talmud_ref.text(
        "en", vtitle="Talmud Bavli. German trans. by Lazarus Goldschmidt, 1929 [de]"
    )
...
def check_uppercase_percentage(talmud_ref):
    ...
    if german_text:
        percent_uppercase = (
            sum(1 for c in german_text if c.isupper()) / len(german_text)
        ) * 100
    else:
...
def phase_one():
    ...
    for link in ls:
        ...
        if talmud_ref.is_range():
            for ref in talmud_ref.range_list():
                generate_data_append_to_list(
                    data_list, ref, mishnah_ref, checking="false-positive"
                )
        else:
            generate_data_append_to_list(
                data_list, talmud_ref, mishnah_ref, checking="false-positive"
            )
...
def generate_map_from_links():
    ...
    csv_list.sort(
        key=lambda x: Ref(
            f"{x['Book']} {x['Mishnah Chapter']}:{x['Start Mishnah']}"
        ).order_id()
    )
...
def false_pos_false_neg_check():
    ...
    generate_csv(
        csv_list,
        ["mishnah_tref", "talmud_tref", "german_text", "issue"],
        "fn_fp_issues",
    )
