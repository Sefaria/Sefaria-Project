...
for author in sorted_public_authors[:20]:
    ...
    print("%s: %d public sheets - www.sefaria.org/profile/%s - %s" % (profile.full_name, author[1], profile.slug, profile.email))

print("\n\nTop Total Sheet Authors")
for author in sorted_authors[:20]:
    profile = UserProfile(id=author[0])
    print("%s: %d sheets - www.sefaria.org/profile/%s - %s" % (profile.full_name, author[1], profile.slug, profile.email))

print("\n\nTop Total Sheet Authors by Views")
for author in sorted_authors_by_views[:20]:
    profile = UserProfile(id=author[0])
    print("%s: %d total sheet views - www.sefaria.org/profile/%s - %s" % (profile.full_name, author[1], profile.slug, profile.email))
