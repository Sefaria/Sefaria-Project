...
with open("spam_emails.csv") as spam_emails, open("spam_outf.csv", "w+") as outf:
    ...
    for index, row in enumerate(csv_reader):
        ...
        if res_get.status_code == 200:
            ...
            id = (
                res_get_data["person"]["id"]
                if "person" in res_get_data
                else res_get_data["id"]
            )
            try:
                headers = {
                ...
                res_put = session.post(
                    f"/api/v1/lists/{SPAM_EMAIL_LIST_ID}/people",
                    data=data,
                    headers=headers,
                )
            ...
            csv_writer.writerow(dict(email=email, status=status))
# corrected emails
with open("corrected_emails.csv") as corrected_emails, open(
    "spam_bademails_outf.csv", "w+"
) as outf:
    ...
    for index, row in enumerate(csv_reader):
        ...
        if "?" in first_name or "?" in last_name:
            ...
            try:
                ...
                if res_get.status_code == 200:
                    res_get_data = res_get.json()
                    first_name = (
                        res_get_data["person"]["first_name"]
                        if "person" in res_get_data
                        else res_get_data["first_name"]
                    )
                    last_name = (
                        res_get_data["person"]["last_name"]
                        if "person" in res_get_data
                        else res_get_data["last_name"]
                    )
                else:
                    status = "Failed - No name provided and email not found in NB"
            except Exception as e:
                status = "Failed - Exception looking up first/last name on nationbuilder {}".format(
                    e
                )
        ...
        if "Failed" not in status:
            try:
                res_get = session.get(
                    f"/api/v1/people/search?first_name={first_name}&last_name={last_name}"
                )
                if res_get.status_code == 200:
                    res_get_data = res_get.json()
                    bad_emails = []
                    for result in res_get_data["results"]:
                        if (
                            result["email"] != email
                            and result["created_at"].startswith("2022-06")
                            and len(result["tags"]) == 1
                        ):
                            bad_emails.append(
                                {"email": result["email"], "nb_id": result["id"]}
                            )  # add email to list
                    if len(bad_emails) == 0:
                        status = (
                            "Failed - No alternate emails found for user email / name"
                        )
                    else:
                        try:
                            headers = {
                            ...
                            print(data)
                            res_put = session.post(
                                f"/api/v1/lists/{BAD_EMAIL_LIST_ID}/people",
                                data=data,
                                headers=headers,
                            )
                            if res_put.status_code == 204:
                                ...
                                for bad_email in bad_emails:
                                    csv_writer.writerow(
                                        dict(
                                            ...
                                            first=first_name,
                                            last=last_name,
                                            incorrect_email=bad_email["email"],
                                            nb_id=bad_email["nb_id"],
                                            status=status,
                                            name_retrieved_from=name_retrieved_from,
                                        )
                                    )
                            else:
