...
for author in sorted_public_authors[:10]:
    ...
    print(
        "%s: %d public sheets - www.sefaria.org/profile/%s"
        % (profile.full_name, author[1], profile.slug)
    )
...
for author in sorted_authors[:10]:
    profile = UserProfile(id=author[0])
    print(
        "%s: %d sheets - www.sefaria.org/profile/%s"
        % (profile.full_name, author[1], profile.slug)
    )
...
for author in sorted_authors_by_views[:10]:
    profile = UserProfile(id=author[0])
    print(
        "%s: %d total sheet views - www.sefaria.org/profile/%s"
        % (profile.full_name, author[1], profile.slug)
    )
