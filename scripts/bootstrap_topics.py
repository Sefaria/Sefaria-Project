...
def autoreconnect_query(collection=None, query=None, proj=None, tries=0):
    if collection:
        try:
            ...
            if tries < 10:
                return autoreconnect_query(
                    collection=collection, query=query, proj=proj, tries=tries + 1
                )
            else:
...
def do_topics(dry_run=False):
    ...
    for t in tqdm(topics, desc="topics"):
        ...
        if len(t.get("en_transliteration", "")) > 0:
            ...
            if len(t.get("en", "")) > 0:
                titles += [{"text": t["en"], "lang": "en"}]
                title_set.add(t["en"])
        elif len(t.get("en", "")) > 0:
            titles += [{"text": t["en"], "lang": "en", "primary": True}]
            title_set.add(t["en"])
        if len(t.get("he", "")) > 0:
            titles += [{"text": t["he"], "lang": "he", "primary": True}]
            title_set.add(t["he"])
        for alt_t in t.get("alt_en", []):
            titles += [{"text": alt_t, "lang": "en"}]
            title_set.add(alt_t)
        for alt_t in t.get("alt_he", []):
            titles += [{"text": alt_t, "lang": "he"}]
            title_set.add(alt_t)
        ...
        for sheet_tag in t.get("source_sheet_tags", []):
            ...
            for title in titles:
                if (
                    title.get("primary", False)
                    and title["text"] != term_prime_titles[title["lang"]]
                    and len(term_prime_titles[title["lang"]]) > 0
                ):
                    # remove primary status from existing title
                    del title["primary"]
                elif (
                    not title.get("primary", False)
                    and title["text"] == term_prime_titles[title["lang"]]
                ):
                    title["primary"] = True
        ...
        for prop in props:
            if len(t.get(prop, "")) > 0:
                final_props[prop] = {"value": t[prop], "dataSource": "talmudic-people"}
        if len(final_props) > 0:
            attrs["properties"] = final_props

        if len(t.get("description", "")) > 0:
            attrs["description"] = {"en": t["description"], "he": None}
        if dry_run:
        ...
        for sheet_tag in t.get("source_sheet_tags", []):
            ...
            if (
                tag_to_slug_map.get(sheet_tag, False)
                and ot.slug != tag_to_slug_map[sheet_tag]
            ):
                print(
                    "TAG TO SLUG ALREADY EXISTS!!!",
                    sheet_tag,
                    ot.slug,
                    tag_to_slug_map[sheet_tag],
                )
            term = Term().load_by_title(sheet_tag)
            term_titles = (
                [sheet_tag]
                if term is None
                else [term_title["text"] for term_title in term.titles]
            )
...
def do_topic_link_types():
    ...
    for edge_type in edge_types:
        ...
        attrs = {
            ...
            "displayName": {"en": edge_type["en name"], "he": edge_type["he name"]},
            "inverseDisplayName": {
        ...
        if len(edge_type["BFO ID"]) > 0:
            ...
            attrs["inverse_alt_ids"] = {
                "bfo": re.findall(r"/([^/]+)$", edge_type["Inverse BFO ID"])[0]
            }
    ...
    tlt = TopicLinkType(
        {
            ...
            "displayName": {"en": "About", "he": "About"},
            "inverseDisplayName": {"en": "Has About", "he": "Has About"},
            "shouldDisplay": True,
    ...
    tlt = TopicLinkType(
        {
            ...
            "displayName": {"en": "Displays Under", "he": "Displays Under"},
            "inverseDisplayName": {"en": "Displays Above", "he": "Displays Above"},
            "shouldDisplay": False,
    ...
    tlt = TopicLinkType(
        {
            ...
            "displayName": {"en": "Related By Sheets", "he": "Related By Sheets"},
            "inverseDisplayName": {
...
def do_data_source():
    ...
    data_sources = [
        {"slug": "talmudic-people", "displayName": "Talmudic People"},
        {"slug": "sefer-haagada", "displayName": "Sefer Haagada"},
        {"slug": "aspaklaria", "displayName": "Aspaklaria"},
        {
            "slug": "aspaklaria-edited-by-sefaria",
            "displayName": "Aspaklaria-edited-by-Sefaria",
        },
        {"slug": "sefaria-users", "displayName": "Sefaria Users"},
        {"slug": "sefaria", "displayName": "Sefaria"},
    ]
...
def do_intra_topic_link(term_to_slug_map, invalid_term_to_slug_map):
    ...
    for edge_type in edge_types:
        ...
        if (
            len(edge_type["Edge Inverse"]) == 0
            or edge_type["Edge Inverse"] == edge_type["Edge"]
        ):
            continue
    ...
    # displays-under links
    for top_term in tqdm(
        TermSet({"scheme": "Tag Category"}), desc="valid intraTopic terms"
    ):
    ...
    for invalid_term, from_slug in tqdm(
        invalid_term_to_slug_map.items(), desc="invalid intraTopic terms"
    ):
        top_topic = Topic().load(
            {"alt_ids._temp_toc_id": fallback_sheet_cat_map[invalid_term]}
        )
        if top_topic is None:
            print(
                "Top Topic Fallback is None",
                invalid_term,
                fallback_sheet_cat_map[invalid_term],
            )
            continue
        tl = IntraTopicLink().load(
            {
...
def do_ref_topic_link(slug_to_sheet_map):
    for l in tqdm(ref_topic_links, desc="refTopic links"):
        ...
        try:
            ...
            pass
    for slug, sheet_id_list in tqdm(
        slug_to_sheet_map.items(), desc="refTopic sheet links"
    ):
...
def set_term_descriptions(topic, en, he, itls):
    ...
    if len(ts) != 1:
        print(
            "{} topics matched for {}: {}".format(
                len(ts), topic, ", ".join([t.slug for t in ts])
            )
        )
        return
    t = ts[0]
...
def import_term_descriptions():
    itls = {
        ...
        for l in IntraTopicLinkSet(
            {"linkType": "is-a", "toTopic": Topic.uncategorized_topic}
        )
    }
...
def pre_dedup_topics():
    ...
    for irow, row in enumerate(dup_full):
        ...
        if irow == len(dup_full) - 1 or len(dup_full[irow + 1]["Key"]) > 0:
            ...
            if (
                num_ps > 1
                or (num_ys == 0 and num_ps > 0)
                or (num_ys > 0 and num_ps == 0)
            ):
                print(
                    "Issue with block with key {} has issues. Row # {}".format(
                        curr_key, irow
                    )
                )
            elif num_ps == 1:
                ...
                dedup_map[p["Slug"]] |= {
                    x["Slug"]
                    for x in filter(lambda x: x["Should Merge"] == "y", curr_block)
                }
            curr_block = []
...
def recat_toc():
    ...
    display_map = {}
    top_level = {
        t.slug: t.get_primary_title("en") for t in TopicSet({"isTopLevelDisplay": True})
    }
...
def renormalize_slugs():
    ...
    for t in tqdm(ts, total=ts.count()):
        title = (
            t.get_primary_title("en")
            if len(t.get_primary_title("en")) > 0
            else t.get_primary_title("he")
        )
        if (
            SluggedAbstractMongoRecord.normalize_slug(title) != t.slug
        ):  # re.search(r'\d+$', t.slug) and
            old_slug = t.slug
...
def recat_law():
    top_law = [
        l.fromTopic
        for l in IntraTopicLinkSet(
            {"linkType": TopicLinkType.isa_type, "toTopic": "halakhah"}
        )
    ]
    name_map = {}
    with open(
        "/Users/nss/Documents/Sefaria-Data/research/knowledge_graph/bootstrapper/final_topic_names.csv",
        "r",
    ) as fin:
        c = csv.DictReader(fin)
...
def export_law_toc():
    def recurse_displays_under(curr_slug="law"):
        ...
        curr_links = curr_topic.link_set(
            "intraTopic", query_kwargs={"linkType": "displays-under"}
        )
        temp_rows = []
        for l in curr_links:
            contents = l.contents()
...
def apply_recat_toc():
    ...
    delete_query = {
        ...
        "$and": [
            {key: {"$ne": lslug}}
            for key in ("fromTopic", "toTopic")
            for lslug in law_slugs
        ],
    }
    find_query = {
        "linkType": "displays-under",
        "or": [{key: lslug} for key in ("fromTopic", "toTopic") for lslug in law_slugs],
    }
...
def find_ambiguous_topics():
    ...
    for k, v in tqdm(ambig.items()):
        ...
        for i, t in enumerate(v):
            row = {
                ...
                "Disambiguation He": "",
                "Other Titles": " | ".join(
                    [tit["text"] for tit in t.titles if not tit.get("primary", False)]
                ),
    ...
    with open("data/ambig_topics.csv", "w") as fout:
        ...
        )  # + ['I{}'.format(i) for i in range(0, max_isa)]
        c.writeheader()
        c.writerows(rows)
...
def more_rabbi_matching_oh_boy():
    ...
    all_link_types = {link_type.slug: link_type for link_type in TopicLinkTypeSet()}
    rows = []
    ...
    with open("data/more_rabbi_matching.csv", "w") as fout:
        c = csv.DictWriter(
            fout,
            ["Slug", "En", "He"] + [lt for lt in unique_link_types] + props + ["Link"],
        )
        c.writeheader()
        c.writerows(rows)
...
def reupdate_descs():
    ...
    for row in rows:
        ...
        if len(row["Copy"]) == 0:
            continue
        setattr(t, "description", {"en": row["Copy"], "he": row["Hebrew Copy"]})
        t.save()
...
def reupdate_titles():
    ...
    for row in rows:
        ...
        for ititle, title in enumerate(t.titles):
            ...
            if (
                (not title_exists)
                and (title["text"] == row["New En"].strip())
                and (title["lang"] == "en")
            ):
                title_exists = True
                title_index = ititle
        ...
        if not title_exists:
            t.titles += [{"primary": True, "text": row["New En"].strip(), "lang": "en"}]
    ...
    for l in tqdm(rtls, total=rtls.count()):
        ...
        try:
            title_lang = (
                "english"
                if re.search(
                    r"[a-zA-Z]", re.sub(r"<[^>]+>", "", sheet.get("title", "a"))
                )
                is not None
                else "hebrew"
            )
    ...
    for topic in tqdm(ts, total=ts.count()):
        ...
        for sheet_link in sheets:
            ...
            for other_topic_slug in other_topic_slug_set:
                ...
                if intra_topic_link:
                    total_links_in_common += getattr(intra_topic_link, "order", {}).get(
                        "linksInCommon", 0
                    )
            avg_in_common = (
                1
                if len(other_topic_slug_set) == 0
                else (total_links_in_common / len(other_topic_slug_set)) + 1
            )
...
def supyo():
    ...
    for l in tqdm(sheet_links, total=sheet_links.count()):
        ...
        if sheet["status"] != "public":
            deletes.add(l._id)
    db.topic_links.bulk_write([DeleteOne({"_id": _id} for _id in deletes)])
...
def add_gens_to_rabs():
    ...
    rabbi_topics = {
        t.fromTopic
        for t in IntraTopicLinkSet(
            {
                "linkType": "is-a",
                "$or": [{"toTopic": "mishnaic-people"}, {"toTopic": "talmudic-people"}],
            }
        )
    }
...
def set_top_holidays():
    ...
    current_slugs = IntraTopicLinkSet(
        {"linkType": "displays-under", "toTopic": "holidays"}
    ).distinct("fromTopic")
