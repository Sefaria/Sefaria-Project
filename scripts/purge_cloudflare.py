...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--files", nargs="*", type=str, default=[], help="list of files to purge"
    )
    parser.add_argument(
        "-t",
        "--timestamp",
        help="unix timestamp to purge files that are newer than",
        type=int,
        default=None,
    )
    ...
    if args.files:
        ...
        time_str = datetime.fromtimestamp(int(args.timestamp)).strftime(
            "%Y-%m-%d %H:%M:%S"
        )  # this also serves to assert the timestamp is valid
        ...
        if USE_CLOUDFLARE:
            ...
            logger.info(
                "Files to purge: {}".format(
                    get_directory_content("static", modified_after=args.timestamp)
                )
            )
    else:
