...
for row in reader:
    name = row[0]
    users_by_name = (
        [u["id"] for u in db.profiles.find({"organization": str(name)})] if name else []
    )

    domain = row[1]
    users_by_domain = (
        [u.id for u in User.objects.filter(email__endswith=domain)] if domain else []
    )
...
print(
    "%d total users found matching either domain or school name." % len(all_users_found)
)
