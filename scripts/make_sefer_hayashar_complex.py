...
def make_map_for_simple_to_complex(en_chapters, he_chapters):
    ...
    for i in range(25):
        ...
        if count > end_of_intro:
            mappings[
                "Sefer HaYashar 1:{}".format(count)
            ] = "Sefer HaYashar, {} {}".format(en_chapters[1], count - end_of_intro)
        else:
            mappings[
                "Sefer HaYashar 1:{}".format(count)
            ] = "Sefer HaYashar, {} {}".format(en_chapters[0], count)

    for count, ch_title in enumerate(en_chapters):
        if count not in [0, 1, 19, 20, 21]:
            mappings["Sefer HaYashar {}".format(count)] = "Sefer HaYashar, {}".format(
                en_chapters[count]
            )
