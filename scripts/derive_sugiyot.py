...
class SugyaDeriver(object):
    ...
    def __init__(self, title):
        ...
        self.all_ranges = (
            self.get_all_ranges()
        )  # [{"ref":ref, "type":"Sugya" or "Mishnah"}]
        self.sugya_range_refs = [
            d["ref"] for d in self.all_ranges if d["type"] == "Sugya"
        ]
    ...
    def get_mishnah_pairs(self):
        ...
        ls = LinkSet(
            {"type": "mishnah in talmud", "refs": {"$regex": "^" + self.title}}
        )
        for l in ls:
    ...
    def get_all_ranges(self):
        ...
        while text_remains:
            ...
            msh_is_next = (
                next_msh.starting_ref().precedes(next_sug) and not hit_last_msh
            )
            ...
            if msh_is_next:
                ...
                if current_start:
                    if self.is_perek_start(next_msh):
                        ...
                        current_range = current_start.to(
                            next_msh.prev_segment_ref().prev_segment_ref()
                        )
                    else:
                ...
                if msh_indx + 1 < len_mshs:
                    ...
                    if (
                        self.is_perek_start(msh_after)
                        and next_msh.ending_ref().distance(msh_after.starting_ref(), 2)
                        == 2
                    ):
                        next_msh = msh_after
                        results += [{"ref": next_msh, "type": "Mishnah"}]
                ...
                results += [
                    {
                        "ref": current_start.to(next_sug.prev_segment_ref()),
                        "type": "Sugya",
                    }
                ]
                ...
                if sug_indx >= len_sugs:
                    results += [
                        {
                            "ref": current_start.to(
                                current_start.last_segment_ref().prev_segment_ref()
                            ),
                            "type": "Sugya",
                        }
                    ]
