...
from sefaria.helper.nationbuilder import (
    create_person,
    delete_from_nationbuilder_if_spam,
    get_everyone,
...
def add_nationbuilder_id_to_mongo(mongo_only):
    ...
    for nationbuilder_user in nationbuilder_get_all(get_everyone):
        ...
        if user_profile.id != None:  # has user profile
            ...
            if (
                User.objects.get(id=user_profile.id).is_active == False
            ):  # delete spam users
                if not mongo_only:
...
def add_profiles_to_nationbuilder(gt=0):
    ...
    for profile in db.profiles.find(
        {"nationbuilder_id": {"$exists": False}, "id": {"$gt": gt}}
    ):
                ...
                try:
                    ...
                    if res_get.status_code == 200:
                        ...
                        try:
                            ...
                            user_profile.save()
                            print(
                                "added user with nationbuilder_id: "
                                + str(nationbuilder_id)
                            )
                        except:
                    ...
                    session = get_nationbuilder_connection()
                    print(
                        "Trying again to create nationbuilder user of mongo user id: {}. Attempts: {}. Exception: {}".format(
                            user_profile.id, attempt + 1, e
                        )
                    )
            else:
                session.close()
