...
c = create_category(
    ["Talmud", "Bavli", "Commentary on Minor Tractates"],
...
for kid in tc.children:
    c = create_category(
        ["Talmud", "Bavli", "Commentary on Minor Tractates", kid.get_primary_title()]
    )
...
c = create_category(
    ["Talmud", "Bavli", "Commentary", "Daf Shevui"], en="Daf Shevui", he="דף שבועי"
)
ts = [
...
move_index_into(
    "A New Israeli Commentary on Pirkei Avot", ["Mishnah", "Commentary"]
)  # deeper cat?
...
for t in ["The Antiquities of the Jews", "The War of the Jews", "Against Apion"]:
...
cat_duties = create_category(
    ["Jewish Thought", "Rishonim", "Duties of the Heart"],
    "Duties of the Heart",
    "חובות הלבבות",
)
books_duties = ["Duties of the Heart", "Duties of the Heart (abridged)"]
...
cat_kook = create_category(
    ["Jewish Thought", "Modern", "Rav Kook"], "Rav Kook", "כתבי הרב קוק"
)
books_kook = [
    "Orot",
    "Orot HaKodesh",
...
c = Category().load(
    {"path": ["Jewish Thought", "Modern", "Works of Eliezer Berkovits"]}
)
...
for t in ["Tov haLevanon", "Pat Lechem", "Marpeh la'Nefesh"]:
    move_index_into(t, p)
...
cat_mitzvot = create_category(
    ["Halakhah", "Sifrei Mitzvot"], "Sifrei Mitzvot", "ספרי מצוות"
)
books_mitzvot = [
...
cat_shm = create_category(
    ["Halakhah", "Sifrei Mitzvot", "Sefer HaMitzvot"], "Sefer HaMitzvot", "ספר המצוות"
)
move_index_into("Sefer HaMitzvot", cat_shm)

cat_shm_com = create_category(
    ["Halakhah", "Sifrei Mitzvot", "Sefer HaMitzvot", "Commentary"]
)
on_sefer_hamitzvot = [
...
cat_chp = create_category(
    ["Halakhah", "Modern", "Contemporary Halakhic Problems"],
    en="Contemporary Halakhic Problems",
...
cat_gm = create_category(
    ["Halakhah", "Modern", "Gray Matter"], en="Gray Matter", he="גריי מאטר"
)
books_gm = [
    "Gray Matter I",
    "Gray Matter II",
...
for ind in IndexSet({"categories": ["Halakhah", "Tur and Commentaries"]}):
    ...
    ind.categories = (
        ["Halakhah", "Tur"]
        if ind.get_title("en") == "Tur"
        else ["Halakhah", "Tur", "Commentary"]
    )
    print("Moving - " + ind.get_title() + " to " + str(ind.categories))
    ind.save(override_dependencies=True)
...
move_index_into(
    "Biur Halacha", ["Halakhah", "Shulchan Arukh", "Commentary", "Mishnah Berurah"]
)

# Move categories of Kesef Mishnah to correct place
new_parent = create_category(
    ["Halakhah", "Mishneh Torah", "Commentary", "Kessef Mishneh"]
)
cs = CategorySet(
    {
        "$and": [
...
for cat, books in [(r_cat, r_books), (a_cat, a_books), (m_cat, m_books)]:
    for book in books:
        move_index_into(book, cat)
...
c = create_category(
    ["Chasidut", "Other Chasidut Works"], "Other Chasidut Works", "ספרי חסידות נוספים"
)
for t in [
    "Flames of Faith",
    "Agra DeKala",
...
for t in ["Or Neerav", "Pardes Rimonim"]:
    move_index_into(t, c)

c = create_category(
    ["Kabbalah", "Arizal / Chaim Vital"],
    "Arizal / Chaim Vital",
    'כתבי האר"י - חיים ויטאל',
)
for t in ["Pri Etz Chaim", "Sefer Etz Chaim", "Sha'ar HaGilgulim", "Shaarei Kedusha"]:
    move_index_into(t, c)

c = create_category(
    ["Kabbalah", "Other Kabbalah Works"], "Other Kabbalah Works", "ספרי קבלה נוספים"
)
for t in [
    "Pri Etz Hadar",
...
c = create_category(
    ["Responsa", "Acharonim", "Shoel uMeshiv"], en="Shoel uMeshiv", he="שואל ומשיב"
)
ts = [
...
c = create_category(
    ["Responsa", "Modern", "B'Mareh HaBazak"],
    en="B'Mareh HaBazak",
    he='שו"ת במראה הבזק',
)
mb = {
    "B'Mareh HaBazak Volume I": 1,
...
move_category_into(
    ["Tanakh", "Commentary", "Malbim Beur Hamilot"], ["Tanakh", "Commentary", "Malbim"]
)

ri_cat = create_category(
    ["Tanakh", "Rishonim on Tanakh"],
    "Rishonim on Tanakh",
    "ראשונים על התנ״ך",
    searchRoot="Tanakh Commentary",
)
ah_cat = create_category(
    ["Tanakh", "Acharonim on Tanakh"],
    "Acharonim on Tanakh",
    "אחרונים על התנ״ך",
    searchRoot="Tanakh Commentary",
)
mo_cat = create_category(
    ["Tanakh", "Modern Commentary on Tanakh"],
    "Modern Commentary on Tanakh",
    "פירושים מודרניים על התנ״ך",
    searchRoot="Tanakh Commentary",
)
chida_cat = create_category(
    ["Tanakh", "Acharonim on Tanakh", "Chida"], "Chida", "חיד״א"
)
rr_cat = create_category(
    ["Tanakh", "Modern Commentary on Tanakh", "Redeeming Relevance"],
    "Redeeming Relevance",
    "פדיון הרלוונטיות",
)
ar_cat = create_category(
    ["Tanakh", "Modern Commentary on Tanakh", "Avraham Remer"],
    "Avraham Remer",
    "אברהם רמר",
)
...
ri_cat = create_category(
    ...
    "Rishonim on Mishnah",
    "ראשונים על המשנה",
    searchRoot="Mishnah Commentary",
)
ah_cat = create_category(
    ["Mishnah", "Acharonim on Mishnah"],
    "Acharonim on Mishnah",
    "אחרונים על המשנה",
    searchRoot="Mishnah Commentary",
)
mo_cat = create_category(
    ["Mishnah", "Modern Commentary on Mishnah"],
...
move_index_into(
    "Motar Kinnim", ah_cat
)  # Don't want to move the category -  deleted below.
...
mo_cat = create_category(
    ["Talmud", "Bavli", "Modern Commentary on Talmud"],
    "Modern Commentary on Talmud",
    "פירושים מודרניים על התלמוד",
    searchRoot="Talmud Commentary",
)
rc_cat = create_category(
    ["Talmud", "Bavli", "Rishonim on Talmud", "Rosh", "Commentary"]
)

groups = [(ri, ri_cat), (ah, ah_cat), (co, mo_cat), (rc, rc_cat)]
...
move_index_into(
    "Yad Ramah on Bava Batra", ["Talmud", "Bavli", "Rishonim on Talmud", "Yad Ramah"]
)
