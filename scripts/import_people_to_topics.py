...
from sefaria.model.person import (
    Person,
    PersonRelationship,
    PersonRelationshipSet,
    PersonSet,
)
...
def create_csvs_to_match():
    ...
    for key, value in by_type.items():
        ...
        for person in tqdm(value, desc=key):
            ...
            try:
                era_name = (
                    person.get_era().primary_name("en")
                    if key == "author"
                    else person.get_generation().primary_name("en")
                )
            ...
            name_pattern = rf"^({'|'.join(name_pattern_list)})$"
        ...
        with open(f'data/person_matching_{key.replace("/", "-")}.csv', "w") as fout:
            c = csv.DictWriter(
                fout,
                ["En", "He", "Era", "Bio", "Equivalent Slug"]
                + [f"Slug {i+1}" for i in range(max_poss_slugs)],
            )
            c.writeheader()
            c.writerows(rows)
    ...
    TopicDataSource(
        {
            ...
            "displayName": {"en": "Person Collection", "he": "Person Collection"},
        }
    ).save()
    ...
    with open(f"{BASE_PATH}/Person Topic Matching - Authors.csv", "r") as fin:
        ...
        for irow, row in enumerate(c):
            ...
            if len(book_slugs) > 1:
                ...
                for title in main_book.titles:
                    if title["text"] not in dont_need_disambig:
                        title["disambiguation"] = {"en": "Book", "he": "ספר"}
                main_book.save()
            ...
            if person is None:
                ...
                person = ps.array()[0]
            main_author_dict = {
                "slug": person.primary_name("en")
                if len(person.primary_name("en")) > 0
                else person.primary_name("he"),
                "titles": [],
...
def refactor_authors_on_indexes():
    ...
    for i in tqdm(indexes, total=indexes.count()):
        ...
        for a in i.authors:
            ...
            assert (
                len(ts) == 1
            ), f"author {a} has {len(ts)} topics. they are {', '.join([t.slug for t in ts])}"
    ...
    TopicLinkType(
        {
            ...
            "displayName": {"en": "Grandparent", "he": "סב"},
            "inverseDisplayName": {"en": "Grandchild", "he": "נכד"},
            "pluralDisplayName": {"en": "Grandparents", "he": "סבים"},
            "inversePluralDisplayName": {"en": "Grandchildren", "he": "נכדים"},
            "shouldDisplay": True,
    ...
    Topic(
        {
            ...
            "titles": [
                {"text": "group of rishon people", "lang": "en", "primary": True}
            ],
            "shouldDisplay": False,
        }
    ).save()
    ...
    for rel in PersonRelationshipSet():
        if rel.from_key in authors or rel.to_key in authors:
            ...
            try:
                to_topic, from_topic = (
                    (from_topic, to_topic)
                    if rel.type in flip_link_dir
                    else (to_topic, from_topic)
                )
                IntraTopicLink(
                    {
                        "toTopic": to_topic,
...
def set_description_published():
    for t in TopicSet(
        {"description": {"$exists": True}, "alt_ids.old-person-key": {"$exists": True}}
    ):
    ...
    Topic(
        {
            ...
            "titles": [
                {"text": "Talmudic Figures", "lang": "en", "primary": True},
                {"text": "דמויות מהתלמוד", "lang": "he", "primary": True},
            ],
    ...
    Topic(
        {
            "slug": "authors",
            "titles": [
                {"text": "Authors", "lang": "en", "primary": True},
                {"text": "מחברים", "lang": "he", "primary": True},
            ],
            "isTopLevelDisplay": True,
            "displayOrder": 101,
        }
...
def find_popular_writings(top_n, min_pr):
    ...
    tlt = {
        ...
        "displayName": {"en": "Top Citations", "he": "ציטוטים עיקריים"},
        "inverseDisplayName": {"en": "Has Popular Writing", "he": ""},
        "shouldDisplay": True,
        "inverseShouldDisplay": False,
        "validTo": ["people", "group-of-people"],
    }
...
def percent_refs_translated(percent):
    rds = sorted(
        {rd.ref: rd.pagesheetrank for rd in RefDataSet()}.items(),
        key=lambda x: x[1],
        reverse=True,
    )
    total = len(rds)
    ...
    for ref, pr in tqdm(rds[:cutoff]):
        try:
            ...
            continue
    print(
        f"Num translated for {percent}% cutoff - {num_translated}/{cutoff} = {num_translated/cutoff}%"
    )
...
def get_wikidata_entries():
    ...
    good_guys = [
        t.alt_ids["wikidata"] for t in TopicSet({"alt_ids.wikidata": {"$exists": True}})
    ]
    out = {"entities": {}}
...
def get_wikipedia_links_for_wikidata_ids():
    try:
        ...
        pass
    TopicDataSource(
        {"slug": "wikidata", "displayName": {"en": "Wikidata", "he": "Wikidata"}}
    ).save()
    good_guys = [
        (t.alt_ids["wikidata"], t)
        for t in TopicSet({"alt_ids.wikidata": {"$exists": True}})
    ]
...
def refactor_isas():
    for p in AuthorTopicSet():
        linkset = p.link_set(
            query_kwargs={"linkType": "is-a", "generatedBy": "import_people_to_topics"}
        )
        for l in linkset:
