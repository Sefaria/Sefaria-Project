...
all_sheet_users_data, all_sheet_users = _(
    db.sheets.aggregate([{"$sortByCount": "$owner"}])
)
public_sheets_data, public_sheets_users = _(
    db.sheets.aggregate([{"$match": {"status": "public"}}, {"$sortByCount": "$owner"}])
)
all_notes_data, all_notes_users = _(db.notes.aggregate([{"$sortByCount": "$owner"}]))
bookmark_data, bookmark_users = _(
    db.user_history.aggregate([{"$match": {"saved": True}}, {"$sortByCount": "$uid"}])
)
followers_data, followers_users = _(
    db.following.aggregate([{"$sortByCount": "$follower"}])
)
