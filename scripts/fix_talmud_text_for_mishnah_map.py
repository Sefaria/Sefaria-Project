...
with open(filename, "rb") as csvfile:
    ...
    for row in csv.reader(csvfile):
        d = {
            "line": row,
            "orig_ref": Ref(
                "{} {}:{}-{}:{}".format(row[0], row[4], row[5], row[6], row[7])
            ),
            "new_ref": None,
        }
...
for k, g in groupby(mishnah_map, lambda d: d["orig_ref"].index.title):
    ...
    for mishnah in g:
        ...
        booklist.append({"ref": ref.starting_ref(), "type": "Mishnah"})
        booklist.append({"ref": ref.ending_ref().next_segment_ref(), "type": "Gemara"})
                ...
                if matni_re.match(tc.text):
                    if not matni_re.match(tc.text).group(3):
                        ...
                        try:
                            splc = (
                                SegmentSplicer()
                                .splice_this_into_next(current["ref"])
                                .bulk_mode()
                            )
                            if live:
                                splc.execute()
                                ...
                                print(
                                    "Merging bare Mishnah at {} into next".format(
                                        current["ref"].normal()
                                    )
                                )
                            change_made = True
                            ...
                            print(
                                "(mf) Failed to splice {} into next: {}".format(
                                    current["ref"].normal(), e
                                )
                            )
                else:
                    print(
                        "(m0) Did not match mishnah {}".format(current["ref"].normal())
                    )
                    ...
                    if not gemarah_re.match(tc.text).group(3):
                        ...
                        try:
                            ...
                            if live:
                                ...
                                print(
                                    "Merging bare Gemara at {} into next".format(
                                        current["ref"].normal()
                                    )
                                )
                            review_map(splc)
                            ...
                            print(
                                "(gf) Failed to splice {} into next: {}".format(
                                    current["ref"].normal(), e
                                )
                            )
                else:
                    print(
                        "(g0) Did not match 'Gemara' in {}".format(
                            current["ref"].normal()
                        )
                    )
            else:
