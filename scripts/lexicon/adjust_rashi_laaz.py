...
def create_ngrams(input_words, n):
    ...
    for k in range(1, n + 1):
        gram_list += [
            " ".join(input_words[i : i + k]) for i in range(len(input_words) - k + 1)
        ]
    return gram_list
...
def find_closest_match(
    text_rows, word, default_compare=True, filter_words_with_quotation_marks=True
):
    ...
    for row in text_rows:
        ...
        if len(text_words):
            matched_word, score = process.extractOne(
                word.replace('"', ""), text_words, processor=laaz_process, scorer=scorer
            )
    ...
    if (
        ...
        and len(word)
        and default_compare
        and float(max(len(top_res[2]), len(word))) / min(len(top_res[2]), len(word))
        >= 2
    ):
...
for entry in laaz_rashi_entries:
    ...
    if only_in_print_str in entry.content["notes"] or all(
        s not in entry.content["notes"] for s in not_in_print_str
    ):
        ...
        if not best_match:
            ...
            best_match = find_closest_match(
                filtered_text_rows,
                entry.headword,
                filter_words_with_quotation_marks=False,
            )
        if best_match:
            print(
                "{}\t[{}]\t{}\t{}\t{}\t{}".format(
                    entry.catane_number.encode("utf-8"),
        ...
        print(
            "{}\t[{}]\t{}\t{}\t{}\t{}".format(
                ...
                entry.headword.encode("utf-8"),
                oref.normal(),
                "Does not appear in print versions",
                -1,
            )
        )
