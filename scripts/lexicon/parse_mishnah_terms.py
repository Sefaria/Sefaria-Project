...
def extract_form_tuples(csv_row):
    ...
    forms += [
        (x.strip(), "heb" if is_hebrew(x) else "eng")
        for x in csv_row[4].split(",")
        if len(x)
    ]
    return forms
...
with open("/var/tmp/HTS3.csv", "rb") as csvfile:
    ...
    if not hts_lexicon:
        hts_lexicon = Lexicon(
            {"name": lexicon_name, "language": "heb.mishnaic", "to_language": "eng"}
        )
        hts_lexicon.save()
    ...
    for entry in lex_csv:
        ...
        hts_entry = LexiconEntry().load(
            {"headword": dict_entry["headword"], "parent_lexicon": lexicon_name}
        )
        ...
        for form in forms:
            if form[0] and form[0] != "":
                ...
                create_word_form(
                    form[0],
                    form[1],
                    [{"headword": entry[1], "parent_lexicon": lexicon_name}],
                )
