...
class WLCStrongWordFormBookParser(object):
    def __init__(self, parent_parser, book):
        ...
        self.strip_cantillation_vowel_regex = re.compile(
            r"[\u0591-\u05bd\u05bf-\u05c5\u05c7]", re.UNICODE
        )
        self.strip_cantillation_regex = re.compile(
            r"[\u0591-\u05af\u05bd\u05bf\u05c0\u05c4\u05c5]", re.UNICODE
        )
        self.strong_num_regex = re.compile(r"([0-9]+)[+]?$")
        xmlp = ET.parse(
            self.parent_parser.get_morphhb_file(self.xml_book_name + ".xml")
        )
        self.xml_contents = xmlp.getroot().find(
            "strong:osisText/strong:div[@type='book']", self.namespace
        )

    def iterate_over_text(self):
        for chap_num, chapter_node in enumerate(
            self.xml_contents.findall("strong:chapter", self.namespace), 1
        ):
            for v_num, verse_node in enumerate(
                chapter_node.findall("strong:verse", self.namespace), 1
            ):
                # print "verse ", v_num, ": ", verse_node.get('osisID').encode('utf-8')
                verse = verse_node.get("osisID")
                verse_ref = Ref(
                    verse.replace(".", " ").replace(self.xml_book_name, self.book)
                ).normal()
                verse_elems = verse_node.findall(".//", self.namespace)
                word_iter = [
                    v
                    for v in verse_elems
                    if self.get_prefixless_tagname(v) == "w"
                    or (
                        self.get_prefixless_tagname(v) == "seg"
                        and v.get("type") == "x-maqqef"
                    )
                ]
                ...
                while not done_looping:
                    try:
                        ...
                        if self.get_prefixless_tagname(word) == "w" and "+" in word.get(
                            "lemma"
                        ):
                            word2 = next(word_iter)
                            if (
                                self.get_prefixless_tagname(word2) == "seg"
                                and word2.get("type") == "x-maqqef"
                                and word2.text == "־"
                            ):
                                delim = "־"
                                word2 = next(word_iter)
    ...
    def reevaluate_word_form(self, word1, word2, ref, delim=" "):
        ...
        if strong_number1 != strong_number2:
            print(
                "{} {} strong #s didnt match {} {}".format(
                    word1, word2, strong_number1, strong_number2
                )
            )
            return
        word_form_text1 = unicodedata.normalize(
            "NFC", self._make_vowel_text(self._strip_wlc_morph_notation(word1.text))
        )
        word_form_text2 = unicodedata.normalize(
            "NFC", self._make_vowel_text(self._strip_wlc_morph_notation(word2.text))
        )
        ...
        self.delete_old_word_form_mappings(
            strong_entry, word_form_text1, word_form_text2, ref
        )
    ...
    def parse_combined_word_forms(self, word_form_text, strong_entry, verse_ref):
        if strong_entry:
            ...
            print
            print(
                "REF {} ) Fixing word form {} to point to headword {}({})".format(
                    verse_ref,
                    word_form_text,
                    strong_entry.headword,
                    strong_entry.strong_number,
                )
            )
            word_form_obj = WordForm().load(
                {
                    "form": word_form_text,
                    "lookups": {
                        "$elemMatch": {
            ...
            if word_form_obj:
                word_form_obj.refs = (
                    word_form_obj.refs + [verse_ref]
                    if hasattr(word_form_obj, "refs")
                    else [verse_ref]
                )
    ...
    def delete_old_word_form_mappings(self, strong_entry, old1, old2, verse_ref):
        ...
        for wf in wfset:
            ...
            wf.lookups = [
                lk
                for lk in wf.lookups
                if not (
                    lk["headword"] == strong_entry.headword
                    and lk["parent_lexicon"] == "BDB Augmented Strong"
                )
            ]
            new_len = len(wf.lookups)
            if old_len > new_len:
                print(
                    " -Deleted lookup {}({}) from wordform {}".format(
                        strong_entry.headword, strong_entry.strong_number, wf.form
                    )
                )
            if new_len < 1:
                print(
                    " -No lookups left, Deleted wordform {} completely".format(wf.form)
                )
                wf.delete()
                continue
            try:
