...
class LaazRashiParser(object):
    ...
    def parse_input(self, filename):
        ...
        l_regex = re.compile(
            "[0-9]{1,4}.*\$?", re.UNICODE
        )  # make sure it's an input line and not comments in the input file
    ...
    def _make_dictionary_entry(self, input_row):
        ...
        if num_parts == 7:
            ...
            if he_regex.search(parts[4]):
                if he_regex.search(
                    parts[5]
                ):  # if the latin transliteation is not here... it's probably in the right place, but contains a bit of hebrew
                    pass
    ...
    def _make_word_form(self, entry):
        ...
        if wf:
            ...
            wf = WordForm(
                {"form": entry.headword, "lookups": [lookup], "refs": [entry.orig_ref]}
            )
        wf.save()
