...
with open("/var/tmp/mishnah_term_coverage.csv", "wb+") as outfile:
                    ...
                    if len(match):
                        if match in results:
                            ...
                            if results[match]["in_lexicon"] is False:
                                results[match]["locations"].append(
                                    Ref("%s %s.%s" % (mishnah.title, chn, msn)).normal()
                                )
                        else:
    ...
    for result in sorted(results):
        row = [
            result.encode("utf-8"),
            results[result]["in_lexicon"],
            results[result]["frequency"],
            ";".join(results[result]["locations"])
            if "locations" in results[result]
            else "",
        ]
