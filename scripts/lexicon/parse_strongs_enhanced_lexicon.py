            ...
            for row in input_csv:
                ...
                if word in word_list:
                    if s_num == word_list[word]["strong_n"]:
                        ...
                        raise Exception(
                            "double word {} but not same numbers".format(
                                word.encode("utf-8")
                            )
                        )
                else:
                    word_list[word] = {
                        ...
                        "list_occur": 1,
                        "found_in_wlc": False,
                        "found_word_form": False,
                    }
        return word_list

    def write_word_csv_dict(self):
        with open(
            os.path.join(self.data_dir, "Words-Comparison.csv"), "wb+"
        ) as outfile:
    ...
    def iterate_over_text(self, compare_mode=False):
        for chap_num, chapter_node in enumerate(
            ...
            for v_num, verse_node in enumerate(
                ...
                for w_num, word in enumerate(
                    verse_node.findall(".//strong:w", self.namespace), 1
                ):

                    word_form_text = self._make_vowel_text(
                        self._strip_wlc_morph_notation(word.text)
                    )
                    ...
                    if strong_number:
                        if compare_mode:
                            ...
                            self.parse_word_forms(
                                word_form_text, strong_number, verse_ref
                            )
...
class StrongHebrewGLexiconXMLParser(object):
    ...
    def __init__(self):
        self.heb_stem_regex = re.compile(
            r"^\((" + "|".join(self.heb_stems) + ")\)", re.IGNORECASE
        )
        self.arc_stem_regex = re.compile(
            r"^\((" + "|".join(self.arc_stems) + ")\)", re.IGNORECASE
        )
        ...
        self.entries_xml = self.dictionary_xml.getroot().findall(
            ".//*[@type='entry']", self.namespace
        )
        self.entries = []
    ...
    def _make_dictionary_entry(self, entry):
        ...
        self._current_entry["language_code"] = headword_xml.get(
            "{http://www.w3.org/XML/1998/namespace}lang"
        )
        defs = [
            x.text for x in entry.findall("strong:list/strong:item", self.namespace)
        ]
    ...
    def _detect_stem_information_in_definition(self, def_item):
        ...
        if heb_stem:
            return self._assemble_sense_def(
                heb_stem.group(1), self.heb_stem_regex.sub("", def_item)
            )
        arc_stem = self.arc_stem_regex.search(def_item)
        if arc_stem:
            return self._assemble_sense_def(
                arc_stem.group(1), self.arc_stem_regex.sub("", def_item)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-l", "--lexicon", help="Parse lexicon", action="store_true")
    parser.add_argument(
        "-w", "--wordform", help="Parse word forms", action="store_true"
    )
