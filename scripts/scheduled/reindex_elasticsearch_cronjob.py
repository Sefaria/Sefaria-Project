...
from scripts.scheduled.reindex_pipeline import run_sheets_catch_up
...
def log_index_state(index_type: str, result: ReindexingResult, debug: bool = False):
    """Log the current state of indexes for a given type."""
    try:
        names = get_new_and_current_index_names(index_type, debug=debug)
        if not names:
...
def run_sheets_by_timestamp(timestamp: str, result: ReindexingResult, debug: bool = False) -> bool:
    """Index sheets modified after timestamp into the live sheet index."""
    logger.debug(f"Starting local sheets-by-timestamp catch-up - timestamp: {timestamp}, debug: {debug}")
    try:
        catch_up_result = run_sheets_catch_up(timestamp, debug=debug)
        succeeded = catch_up_result.get("succeeded", {}).get("num", 0)
        failed = catch_up_result.get("failed", {}).get("num", 0)
        result.record_step_success(
            "sheets_by_timestamp",
            f"succeeded: {succeeded}, failed: {failed}",
        )
        if failed:
            result.add_warning(f"Sheet catch-up had {failed} failures")
        return True
    except Exception as e:
        result.record_step_failure("sheets_by_timestamp", str(e))
        return False


def validate_shard_params(shard_index, shard_count, result: ReindexingResult) -> bool:
    """Require shard identity before indexing; prevents full-corpus duplicate work."""
    if shard_index is None or shard_count is None:
        result.record_step_failure(
            "shard_params",
            "shard_index and shard_count are required in --mode shard "
            "(via CLI or JOB_COMPLETION_INDEX/SHARD_COUNT env vars)",
        )
        return False
    if shard_index < 0 or shard_count < 1 or shard_index >= shard_count:
        result.record_step_failure(
            "shard_params",
            f"Invalid shard params - shard_index: {shard_index}, shard_count: {shard_count}",
        )
        return False
    return True
...
def main(argv=None):
    ...
    parser.add_argument("-d", "--debug", action="store_true", help="Enable debug logging and debug indexes")
    parser.add_argument("--mode", choices=["monolith", "shard"], default="monolith",
                        help="Execution mode: monolith (full run) or shard (index one text shard)")
    parser.add_argument("--type", choices=["text", "sheet"], default="text",
                        help="Index type (shard mode always indexes text)")
    ...
    shard_count = args.shard_count if args.shard_count is not None else (
        int(os.environ["SHARD_COUNT"]) if os.environ.get("SHARD_COUNT") else None
    )
    ...
    last_sheet_timestamp = os.environ.get("SHEET_CATCH_UP_TIMESTAMP") or datetime.now().isoformat()
    logger.debug(f"Sheet catch-up timestamp - timestamp: {last_sheet_timestamp}")
    ...
    if args.mode != "shard":
        try:
            log_index_state('text', result, debug=args.debug)
            log_index_state('sheet', result, debug=args.debug)
    ...
    if args.mode == "monolith":
        ...
        if not run_index_all(result, debug=args.debug):
        ...
        run_sheets_by_timestamp(last_sheet_timestamp, result, debug=args.debug)

    elif args.mode == "shard":
        if not validate_shard_params(shard_index, shard_count, result):
            logger.info(result.get_summary())
            sys.exit(1)
        ...
        logger.info(
            ...
            f"failed_versions: {len(result.failed_text_versions)}, "
