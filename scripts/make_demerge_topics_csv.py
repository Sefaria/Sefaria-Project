...
def make_demerge_csv_sheets(slug_to_demerge, out_file):
    ...
    for sheet in sheets:
        topics = [
            t["asTyped"]
            for t in filter(lambda t: t["slug"] == slug_to_demerge, sheet["topics"])
        ]
        rows += [
            {
                "Id": str(sheet["id"]),
...
def get_args(argv):
    ...
    try:
        ...
        print(
            "make_demerge_topics_csv.py -s <slug_to_demerge> -o <out_file> -t <type (sources or sheets)>"
        )
        sys.exit(2)
    for opt, arg in opts:
        if opt == "-h":
