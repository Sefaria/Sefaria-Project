...
def check_texts(title1, title2_text):
    for i in range(24):
        if i % 5 == 0:  # dont need to check every perek
            ...
            title1_perek = get_text(
                f"{title1} {i+1}", "en", versionTitle=vtitle, server=server
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-r",
        "--restore",
        help="1 or 0.  Restore cauldron to original condition. Delete anything posted in last run.",
        default=0,
    )
    ...
    if restore:
        print("Restoring...")
        assert os.path.exists(joshua_path) and os.path.exists(
            samuel_path
        ), "Didn't yet download Joshua and II Samuel.  Can't restore."
    ...
    print(
        "https://ste.cauldron.sefaria.org/Joshua should now contain the text of II Samuel."
    )
    ...
    check_texts("Joshua", orig_joshua_text)
    print(
        "https://ste.cauldron.sefaria.org/Joshua should now contain the text of Joshua."
    )
