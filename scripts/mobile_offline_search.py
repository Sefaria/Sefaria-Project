...
def save_read_filename(name, ext="json"):
    return "../mobile_offline_search/dumps/{}.{}.{}.{}".format(
        DESCRIPTION, name, "_".join(LANGS), ext
    )
...
class OfflineTextIndexer(object):
    ...
    def index_segment(cls, text, ref, heRef, version):
        ...
        if title != cls.curr_title:
            ...
            cls.ref_num_min_N_title.append(
                (
                    cls.ref_num,
                    title,
                )
    ...
    def parse_lib_to_json(cls, start, end):
        ...
        cls.ref_num_min_N_title = (
            []
        )  # min ref_num of each book.title [[min_ref_num, book_title], ...]
        ...
        for i, index in enumerate(indexes):
            ...
            try:
                ...
                for section_ref in section_refs:
                    ...
                    ref_part = re.sub(
                        r"^{}".format(re.escape(title)), "", section_ref.normal()
                    )
        ...
        words_2_ref_nums = {
            key: sorted(list(value)) for key, value in words_2_ref_nums.items()
        }
...
def get_words(text):
    ...
    text = re.sub(
        r"[\u05be\s+\.\-;:,?!{}]", " ", text
    )  # convert dashs/dots/etc to space
    # text = re.sub(ur'[\u0591-\u05C7\u05f3\u05f4]', '', text)
    # text = re.sub(ur'[\u0591-\u05c7]', '', text)

    text = re.sub(
        r"([^\u05d0-\u05eaA-Za-z0-9\s])", "", text
    )  # remove non-regular chars
    text = text.lower()
    # bf_text = text
    text = text.replace(" \u05d5", " ")
...
def convert_to_josh_packets(words_2_ref_nums):
    ...
    while True:
        ...
        if packet_count < 2**PACKET_INDEXING_BITS:
            ...
            print(
                (
                    "TOO MANY PACKETS (only have 8 bits) NEED TO RAISE CHUNK SIZE to: ",
                    chunk_size,
                )
            )
    ...
    for word, ref_nums in words_2_ref_nums.items():
        ...
        for packet_index in range(packet_count):
            packet_bits = bitstring[
                (packet_index * PACKET_SIZE) : ((packet_index + 1) * PACKET_SIZE)
            ]
        ...
        if words_done % 100000 == 0:
            print(
                (
                    words_done,
                    1.0 * words_done / len(words_2_ref_nums),
                    str(dt.now().time()),
                )
            )
    ...
    words_2_packets[METADATA_CHUNKS_PACKETSIZE] = [
        chunk_size,
        PACKET_SIZE,
    ]  # STORE METADATA into the table itself
...
###### GETTING data:
def get_from_word_2_ref(
    word, words_2_ref_nums, ref_num_min_N_title, ref_num_2_part, ref_num_2_full_name
):
    ...
    for r in ref_strs:
        ...
        print(
            ("text from ref search", r, word in full_text)
...
def get_from_json(word):
    return get_from_word_2_ref(
        word,
        read(WORDS_2_REF_NUMS),
        read(REF_NUM_MIN_N_TITLE),
        read(REF_NUM_2_PART),
        None,
    )
...
def get_from_db(word):
    ...
    print(
        (_id, blob, hex_str, len(hex_str), chunk_size, PACKET_SIZE)
    )  # 200, 24 ... this looks correct
    ##
    ...
    for word_id, blob in cur.fetchall():
        ...
        for index in range(
            0, len(hex_str), 8
        ):  # each hex is half a byte and 4 bytes to a JH_packet
            ...
            packet_bits = "".join(
                [
                    packet_bits[i - 8 : i][::-1]
                    for i in range(8, len(packet_bits) + 1, 8)
                ]
            )
            for bit_index, bit in enumerate(packet_bits):
                if bit == "1":
                    chunk_start_num = (
                        packet_index * PACKET_SIZE + bit_index
                    ) * chunk_size
