...
class RefPartModifier:
    ...
    def t(self, **kwargs):
        ...
        term = NonUniqueTerm({"slug": slug, "titles": []})
        for lang in ("en", "he"):
    ...
    def t_from_titled_obj(self, obj, ref_part_role):
        ...
        alt_en_titles = [
            title for title in title_group.all_titles("en") if title != en_title
        ]
        alt_he_titles = [
            title for title in title_group.all_titles("he") if title != he_title
        ]
        return self.t(
            en=en_title,
            he=he_title,
            alt_en=alt_en_titles,
            alt_he=alt_he_titles,
            ref_part_role=ref_part_role,
        )
    ...
    def modify_bavli_commentaries(
        self, fast=False, create_dhs=False, add_alt_structs=False
    ):
        """
        ...
        for comm_ref_prefix, comm_term_slug, comm_cat_path in (
            ...
            (
                "Gilyon HaShas on ",
                "gilyon-hashas",
                ["Talmud", "Bavli", "Acharonim on Talmud", "Gilyon HaShas"],
            ),
        ):
            ...
            for index in tqdm(indexes, desc=comm_term_slug, total=indexes.count()):
                self.add_new_fields_to_commentary_root_node(
                    comm_ref_prefix, comm_term_slug, index, fast
                )
                if add_alt_structs:
                    self.add_alt_structs_to_talmud_commentaries(
                        comm_ref_prefix, comm_term_slug, index
                    )
                if create_dhs:
                    self.add_dibur_hamatchils(index)

    def add_new_fields_to_commentary_root_node(
        self, comm_ref_prefix, comm_term_slug, index, fast=False
    ):
        ...
        n.referenceableSections = (
            [True, False, True] if n.depth == 3 else [True, True]
    ...
    def add_dibur_hamatchils(self, index):
        index = Index().load(
            {"title": index.title}
        )  # reload index to make sure perek nodes are correct
        seg_perek_mapping = {}
            ...
            DiburHamatchilNode(
                {
                    "dibur_hamatchil": dh,
                    "container_refs": [
                        segment_ref.top_section_ref().normal(),
                        perek_ref,
                        index.title,
                    ],
    ...
    def add_alt_structs_to_talmud_commentaries(
        self, comm_ref_prefix, comm_term_slug, index
    ):
        base_index = library.get_index(index.base_text_titles[0])
    ...
    def modify_bavli(self, fast=False):
        ...
        minor_tractates = {
            title for title in library.get_indexes_in_category("Minor Tractates")
        }
        ...
        for index in tqdm(indexes, desc="talmud", total=indexes.count()):
            ...
            for iperek, perek_node in enumerate(perakim):
                perek_term = self.t(
                    he=perek_node.get_primary_title("he"), ref_part_role="structural"
                )  # TODO english titles are 'Chapter N'. Is that an issue?
                ...
                perek_node.match_templates = [
                    {"term_slugs": [perek.slug, perek_term.slug], "scope": "any"},
                    {
                        "term_slugs": [
                            perek.slug,
                            self.perek_number_map[numeric_equivalent].slug,
                        ]
                    },
                    {"term_slugs": [perek_term.slug], "scope": "any"},
                    ...
                    perek_node.match_templates += [
                        {
                            "term_slugs": [
                                perek.slug,
                                self.perek_number_map["last"].slug,
                            ]
                        },
    ...
    def modify_base_text_commentaries(
        self, base_text_title, base_match_templates, fast=False
    ):
        # commentaries
        ...
        for comm_index in comm_indexes:
            ...
            for template in base_match_templates:
                ...
                temp_match_template["term_slugs"] = [comm_term.slug] + template[
                    "term_slugs"
                ]
            ...
            if index.categories[-1] == "Torah":
                for parsha_node in index.get_alt_struct_nodes():
                    ...
                    parsha_node.match_templates = [
                        ...
                        {"term_slugs": [parsha_term.slug], "scope": "any"},
                    ]
            if fast:
                self.fast_index_save(index)
            else:
                index.save()
            self.modify_base_text_commentaries(
                index.title, [{"term_slugs": [index_term.slug]}], fast=fast
            )

    def modify_rest_of_shas(self, fast=False):
        title_swaps = {"Moed Kattan": "Moed Katan", "Oktsin": "Oktzin"}
        for cat, title_prefix in [
            ("Mishnah", "Mishnah "),
            ("Tosefta", "Tosefta "),
            ...
            for index in tqdm(indexes, desc=cat, total=indexes.count()):
                ...
                if cat == "Mishnah":
                    index.nodes.match_templates += [
                        ...
                        {"term_slugs": ["tractate", generic_term.slug]},
                    ]
                if fast:
                    self.fast_index_save(index)
                else:
    ...
    def modify_sifra(self, fast=False):
        ...
        index.nodes.match_templates = [{"term_slugs": ["sifra"]}]
        for node in index.nodes.children:
            ...
            if parsha_term is None:
                ...
                node.match_templates = [{"term_slugs": [node_term.slug]}]
            else:
                node.match_templates = [{"term_slugs": [parsha_term.slug]}]
            for par_per_node in node.children:
                ...
                if named_term_slug is None:
                    alt_titles = other_perek_node_map[
                        re.search(r"^(.+) \d+$", temp_title).group(1)
                    ]
                    named_term_slug = self.t(
                        en=temp_title,
                        he=par_per_node.get_primary_title("he"),
                        alt_en=alt_titles,
                        ref_part_role="structural",
                    ).slug
                ...
                num_term = self.perek_number_map[
                    numeric_equivalent
                ]  # NOTE: these terms can be used for both parsha and perek nodes b/c they only contain a "פ" prefix.
    ...
    def modify_mishneh_torah(self, fast=False):
        for index in tqdm(
            library.get_indexes_in_category("Mishneh Torah", full_records=True),
            desc="mishneh torah",
        ):
            ...
            self.modify_base_text_commentaries(
                index.title, base_match_templates, fast=fast
            )

    def modify_tur(self, fast=False):
        index = library.get_index("Tur")
        sa_title_swaps = {"Orach Chaim": "Orach Chayim", "Yoreh Deah": "Yoreh De'ah"}
        term = self.t_from_titled_obj(
            TitleGroup(index.schema["titles"]), ref_part_role="structural"
        )
        index.nodes.match_templates = [{"term_slugs": [term.slug]}]
        for node in index.nodes.children:
            node_title = node.get_primary_title("en")
            node_title = sa_title_swaps.get(node_title, node_title)
            sa_term = self.sa_map.get(node_title, None)
            node.match_templates = [{"term_slugs": [sa_term.slug]}]
        if fast:
            self.fast_index_save(index)
        else:
            index.save()
    ...
    def modify_shulchan_arukh(self, fast=False):
        for index in library.get_indexes_in_category(
            "Shulchan Arukh", full_records=True
        ):
            ...
            self.modify_base_text_commentaries(
                index.title, [{"term_slugs": [sa_term.slug]}], fast
            )
            if fast:
                self.fast_index_save(index)
            else:
    ...
    def create_base_non_unique_terms(self):
        ...
        self.t(
            en="Bavli",
            he="בבלי",
            alt_en=["Babylonian Talmud", "B.T.", "BT", "Babli"],
            ref_part_role="structural",
        )
        ...
        self.t(
            en="Tractate",
            he="מסכת",
            alt_en=["Masekhet", "Masechet", "Masekhes", "Maseches"],
            ref_part_role="alt_title",
        )
        self.t(en="Rashi", he='רש"י', alt_he=['פירש"י'], ref_part_role="structural")
        self.t(
            en="Mishnah",
            he="משנה",
            alt_en=["M.", "M", "Mishna", "Mishnah", "Mishnaiot"],
            ref_part_role="structural",
        )
        self.t(
            en="Tosefta",
            he="תוספתא",
            alt_en=["Tosephta", "T.", "Tosef.", "Tos."],
            ref_part_role="structural",
        )
        self.t(
            en="Yerushalmi",
            he="ירושלמי",
            alt_en=["Jerusalem Talmud", "J.T.", "JT"],
            ref_part_role="structural",
        )
        self.t(
            en="Tosafot",
            he="תוספות",
            alt_he=["תוס'", 'תוד"ה', 'תד"ה', "תו'"],
            alt_en=["Tosaphot"],
            ref_part_role="structural",
        )
        ...
        self.t(
            en="Midrash Rabbah",
            he="מדרש רבה",
            alt_en=["Midrash Rabba", "Midrash Rabah"],
            alt_he=["מדרש רבא"],
            ref_part_role="structural",
        )  # TODO no good way to compose titles for midrash rabbah...
        self.t(
            en="Rabbah",
            he="רבה",
            alt_en=[
        ...
        self.t(
            en="Halakha",
            he="הלכה",
            alt_en=["Halakhah", "Halacha", "Halachah", "Halakhot"],
            ref_part_role="context_swap",
        )
        self.t(
            en="Mishneh Torah",
            he="משנה תורה",
            alt_en=["Mishnah Torah"],
            ref_part_role="structural",
        )
        ...
        self.t(
            ...
            alt_he=['שו"ע', "שלחן ערוך"],
            ref_part_role="structural",
        )
        self.t(
            en="Hilchot",
            he="הלכות",
            alt_en=["Laws of", "Laws", "Hilkhot", "Hilhot"],
            alt_he=["הל'"],
            ref_part_role="alt_title",
        )
        self.t_from_titled_obj(
            Term().load({"name": "Parasha"}), ref_part_role="alt_title"
        )
        for old_term in TermSet(
            {"scheme": {"$in": ["toc_categories", "commentary_works"]}}
        ):
        ...
        for name in missing_old_term_names:
            new_term = self.t_from_titled_obj(
                Term().load({"name": name}), ref_part_role="structural"
            )
    ...
    def create_shas_terms(self):
        ...
        repl_reg = rf'^({"|".join(re.escape(r) for r in repls)}) '
        ...
        for index in indexes:
            title_map[
                (
                    index.title.replace("Mishnah ", ""),
                    index.get_title("he").replace("משנה ", ""),
                )
            ] |= {
                re.sub(r"<[^>]+>", "", re.sub(repl_reg, "", tit["text"]))
                for tit in index.nodes.title_group.titles
            }
        ...
        for index in tqdm(indexes, desc="talmud", total=indexes.count()):
            if index.title in minor_tractates:
                continue
            title_map[(index.title, index.get_title("he"))] |= {
                re.sub(r"<[^>]+>", "", re.sub(repl_reg, "", tit["text"]))
                for tit in index.nodes.title_group.titles
            }
        ...
        for title in [
            ...
            title_map[
                (
                    index.title.replace("Tosefta ", ""),
                    index.get_title("he").replace("תוספתא ", ""),
                )
            ] |= {
                re.sub(r"<[^>]+>", "", re.sub(repl_reg, "", tit["text"]))
                for tit in index.nodes.title_group.titles
        ...
        title_term_map = {}
        for (generic_title_en, generic_title_he), alt_titles in sorted(
            title_map.items(), key=lambda x: x[0]
        ):
            alt_he = [
                tit for tit in alt_titles if is_hebrew(tit) and tit != generic_title_he
            ]
            alt_en = [
                tit
                for tit in alt_titles
                if not is_hebrew(tit) and tit != generic_title_en
            ]
    ...
    def create_tanakh_terms(self):
        ...
        for index in tqdm(indexes, desc="tanakh", total=indexes.count()):
            ...
            titles += [
                {"text": alt_title, "lang": "en"}
                for alt_title in hard_coded_tanakh_map.get(index.title, [])
            ]
            titles += [
                {"text": alt_title_obj["text"].replace(".", " ."), "lang": "en"}
                for alt_title_obj in titles
                if re.search(r"\.$", alt_title_obj["text"]) is not None
            ]
            index_term = NonUniqueTerm(
                {"slug": index.title, "titles": titles, "ref_part_role": "structural"}
            )
            index_term.save()
            ...
            parsha_title_regex = "|".join(
                re.escape(title["text"]) for title in parsha_title_term.titles
            )
            if index.categories[-1] == "Torah":
                for parsha_node in index.get_alt_struct_nodes():
                    ...
                    titles += [
                        ...
                        for alt_title in hard_coded_parsha_map.get(
                            parsha_node.get_primary_title("en"), []
                        )
                    ]
                    titles += [
                        {"text": alt_title_obj["text"].replace(".", " ."), "lang": "en"}
                    ...
                    for title in titles:
                        title["text"] = re.sub(
                            rf"(?:{parsha_title_regex}) ", "", title["text"]
                        )
    ...
    def create_mt_terms(self):
        ...
        repl_reg = rf'^(({"|".join(re.escape(r) for r in repls)}) )?(({"|".join(re.escape(r) for r in hil_repls)}) )?'
    ...
    def create_sa_terms(self):
        ...
        for index in indexes:
            title_map[
                (index.title.replace("Shulchan Arukh, ", ""), index.get_title("he"))
            ] |= {
                re.sub(r"<[^>]+>", "", re.sub(repl_reg, "", tit["text"]))
                .replace("סי'", "")
                .strip()
    ...
    def modify_all(self):
        ...
        self.modify_bavli_commentaries(
            fast, create_dhs, add_comm_alt_structs
        )  # on first run, rerun because ArrayMapNodes are cached
