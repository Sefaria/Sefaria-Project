...
for title in base_titles:
    ...
    while current_rashi:
        ...
        for l in links:
            if (
                l != current_base
                and l.index.categories[0] == "Tanakh"
                and (not l.is_dependant())
                and l.is_segment_level()
            ):
...
sum_edges = 0
sorted_base_titles = sorted(
    base_titles, key=lambda idx: library.get_index(idx).order[0]
)
for title in sorted_base_titles:
    ...
    for verse in sorted_verses:
        ...
        flat_results.append(
            {"name": verse, "size": 0, "imports": book_links_dict[verse]}
        )
