...
def delete_link(id_or_ref, server="", API_KEY="", VERBOSE=False):
    ...
    result = http_request(
        url, body={"apikey": API_KEY}, json_payload=url, method="DELETE"
    )
    if VERBOSE:
        print(result)
    return result
...
def post_link(
    ...
    url = (
        ...
        else server + "/api/links/?skip_lang_check=1"
    )
    result = http_request(
        url, body={"apikey": API_KEY}, json_payload=info, method=method
    )
    if VERBOSE:
        print(result)
    return result
...
def http_request(url, params=None, body=None, json_payload=None, method="GET"):
    ...
    if json_payload:
        body["json"] = json.dumps(
            json_payload
        )  # Adds the json as a url parameter - otherwise json gets lost
...
def check_links(
    current_II_Samuel_links, orig_II_Samuel_links, additional_II_Samuel_links
):
    ...
    additional_II_Samuel_links = {
        str(sorted(l["refs"])) for l in additional_II_Samuel_links
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--server", help="Sefaria server")
    parser.add_argument(
        "-q", "--quantity", default=-1, help="Amount of links to post."
    )  # -1 means post all links
    parser.add_argument(
        "-r",
        "--restore",
        help="Restore cauldron to original condition. Delete anything posted in last run.",
        default=0,
    )
    ...
    if os.path.exists(samuel_path):
        ...
        for i, l in tqdm(enumerate(orig_II_Samuel_links)):
            orig_II_Samuel_links[i] = {
                "refs": [l["ref"], l["anchorRef"]],
                "auto": True,
                "generated_by": "chaos script",
                "type": "Commentary",
            }
    ...
    if restore:
        assert os.path.exists(
            samuel_path
        ), "Did not download II Samuel links yet.  Therefore, can't delete them."
    ...
    if os.path.exists(additional_samuel_path):
        ...
        for i, l in tqdm(enumerate(Joshua_links)):
            ...
            additional_II_Samuel_links.append(
                {
                    "refs": [new_samuel_ref, other_ref],
                    "auto": True,
                    "generated_by": "chaos script",
                    "type": "Commentary",
                }
    ...
    additional_II_Samuel_links = (
        additional_II_Samuel_links[0:quantity]
        if quantity > 0
        else additional_II_Samuel_links
    )
    if quantity > len(additional_II_Samuel_links):
        print(
            f"Warning: Cannot post {quantity} as there are only {len(additional_II_Samuel_links)} links to post"
        )
    ...
    if len(final_II_Samuel_links) == 0:
        ...
        print(
            f"Finding {len(final_II_Samuel_links)} and expecting {len(orig_II_Samuel_links)} on {server}.  Post unsuccessful"
        )
