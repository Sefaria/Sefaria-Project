...
def print_out_refs(daf, line, segment, prev_daf, prev_line, prev_segment):
    second = "{} {}:{}:{}".format(
        title, AddressTalmud.toStr("en", daf + 1), line + 1, segment + 1
    )
    first = "{} {}:{}:{}".format(
        title, AddressTalmud.toStr("en", prev_daf + 1), prev_line + 1, prev_segment + 1
    )
            ...
            for line in range(len(text[daf])):
                for segment in range(len(text[daf][line])):
                    ...
                    if prev == curr and (prev_daf != daf or prev_line != line):
                        print_out_refs(
                            daf, line, segment, prev_daf, prev_line, prev_segment
                        )
                        diff_line_cases += 1
                    elif prev == curr:
                        same_line_cases.append(
                            "{} {}:{}:{}".format(
                                title,
                                AddressTalmud.toStr("en", daf + 1),
                                line + 1,
                                segment + 1,
                            )
                        )
