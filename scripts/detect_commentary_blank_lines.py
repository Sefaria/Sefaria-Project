...
def find_blanks_at_start(vlist):
    if isinstance(vlist, list) and len(vlist) > 1:
        index = next(
            i for i, j in enumerate(vlist) if (isinstance(j, (str)) and j.strip() != "")
        )
        return index > 0
    return 0
                    ...
                    if offset > 0:
                        ...
                        if fix:
                            if commentary.versionTitle == "On Your Way":
                                ...
                                print(
                                    "Slicing verse (has %d comments) from %d"
                                    % (len(verse), offset)
                                )
                                commentary.chapter[ch - 1][vs - 1][:] = verse[offset:]
                                commentary.save()
                            deleted_segment = "%s %s:%s:%s" % (
                                commentary.title,
                                ch,
                                vs,
                                len(verse),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--fix", help="Also fix detected blank lines", action="store_true"
    )
