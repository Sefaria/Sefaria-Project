                ...
                for match in matches:
                    ...
                    if len(match):
                        if match in results:
                            ...
                            if wf:
                                results[match] = {'in_lexicon' : True}
                            else:
                                results[match] = {'in_lexicon' : False, 'locations': [Ref("%s %s.%s" % (mishnah.title, chn, msn)).normal()]}
    for result in sorted(results):
        row = [result.encode('utf-8'), results[result]['in_lexicon'], ";".join(results[result]['locations']) if 'locations' in results[result] else '']
