    ...
    for row in tags:
        ...
        if parent_term.load_by_title(parent_tag) and term.load_by_title(tag):
            ...
            if tag_term == parent_tag_term:
                ...
                if (
                    tag == tag_term.get_primary_title()
                    and parent_tag == parent_tag_term.get_primary_title()
                ):
                    titles_to_add = tag_term.get_titles_object()
                    for t in titles_to_add:
                        parent_tag_term.add_title(
                            t["text"], t["lang"]
                    ...
                    conflicting_terms.append(
                        (
                            parent_tag,
                            tag,
                            parent_tag_term.get_primary_title(),
                            tag_term.get_primary_title(),
                        )
                    )
