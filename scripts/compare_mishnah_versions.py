        ...
        for idx in index_set:
            ...
            for chnum, chapter in enumerate(version.chapter, 1):
                ...
                for verse in chapter:
                    verses += re.sub(
                        r"\s{2,}", r" ", strip_cantillation_vowel_regex.sub("", verse)
                    ).strip()
...
def calculate_len_diffs(idx, version1, version2):
    ...
    if v1_len != v2_len:
        results.append(
            "%s chapter count mismatch. [%s has %d|%s has %d]\n"
    ...
    for chapter1, chapter2 in zip(version1.chapter, version2.chapter):
        ...
        if vs1_len != vs2_len:
            results.append(
                "%s chapter %s mishnayot count mismatch. [%s has %d|%s has %d]\n"
                % (
                    idx.title,
                    chapter_num,
                    version1.versionTitle,
                    vs1_len,
                    version2.versionTitle,
                    vs2_len,
                )
            )
        chapter_num += 1
...
print_texts(
    "mishnah - Wikisource Mishna.txt", mishnah_books, mishnah_version_a, "he", True
)
print_texts(
    "mishnah - Torat Emet 357.txt", mishnah_books, mishnah_version_b, "he", True
)
for book in mishnah_books:
    v1 = Version().load(
        {"title": book.title, "versionTitle": mishnah_version_a, "language": "he"}
    )
    v2 = Version().load(
        {"title": book.title, "versionTitle": mishnah_version_b, "language": "he"}
    )
