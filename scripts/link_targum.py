...
def link_targums(prepend, titles):
    for title in titles:
        ...
        if options.verify_mode:
            ...
            assert b_len == t_len, "{}, {}/{}".format(b_ref.normal(), b_len, t_len)
            ...
            for b_ref in b_index.all_segment_refs():
                t_ref = Ref(prepend + b_ref.normal())
                assert not t_ref.is_empty(), "{} empty".format(t_ref.normal())
                assert not b_ref.is_empty(), "{} empty".format(b_ref.normal())
